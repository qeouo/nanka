	...
	var PoseArmature = ret.PoseArmature = (function(){
		...
		ret.prototype.mixBoneMatrix = function(n){
			...
			if(bone.parent){
				...
				Mat43.dot(matrix,this.matrices[m],matrix); //親行列掛ける
			}
			...
			if(this.object.poseBones){
				...
				for(var i=0;i<poseBone_.constraints.length;i++){
					...
					case "COPY_ROTATION":
						...
						Mat33.copy(matrix,constraint.target.mixedmatrix); //角度を強制的に設定
						break;
...
//					setdata(dstobj,src[member]);
	...
	var freezeMesh =ret.freezeMesh= function(dst,obj,physics){
		//モデファイアとワールド行列を反映
	...
	ret.moveCollision=function(collision,object,ono3d){
		...
		case Collider.CAPSULE:
			bold = Math.max(sx,sz);
			sy=Math.max((b[4]-b[1])*0.5 -bold,0)*scale[1];
			break;
		case Collider.MESH:
