...
ret.init=function(_gl){
	...
	ret.commonFunction=" \n \
		...
		lowp vec4 packR11G11B10(highp vec3 raw){ \n \
			mediump vec3 idx = log2(raw+0.000001); \n \
			idx = clamp(ceil(idx),-0.0,1.0);  \n \
			vec3 num = raw/exp2(idx);\n \
			num=clamp(num,0.0,1.0);\n \
			idx += 0.0; \n \
			vec4 result; \n \
			result.r = floor((num.r )*63.0)*4.0 + floor(idx.r/8.0); \n \
			result.g = floor((num.g )*63.0)*4.0 + floor(idx.g/8.0); \n \
			result.b = floor((num.b )*31.0)*8.0 + floor(idx.b/4.0); \n \
			result.a = fract(idx.r/8.0)*8.0*32.0 + fract(idx.g/8.0)*8.0*4.0 + fract(idx.b/4.0)*4.0; \n \
			clamp(result,0.0,255.0); \n \
			return floor(result)/255.0; \n \
		} \n \
		highp vec3 unpackR11G11B10(lowp vec4 raw){ \n \
			mediump vec3 idx; \n \
			mediump vec4 num; \n \
			num = floor(raw*255.0); \n \
			idx.r = fract(num.r/4.0)*4.0*8.0 + floor(num.a/32.0); \n \
			idx.g = fract(num.g/4.0)*4.0*8.0 + floor(fract(num.a/32.0)*8.0); \n \
			idx.b = fract(num.b/8.0)*8.0*4.0 + floor(fract(num.a/4.0)*4.0); \n \
			num.r = floor(num.r/4.0)/63.0; \n \
			num.g = floor(num.g/4.0)/63.0; \n \
			num.b = floor(num.b/8.0)/31.0; \n \
			return  (num.rgb )* exp2(idx -0.0) ; \n \
		} \n \
