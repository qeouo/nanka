...
ret.init=function(_gl){
	...
	ret.commonFunction=" \n \
		...
		lowp vec2 packUFP16(mediump float raw){ \n \
			...
			mediump float idx = clamp(floor(log2(raw)),-16.0,15.0);  \n \
			mediump float f = raw /exp2(idx) -1.0; \n \
			return vec2(floor(f*256.0),floor(fract(f*256.0)*8.0)*32.0 + (idx+16.0))/255.0; \n \
		} \n \
		mediump float unpackUFP16(lowp vec2 src){ \n \
			if(src.r==0.0 && src.g==0.0)return 0.0; \n \
			mediump float idx = floor(fract(floor(src.g*255.5)/32.0) * 32.0 - 16.0); \n \
			mediump float b = floor(src.g *255.5 /32.0) /8.0 ; \n \
			return ((floor(src.r*255.5)/256.0 + b/256.0) +1.0)* exp2(idx) ; \n \
		} \n \
