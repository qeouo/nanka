...
ret.init=function(_gl){
	...
	ret.commonFunction=" \n \
		...
		lowp vec4 packR11G11B10(mediump vec3 raw){ \n \
			...
			num = (num/exp2(idx) -1.0) * sign(raw);\n \
			idx += 16.0; \n \
			return vec4(\n \
				idx * 8.0 + floor(vec3(num.r,fract(num.r * 8.0),num.b)*8.0) \n \
				,floor(num.g * 64.0)*4.0 + fract(num.b * 8.0)*4.0) \n \
				/255.0; \n \
		} \n \
		const lowp vec4 v_8884 = 1.0/vec4(8.0,8.0,8.0,4.0); \n \
		mediump vec3 unpackR11G11B10(lowp vec4 raw){ \n \
			mediump vec4 num; \n \
			mediump vec4 num2; \n \
			num = floor(raw*255.5); \n \
			num2 = fract(num * v_8884); \n \
			num2.rb = num2.rb + num2.ga/8.0; \n \
			num2.g = floor(num.a/4.0)/64.0; \n \
			return  (num2.rgb +1.0)* exp2(floor(num.rgb/8.0)-16.0) ; \n \
		} \n \
