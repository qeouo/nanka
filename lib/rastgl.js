...
ret.init=function(_gl){
	...
	ret.commonFunction=" \n \
		...
		lowp vec4 packR11G11B10_(mediump vec3 raw){ \n \
			lowp vec3 idx = ceil(log2(clamp(raw,1.0,256.0))); \n \
			idx = floor(idx/2.0); \n \
			lowp vec3 idx2 = idx; \n \
			idx2 = floor(idx *2.0); \n \
		...
		mediump vec3 unpackR11G11B10_(lowp vec4 raw){ \n \
			lowp vec3 idx = floor(fract(raw.a*255.0 / vec3(256.0,32.0,4.0))*vec3(8.0,8.0,4.0)*2.0); \n \
		...
		lowp vec4 packR11G11B10(mediump vec3 raw){ \n \
			mediump vec3 num = clamp(raw,0.00002,32767.0); \n \
			mediump vec3 idx = floor(log2(num)); \n \
			num = num/exp2(idx) -1.0;\n \
			num = num * sign(raw); \n \
			idx += 16.0; \n \
			vec4 result; \n \
			result.rgb = idx * 8.0 + vec3(floor(num.r * 8.0),floor(fract(num.r * 8.0)*8.0),floor(num.b*8.0)); \n \
			result.a = floor(num.g * 64.0)*4.0 + floor(fract(num.b * 8.0)*4.0); \n \
			return result/255.0; \n \
		} \n \
		const lowp vec3 v_256324 = 1.0/vec3(256.0,32.0,4.0); \n \
		mediump vec3 unpackR11G11B10(lowp vec4 raw){ \n \
			mediump vec3 idx; \n \
			mediump vec4 num; \n \
			mediump vec3 num2; \n \
			num = floor(raw*255.5); \n \
			idx = floor(num.rgb / 8.0); \n \
			num2.r = fract(num.r/8.0) + fract(num.g/8.0)/8.0; \n \
			num2.g = floor(num.a/4.0)/64.0; \n \
			num2.b = fract(num.b/8.0) + fract(num.a/4.0)/8.0; \n \
			return  (num2 +1.0)* exp2(idx-16.0) ; \n \
		} \n \
		lowp vec4 packRGBE(mediump vec3 src){ \n \
			mediump float m = max(src.r,max(src.g,src.b)); \n \
			mediump float idx = ceil(log2(m+0.001)); \n \
 ...
 \n \
		...
		mediump float decodeShadow(sampler2D tex,vec2 texsize,vec2 uv){ \n \
			...
			return mix(mix(unpackUXP16(texture2D(tex,uv).rg) \n \
					,unpackUXP16(texture2D(tex,uv+vec2(unit.x,0.0)).rg),r.x) \n \
				,mix(unpackUXP16(texture2D(tex,uv+vec2(0.0,unit.y)).rg) \n \
					,unpackUXP16(texture2D(tex,uv+unit).rg),r.x) \n \
				,r.y); \n \
		} \n \
...
//			+ floor(fract(num.a * v_256324) * v884); \n \
