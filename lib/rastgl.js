...
ret.init=function(_gl){
	...
	ret.commonFunction=" \n \
		...
		lowp vec4 packR11G11B10(mediump vec3 raw){ \n \
			mediump vec3 num = clamp(raw,0.00002,65535.0); \n \
			mediump vec3 idx = floor(log2(num)); \n \
			num = (num/exp2(idx) -1.0) * sign(raw);\n \
			return (vec4((idx +16.0),floor(num.b * 32.0))*8.0\n \
				+ floor(vec4(num.rg,fract(num.rg * 8.0))*8.0) \n \
			)/255.0; \n \
		} \n \
		mediump vec3 unpackR11G11B10(lowp vec4 raw){ \n \
			mediump vec4 hi= floor(raw*255.5/8.0); \n \
			mediump vec4 lo = fract(raw*255.5/8.0); \n \
