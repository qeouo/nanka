...
var stereoDraw=function(ono3d,func){
...
var initAtt=function(program,attName,size,type,buffer){
	var arg={};
...
ret.framebuffer=function(ono3d){
	...
	gaussfilter(frameBuffer,fTexture2,100);
	gl.enable(gl.BLEND);
	gl.blendFunc(gl.SRC_ALPHA,gl.ONE);
	copyframe(null,fTexture2);
...
var gaussfilter=function(dst,src,d){
...
ret.init=function(_gl,_ono3d){
	...
	ono3d = _ono3d;
	try{
		posGlBuffer=createArrayBuffer(gl.ARRAY_BUFFER,VERTEX_MAX*3);
		colorGlBuffer=createArrayBuffer(gl.ARRAY_BUFFER,VERTEX_MAX*4);
		normalGlBuffer=createArrayBuffer(gl.ARRAY_BUFFER,VERTEX_MAX*3);
		uvGlBuffer=createArrayBuffer(gl.ARRAY_BUFFER,VERTEX_MAX*2);
		emiGlBuffer=createArrayBuffer(gl.ARRAY_BUFFER,VERTEX_MAX);
//		gl.bufferData(gl.ARRAY_BUFFER, emiGlBuffer.jsbuffer, gl.STATIC_DRAW);
		...
		fTexture = createTexture(null,1024,1024);
		gl.bindTexture(gl.TEXTURE_2D, fTexture);
