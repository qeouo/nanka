...
ret.init=function(_gl){
	...
	ret.commonFunction=" \n \
		...
		lowp vec2 packUFP16(highp float ff){ \n \
			float idx = ceil(log2(ff+0.001)); \n \
			float f = ff/exp2(idx); \n \
			float a = floor(ff*256.0); \n \
			float b = floor((ff - a)*8.0); \n \
			return vec2(a,b*32.0 + (idx+16.0))/255.0; \n \
		} \n \
		highp float unpackUFP16(lowp vec2 src){ \n \
			float idx = floor(fract(src.g*8.0) * 32.0 - 16.0); \n \
			float b = floor(src.g*8.0) /8.0 / 256.0; \n \
			return (src.r + b) * exp2(idx) ; \n \
		} \n \
 ...
 \n \
		...
		highp vec2 decodeFull_(lowp vec4 src){ \n \
			return vec2(unpackUXP16(src.rg),unpackUXP16(src.ba)); \n \
		} \n \
