...
,["aPos"],[ "uPosScale", "uPosOffset", "uUvScale", "uUvOffset","uSampler"]);
	 ...
	 ret.copyframe =function(src,x,y,w,h,u,v,u2,v2,sh){
		if(v==null){
			ret.copyframe(src,0,0,2,2,x,y,w,h,u);
			return;
		}
...
var copyframe=ret.copyframe =function(src,x,y,w,h,u,v,u2,v2,sh){
	Paste.copyframe(src,x,y,w,h,u,v,u2,v2,sh);
}
...
ret.init=function(_gl){
	...
	ret.commonFunction=" \n \
		vec4 encode(vec4 src){ \n \
			...
			float idx = floor(log2(m))+1.0; \n \
			return vec4(src.rgb / exp2(idx),(idx+127.0)/256.0); \n \
		} \n \
		vec4 decode(vec4 src){ \n \
			float idx = floor(src.a * 256.0 - 127.0); \n \
	...
	ret.textureRGBE=" \n \
		vec4 textureRGBE(sampler2D tex,vec2 texsize,vec2 uv){ \n \
			...
			return mix(mix(decode(texture2D(tex,uv)) \n \
					,decode(texture2D(tex,uv+vec2(1.0/texsize.x,0.0))),r.x) \n \
				,mix(decode(texture2D(tex,uv+vec2(0.0,1.0/texsize.y))) \n \
					,decode(texture2D(tex,uv+1.0/texsize)),r.x) \n \
				,r.y); \n \
		} \n \
	";
