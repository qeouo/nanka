...
var stereofunc=function(func,ono2d){
...
"
	...
	args["aUv"].buffer.itemSize=2;
	args["aUv"].buffer.numltems=3;
	args["aColor"]=createBuffer2(program,"aColor",4,gl.FLOAT);
	args["uColor"]=gl.getUniformLocation(program,"uColor");
	...
	mainShader.args=args;
}	

var drawMain=ret.drawMain =function(ono3d){
	var renderface
	var posindex=0;
	var colorindex=0;
	var uvindex=0;
...
//				texture=renderface.texture.gltexture;
			...
			emibuffer[posindex]=renderface.emt;
			emibuffer[posindex+1]=renderface.emt;
			emibuffer[posindex+2]=renderface.emt;
		...
		gl.bindBuffer(gl.ARRAY_BUFFER, args["aNormal"].buffer);
		gl.enableVertexAttribArray(args["aNormal"].att);
		gl.vertexAttribPointer(args["aNormal"].att, 3,gl.FLOAT , false, 0, 0);
		gl.bufferData(gl.ARRAY_BUFFER, normalbuffer, gl.STATIC_DRAW);
...
var createBuffer2=ret.createBuffer2=function(program,attName,size,type){
	var obj={};
...
ret.flush=function(ono3d){
	...
	stereofunc(drawMain,ono3d);
	stereofunc(drawLine,ono3d);
	gl.bindFramebuffer(gl.FRAMEBUFFER, frameBuffer);

	gl.clearColor(0.0,0.0,0.0 ,1.0);
	gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
	stereofunc(drawEmi,ono3d);
	//drawPoint(ono3d);
...
void main(void){ \
	gl_FragColor= vec4(gl_FragCoord.z,gl_FragCoord.z,gl_FragCoord.z,1.0); \
	if(abs(gl_FragCoord.x-512.)>410. || abs(gl_FragCoord.y-512.)>410.){ \
...
"
	...
	ret.render=function(){
		...
		gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, args["idx"]);
		gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, idxbuffer, gl.STATIC_DRAW);
		gl.drawElements(gl.TRIANGLES, faceindex, gl.UNSIGNED_SHORT, 0);
	}
	return ret;
