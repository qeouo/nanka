...
var Collider = (function(){
	...
	var Collision=(function(){
		...
		ret.prototype.update=function(){
			//衝突判定前処理
			//Mat43.fromLSR(this.matrix,this.location,this.scale,this.rotq);
	...
	ret.prototype.createConvexHull= function(mesh){
		//内包作成
		var res=new ConvexHull();
		res.mesh = mesh;
		res.id=this.collisionIndexList.pop();
		this.collisions.push(res)
		for(var i=0;i<3;i++){
	...
	ret.prototype.createMesh = function(){
		//メッシュオブジェクト作成
	...
	ret.Mesh = (function(){
		...
		ret.prototype.calcAABB=function(){
			...
			for(var i=0;i<faces.length;i++){
				var face = faces[i];
				AABB.createFromPolygon(faceAABBs[i],poses[face[0]],poses[face[1]],poses[face[2]]);
	...
	var MESH_ANY=function(ans1,ans2,col1,col2){
		...
		for(var i=0;i<faces.length;i++){
			...
			Vec3.copy(triangle.poses[0],poses[face[0]]);
			Vec3.copy(triangle.poses[1],poses[face[1]]);
			Vec3.copy(triangle.poses[2],poses[face[2]]);
	...
	ret.MESH_LINE2 = function(p0,p1,obj) {
		...
		for(var i=0;i<faces.length;i++){
			...
			var l=this.TRIANGLE_LINE(p0,p1,poses[face[0]]
				,poses[face[1]],poses[face[2]]);
			if(l>min && l<0){
				min=l;
			}
		}
