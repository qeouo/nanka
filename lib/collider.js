		...
		ret.prototype.ray= function(p0,p1){
			...
			while(1){
				...
				if(idx===0){
					...
					min=Vec3.dot(axis,v[0]);
					min=Math.min(min,Vec3.dot(axis,v[1]));
					next=2;
					idx++;
				}else if(idx===2){
					...
					if(ret===0){
						...
						min=Vec3.dot(axis,v[0]);
						min=Math.min(min,Vec3.dot(axis,v[1]));
						min=Math.min(min,Vec3.dot(axis,v[2]));
						next=3;
						idx++;
						flg=true;
					}else{
					...
					min=Math.min(min,Vec3.dot(axis,v[1]));
					min=Math.min(min,Vec3.dot(axis,v[2]));
					min=Math.min(min,Vec3.dot(axis,v[3]));
							break;
						}
					}
					if(flga){
	...
	return function(ans1,ans2,obj1,obj2){
		...
		while(1){
			...
			|| faceIndex>=200){
				...
				var V2=[v2[face.v[0]],v2[face.v[1]],v2[face.v[2]]];
