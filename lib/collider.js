...
//ここから基本図形-----------------------------------------------------
	...
	var ConvexHull = ret.ConvexHull= (function(){
		...
		ret.prototype.rayCast= function(p0,p1,normal){
			...
			Vec3.sub(ang,p1,p0);
			var l =Collider.convexCast(ang,sphere,this,normal);
			Vec3.poolFree(1);
...
//----------------------------------------------------------------------------------------
	...
	ret.prototype.rayCastAll = function(p,v){
		...
		var ans1 = Vec3.poolAlloc();
		for(var j=0;j<AABBSort.length;j++){
			var a=AABBSort[j].rayCast(p,v,ans1);
			if(a !== INVALID){
				var elem = this.hitList[this.hitListIndex];
				Vec3.copy(elem.pos1,ans1);
