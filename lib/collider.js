...
var Collider = (function(){
	...
	var ConvexHull = ret.ConvexHull= (function(){
		...
		ret.prototype.ray= function(p0,p1){
			...
			while(1){
				...
				if(Vec3.dot(v[idx],axis)>min-Math.abs(min*0.001)){
					break;
				}
	...
	var calcClosestPrimitive = (function(){
		var TRIANGLE_TRIANGLE__=function(ans1,t1,t2,t3,t4,t5,t6){
			var cross = Vec3.poolAlloc();
			var cross2 = Vec3.poolAlloc();

			var ts=[t1,t2,t3,t1];
			var t2s=[t4,t5,t6,t4];
			Vec3.cross2(cross,t2s[0],t2s[1],t2s[2]);
			for(var j=0;j<3;j++){
				var p = ts[j];
				for(var i=0;i<3;i++){
					var v1 = t2s[i];
			...
			for(var j=0;j<3;j++){
				...
				if(i===3){
					Vec3.sub(cross2,p,t2s[0]);
					Vec3.muladd(ans1,p,cross,-Vec3.dot(cross,cross2)/Vec3.dot(cross,cross));
					Vec3.poolFree(2);
					return;
				}
			}
			...
			for(var j=0;j<3;j++){
				...
				for(var i=0;i<3;i++){
					...
					Vec3.cross(cross,a1,a2,b1);
					Vec3.cross(cross,cross,cross2);
					Vec3.sub(cross2,b2,b1);
					var l = Vec3.dot(cross,cross2)
					if(l<0){
						Vec3.sub(cross2,b1,a1);
						Vec3.muladd(ans1,b1,cross,-Vec3.dot(cross,cross2)/l);
						Vec3.poolFree(2);
						return;
					}
				}
	...
	return function(ans1,ans2,obj1,obj2){
		...
		Vec3.mul(axis,axis,-1);
		obj2.calcSupport(s1,axis);
		Vec3.sub(v[0],v1[0],s1);
		Vec3.copy(axis,v[0]);
		...
		while(1){
			...
			if(Vec3.dot(s,axis)>min-Math.abs(min*0.0001)){
				//取得した点が最短と同じなら外判定
				break;
			}
	...
	//				Vec3.norm(vbuf);
					...
					var ii=(i+1)&3;
					var min = Vec3.dot(vbuf,v[(i+1)&3]);
					var max = min;
					for(var j=1;j<3;j++){
						var t=v[(i+j+1)&3];
						var m = Vec3.dot(vbuf,t);
						if(m>max){
							max=m;
							ii=(i+j+1)&3;
						}
					...
					i=ii;
					if(Vec3.dot(vbuf,t4)>min-Math.abs(min*0.0001)){
						break;
					}
				}
				var next=i;
