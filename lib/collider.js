...
var Collider = (function(){
	...
	var Collision=(function(){
		...
		ret.prototype.calcPre=function(){
			...
			var sx=this.size[0]*this.scale[0];
			var sy=this.size[1]*this.scale[1];
			var sz=this.size[2]*this.scale[2];
			...
			Mat44.getInv(this.inv_matrix,this.matrix);
	...
	var Cylinder= ret.Cylinder= (function(){
		...
		ret.prototype.calcSupport=function(ans,v){
			...
			var l = v[0]*m[8]+v[1]*m[9]+v[2]*m[10];
			if(l>0){
				ans[2]=-1;
			}else{
				ans[2]=1;
			}
			var r = Math.max(this.size[0],this.size[1]);
			var x = v[0]*m[0]+v[1]*m[1]+v[2]*m[2];
			var y = v[0]*m[4]+v[1]*m[5]+v[2]*m[6];
			l = Math.sqrt(x*x+y*y);
			if(l!==0){
				l=-1/l;
			}
	...
	var Capsule = ret.Capsule = (function(){
		...
		ret.prototype.calcPre =function(){
			...
			this.bold = Math.max(this.size[0],this.size[1]);
			var sx=this.size[0]*this.scale[0];
			var sy=this.size[1]*this.scale[1];
			var sz=Math.max(this.size[2]-this.bold,0)*this.scale[2];
	...
	var calcClosest = (function(){
		...
		//	Vec3.norm(n);
		//	Vec3.muladd(ans1,ans1,n,col1.bold);
		//	Vec3.muladd(ans2,ans2,n,-col2.bold);
			return l;
		}
	})();
...
//				if(Math.abs(Vec3.dot(vbuf,axis))<0.0000000000001){
	...
	var MESH_ANY=function(ans1,ans2,col1,col2){
		...
		if(col2.AABB.min[0]===col2.AABB.max[0]){
			for(var i=0;i<faces.length;i++){
				var face = faces[i];
	...
	setHantei(CAPSULE, SPHERE, function(ans1,ans2,col1,col2){
		var m = col1.matrix;
		Vec3.set(bV0,m[12]+m[8],m[13]+m[9],m[14]+m[10]);
		Vec3.set(bV1,m[12]-m[8],m[13]-m[9],m[14]-m[10]);
		Vec3.set(ans2,col2.matrix[12],col2.matrix[13],col2.matrix[14]);
	...
	ret.SPHERE_LINE = (function(){
		var p=new Vec3();
		return function(p0,p1,obj) {
			Vec3.set(p,obj.matrix[12],obj.matrix[13],obj.matrix[14]);
			return this.SPHERE_LINE2(p0,p1,p,obj.bold);
			}
	})();
	...
	ret.SPHERE_LINE2 = function(p0,p1,p2,r) {
		...
		Vec3.sub(p,p0,p2);
		Vec3.sub(d,p1,p0);
		var A = Vec3.scalar2(d);
		var B = Vec3.dot(p,d);
		var C = Vec3.scalar2(p) - r*r;
		if(A===0){
			return 9999;
		}
		var l = B*B-A*C;
	...
	ret.CYLINDER_LINE = function(p0,p1,obj) {
		//シリンダと直線
		var min=-99999;
		var max=99999;
		...
		var d = bV0;
		var p = bV1;
		Mat44.dotVec3(p,obj.inv_matrix,p0);
		Mat44.dotVec3(d,obj.inv_matrix,p1);
		Vec3.sub(d,d,p);
	...
	ret.CAPSULE_LINE = function(p0,p1,obj) {
		...
		min=this.CYLINDER_LINE(p0,p1,obj);
		var m = obj.matrix;
		Vec3.set(p,m[8]+m[12],m[9]+m[13],m[10]+m[14]);
		min=Math.min(min,this.SPHERE_LINE2(p0,p1,p,obj.bold));
		Vec3.set(p,-m[8]+m[12],-m[9]+m[13],-m[10]+m[14]);
		min=Math.min(min,this.SPHERE_LINE2(p0,p1,p,obj.bold));


		//min=min(min,SPHERE_LINE
	...
	ret.MESH_LINE = function(p0,p1,obj) {
		var t = bV0;
		var dpos = bV1;
		var cross=bV3;
		var pos2= bV4;
	...
	return function(p0,p1,obj){
		...
		while(1){
			...
			if(idx===1){
				...
				this.TRIANGLE_LINE(Vec3.ZERO,t,v[0],v[1],v[2],ref);
				if(ref.out===0){
					Vec3.sub(vbuf,v[1],v[0]);
					Vec3.sub(axis,v[2],v[0]);
					Vec3.cross(axis,vbuf,axis);
				...
				for(var i=0;i<3;i++){
					...
					var t2=v[(i+1)%3];
					var t3=v[3];
					this.TRIANGLE_LINE(Vec3.ZERO,t,t1,t2,t3,ref);
					if(ref.out===0){
						Vec3.sub(axis,t2,t1);
						Vec3.sub(vbuf,t3,t1);
	...
	ret.collisionLine = function(p0,p1,collision){
		...
		case SPHERE:
			...
			break;
		case CYLINDER:
			return Collider.CYLINDER_LINE(p0,p1,collision);
			break;
		case CAPSULE:
			return Collider.CAPSULE_LINE(p0,p1,collision);
			break;
		case CONVEX:
			return Collider.CONVEX_LINE(p0,p1,collision);
			break;
		case MESH:
			return Collider.MESH_LINE(p0,p1,collision);
			break;
		}
		return -1;
	}
