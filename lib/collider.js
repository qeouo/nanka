...
var Collider = (function(){
	...
	var calcClosestPrimitive = (function(){
		...
		var _addFace = function(v1,v2,v3,obj1,obj2){
			...
			face.len2 = Math.max(face.len,Vec3.dot(face.cross,vs[v2]));
			face.len2 = Math.max(face.len,Vec3.dot(face.cross,vs[v3]));
		...
		var _addEdge = function(v1,v2){
			...
			//無効インデックスを探す
			var idx = _edges.findIndex(function(e){return e[0]<0;});
			if(idx>=_edgesIndex){
				//エッジ増えたらインデックスを増やす
				idx=_edgesIndex;
				_edgesIndex++;
				_edges[_edgesIndex][0]=-1;
			}
			var edge=_edges[idx];
			edge[0]=v1;
			edge[1]=v2;
	...
	return function(ans1,ans2,obj1,obj2){
		...
		while(1){
			...
			//終了チェック
			if((Vec3.dot(s,axis) - face.len2<0.00001)
			|| faceIndex>=200){
