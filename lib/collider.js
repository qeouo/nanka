...
//				if(Math.abs(Vec3.dot(vbuf,axis))<0.0000000000001){
	...
	ret.CAPSULE_LINE = function(p0,p1,obj) {
		...
		d[0]=(m[12]-p0[0]);
		d[1]=(m[13]-p0[1]);
		d[2]=(m[14]-p0[2]); //線分から円柱中心までの距離

		Vec3.cross(n,r,t); //2線に垂直なベクトル
		Vec3.norm(n);

		var y = Vec3.dot(n,d); //円柱中心と線分の距離
		if(y*y > obj.bold*obj.bold){
			//半径より大きい場合は外れている
			return 99999;
		}
		var x=Math.sqrt(obj.bold*obj.bold-y*y); //線分を含む平面中のシリンダ幅?

		Vec3.cross(n2,n,r); //円柱に対して垂直なベクトル
		Vec3.norm(n2);

		var len=Vec3.dot(d,n2);
		if(len>0){
			x*=-1;
		}
		len = (Vec3.dot(d,n2)-x)/Vec3.dot(t,n2); //シリンダと線分の交点距離
		Vec3.mul(d,d,-1);
		Vec3.muladd(d,d,t,len); //交点
		var ra = Vec3.dot(r,d);
		if(ra>sc){ //交点がシリンダ部分の外側の場合
			d[0]=m[12]+r[0]*sc;
			d[1]=m[13]+r[1]*sc;
			d[2]=m[14]+r[2]*sc;
