...
var Collider = (function(){
	...
	var Collision=(function(){
		...
		ret.prototype.calcSupportB=function(ret,axis){
		...
		ret.prototype.rayCast=function(p0,p1,normal){
			if(normal){
...
//ここから基本図形-----------------------------------------------------
	...
	var Sphere = ret.Sphere = (function(){
		...
		ret.prototype.rayCast = function(p0,p1,normal) {
			...
			var l =SPHERE_LINE2(p0,p1,p,this.bold,normal);
			Vec3.poolFree(1);
			return l;
		}
		return ret;
	...
	var Cylinder= ret.Cylinder= (function(){
		...
		ret.prototype.rayCast= function(p0,p1,normal) {
			...
			if(normal){
				...
				Mat33.dot(normal,this.matrix,normal);
				Vec3.norm(normal);
	...
	var Cone= ret.Cone= (function(){
		...
		ret.prototype.rayCast= function(p0,p1) {
			//コーンと直線
			var min=-99999;
			var max=99999;
			var flg =false;
	...
	var Capsule = ret.Capsule = (function(){
		...
		ret.prototype.rayCast = function(p0,p1,normal) {
			...
			min=Cylinder.prototype.rayCast.call(this,p0,p1,normal);
			var m = this.matrix;
			Vec3.set(p,m[3]+m[9],m[4]+m[10],m[5]+m[11]);
			var l = SPHERE_LINE2(p0,p1,p,this.bold,n2);
			if(l<min){
				min=l;
				if(normal){
					Vec3.copy(normal,n2);
			...
			Vec3.set(p,-m[3]+m[9],-m[4]+m[10],-m[5]+m[11]);
			l=SPHERE_LINE2(p0,p1,p,this.bold,n2);
			if(l<min){
				min=l;
				if(normal){
					Vec3.copy(normal,n2);
	...
	var ConvexHull = ret.ConvexHull= (function(){
		...
		ret.prototype.rayCast= function(p0,p1){
			sphere.bold=0;
			Mat43.set(sphere.matrix,1,0,0,0,1,0,0,0,1,p0[0],p0[1],p0[2]);
			sphere.refresh();
			var ang=Vec3.poolAlloc();
			Vec3.sub(ang,p1,p0);
			var l =Collider.convexCast(ang,sphere,this);
			Vec3.poolFree(1);
...
//----------------------------------------------------------------------------------------
	...
	ret.prototype.convexCastAll = function(v,col){
		var AABBSort = this.aabbSorts[0];
		this.hitListIndex = 0;
	...
	var SPHERE_LINE2 = function(p0,p1,p2,r,normal) {
		var p = Vec3.poolAlloc();
		var d = Vec3.poolAlloc();
		Vec3.sub(p,p0,p2);
		Vec3.sub(d,p1,p0);
	...
	ret.convexCast= function(t,o2,o1,ans1,ans2){
		var axis=Vec3.poolAlloc();
