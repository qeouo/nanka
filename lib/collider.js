	...
	var calcClosestPrimitive = (function(){
		...
		var TRIANGLE_TRIANGLE__=function(ans1,ans2,axis,A1,A2,A3,B1,B2,B3){
			...
			if(idxa===1 && idxb===1){
				//どっちも線分の場合
				Vec3.sub(cross2,A2,A1);
				Vec3.madd(ans1,A1,cross2,checkLINE_LINE(A1,A2,B1,B2,axis));
				Vec3.sub(ans2,ans1,axis);
				Vec3.poolFree(2);
				return;
			}
			...
			if(idxb===3){
				...
				Vec3.cross(cross2,cross,axis);
				Vec3.norm(cross);
				if(Vec3.dot(cross,cross2)>0.1){
					console.log("OKASII");
				}
	...
	return function(ans1,ans2,obj1,obj2){
		...
		while(1){
			...
			if(idx===1){
				...
				Vec3.cross(axis,v[0],v[1]);
				Vec3.cross(axis,axis,v[0]);
