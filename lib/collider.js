...
var Collider = (function(){
		...
		ret.prototype.ray= function(p0,p1){
			...
			while(1){
				...
				if(idx===1){
					...
					var ret=TRIANGLE_LINE(Vec3.ZERO,t,v[0],v[1],v[2]);
					if(!ret){
					...
					for(var i=0;i<3;i++){
						...
						var t2=v[(i+1)%3];
						var t3=v[3];
						if(!TRIANGLE_LINE(Vec3.ZERO,t,t1,t2,t3)){
							Vec3.sub(axis,t2,t1);
							Vec3.sub(vbuf,t3,t1);
		...
		while(1){
			...
			if(Vec3.dot(s,axis)+min<0.00001){
				...
				if(idx===1){
					...
					//頂点が3個の場合は良く分からんので三角形同士で判定する
					//頂点が3個の場合は点x三角形か点x辺か辺x辺のどれか
	...
	var TRIANGLE_LINE=function(p0,p1,t0,t1,t2){
		var t = Vec3.poolAlloc();
		var cross=Vec3.poolAlloc();
		var T0=Vec3.poolAlloc();
		var T1=Vec3.poolAlloc();
		var T2=Vec3.poolAlloc();
		...
		for(var j=0;j<3;j++){
			...
			var v2 = vs[j+1];
			var v3 = vs[j+2];
			Vec3.sub(cross,v2,v1);
			Vec3.cross(cross,t,cross);
			
			if(Vec3.dot(cross,v1)*Vec3.dot(cross,v3)>0){
				//辺の外の場合はずれ
				ret=j+1;
				break;
			}
