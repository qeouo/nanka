...
var Collider = (function(){
		...
		while(1){
			...
			if(Vec3.dot(s,axis)+min<0.00001){
				...
				if(idx===1){
					...
					//頂点が2個の場合はどっちかが辺でどっちか点の組み合わせ
					if(Vec3.len2(v1[0],v1[1])<Vec3.len2(v2[0],v2[1])){
						Vec3.copy(ans1,v1[0]);
						Vec3.sub(ans2,axis,ans1);
						//Geono.LINE_POINT(ans2,v2[0],v2[1],v1[0]);
					}else{
						Vec3.copy(ans2,v2[0]);
						Vec3.add(ans1,ans1,axis);
						//Geono.LINE_POINT(ans1,v1[0],v1[1],v2[0]);
					}
				}else{
					if(idx===4){
					...
					if(Vec3.len2(v1[0],v1[1])<0.00001 &&  Vec3.len2(v1[0],v1[2])<0.00001){
						...
						if(Vec3.len2(v1[0],v1[1])<0.00001){
							Vec3.copy(v1[1],v1[2]);
						}
						if(Vec3.len2(v2[0],v2[1])<0.00001){
							Vec3.copy(v2[1],v2[2]);
						}
	...
	var TRIANGLE_LINE=function(p1,t0,t1,t2){
		...
		for(var j=0;j<3;j++){
			...
			var l=Vec3.dot(cross,vs[j]); //線から辺までの垂直距離
			Vec3.sub(dt,vs[j+2],vs[j+1]);
			var l2=Vec3.dot(cross,dt); //三角形の向き?
