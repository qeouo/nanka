...
var Bsp= (function(){
	...
	ret.hitCheck = function(p,bsp){
		return bsp.m <=Vec3.dot(p,bsp.v);
	}
	...
	ret.createBspTree = function(list){
		
		return _createBspTree(list,0,list.length-1);

	}
	var _createBspTree = function(list,first,last){
		if(first === last){
			return list[first];
		}
		var axis = new Vec3();
		for(var i=first;i<=last){
			var node = list[i];
			var l = Vec3.dot(node.v[0],axis));
			l = MIN(l,Vec3.dot(node.v[1],axis));
			l = MIN(l,Vec3.dot(node.v[2],axis));
			l = MIN(l,Vec3.dot(node.v[3],axis));
			node.m = l;
		}
		Sort.qSort(list,first,last,function(a,b){return a.m - b.m});
