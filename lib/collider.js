...
var Collider = (function(){
	...
	var ConvexHull = ret.ConvexHull= (function(){
		...
		ret.prototype.ray= function(p0,p1){
			...
			while(1){
				...
				if(-Vec3.dot(s,axis)+min<0.00001){
					//その時点が最短
					if(idx>2){
				...
				if(idx===0){
					...
					if(ret===0){
						...
						Vec3.sub(vbuf,v[ret%3],v[(ret+2)%3]);
						Vec3.cross(axis,vbuf,t);
						if(Vec3.dot(v[(ret+1)%3],axis)<0){
							Vec3.mul(axis,axis,-1);
						}
						Vec3.copy(v[(ret+1)%3],v[2]);
					...
					for(var i=0;i<3;i++){
						...
						var t2=v[(i+1)%3];
						var t3=v[3];
						if(!TRIANGLE_LINE(t,t1,t2,t3)){
							Vec3.cross2(axis,t1,t2,t3);
					...
					if(flga){
						Vec3.copy(axis,t);
						var max=Vec3.dot(axis,v[0]);
						var next=0;
						for(var i=1;i<oldidx+1;i++){
							var m=Vec3.dot(axis,v[i]);
							if(m>max){
								next=i;
								max=m;
							}
						}	
						Vec3.copy(v[next],v[3]);
	...
	var calcClosestPrimitive = (function(){
		...
		var triangle_zero_axis=function(ans,t1,t2,t3){
			var dir = Vec3.poolAlloc(); 
			var cross = Vec3.poolAlloc();
			var cross2 = Vec3.poolAlloc();
			...
			//面の上にいる場合
			Vec3.mul(ans,cross ,Vec3.dot(cross,v1));
			Vec3.poolFree(3);
			return 0;
		...
		var triangle_zero=function(ans,t1,t2,t3){
			...
			//面の上にいる場合
			Vec3.mul(ans,cross
				,Vec3.dot(cross,v1)/(cross[0]*cross[0]+cross[1]*cross[1]+cross[2]*cross[2]));
			Vec3.poolFree(3);
			return 0;
		...
		var _addFace = function(v1,v2,v3,obj1,obj2){
			...
			//triangle_zero(addFaceBuf,vs[v1],vs[v2],vs[v3]);
			//face.len = Vec3.dot(fac,addFaceBuf);
...
//		Vec3.mul(axis,axis,0.5);
		...
		//初期頂点
		var axises=[[0,1,0],[-1,-1,1],[1,1,1],[0,-1,-1]];
		for(var i=0;i<4;i++){
			obj1.calcSupport(v1[i],axises[i]);
			Vec3.mul(axis,axises[i],-1);
			obj2.calcSupport(v[i],axis);
			Vec3.sub(v[i],v1[i],v[i]);
		}
		...
		while(1){
			...
			for(var i=0;i<4;i++){
				...
				if( l1*l2>0){
					//四面体に内包されない
					triangle_zero_axis(axis,t1,t2,t3);
					var max = Vec3.dot(axis,t4);
					var next=i;
					for(var j=0;j<3;j++){
						var t=v[(i+j+1)&3];
						var m = Vec3.dot(axis,t);
						if(m>max){
							max=m;
							next=(i+j+1)&3;
						}
					...
					if(next!==i){
						triangle_zero_axis(axis,v[0],v[1],v[2]);
					}
					break;
				}
			}
			...
			//axisの向きで一番近い点をとる
			obj1.calcSupport(v1[3],axis);
			Vec3.mul(axis,axis,-1);
			obj2.calcSupport(v[3],axis);
			Vec3.sub(v[3],v1[3],v[3]);
			Vec3.mul(axis,axis,-1);
			...
			//取得した点が最短と同じかチェック
			if(-Vec3.dot(v[3],axis)+min<0.00001){
				//同じ場合はその時点での点が最短
				...
				if(!ans1 || !ans2){
					...
					Vec3.mul(axis,axis,min/Vec3.scalar2(axis));

					if(Vec3.len2(v1[0],v1[1])<0.00001 &&  Vec3.len2(v1[0],v1[2])<0.00001){
						Vec3.copy(ans1,v1[0]);
						Vec3.sub(ans2,axis,ans1);
					}else if(Vec3.len2(v2[0],v2[1])<0.00001 && Vec3.len2(v2[0],v2[2])<0.00001){
						Vec3.copy(ans2,v2[0]);
						Vec3.add(ans1,ans1,axis);
					}else{
						if(Vec3.len2(v1[0],v1[1])<0.0001){
							Vec3.copy(v1[0],v1[2]);
						}
						if(Vec3.len2(v2[0],v2[1])<0.0001){
							Vec3.copy(v2[0],v2[2]);
						}
						Vec3.sub(vbuf,v2[1],v2[0]);
						Vec3.cross(vbuf,vbuf,axis);

						Vec3.sub(ans1,v1[1],v1[0]);
						Vec3.sub(ans2,v2[0],v1[0]);

						Vec3.madd(ans1,v1[0],ans1,Vec3.dot(vbuf,ans2)/Vec3.dot(vbuf,ans1));
						Vec3.sub(ans2,axis,ans1);

					}
					
				}
				var l=Vec3.scalar(axis);
				Vec3.poolFree(4);
				return l;
			}
		...
		for(var i=0;i<faceIndex;i++){
			...
			var res = triangleCheck(v[f.v[0]],v[f.v[1]],v[f.v[2]]);
			if(res !==0){
				continue;
			}
			if(min>f.len){
		...
		triangle_zero(axis,v[face.v[0]],v[face.v[1]],v[face.v[2]]);
