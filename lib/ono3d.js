	...
	ret.prototype.renderShader=function(start,size,opFlg,customMaterial){
		...
		while(i<start+size){
			...
			if(i==start || arr[i].environment != arr[Math.max(i-1,0)].environment){
				...
				var environment2 = arr[i].environment;
				...
				gl.uniform3fv(unis["uLightColor2"],new Float32Array(environment2.sun.color));

				Mat44.copy(fa16,environment.sun.viewmatrix);

				gl.uniform3fv(unis["uAmbColor"],new Float32Array(environment.area.color));
				gl.uniform3fv(unis["uAmbColor2"],new Float32Array(environment2.area.color));
				...
				if(environment2.envTexture){
					gl.bindTexture(gl.TEXTURE_2D,environment2.envTexture);
				}else{
					gl.bindTexture(gl.TEXTURE_2D,Rastgl.dummyTexture); //テクスチャ未指定の場合はダミーを設定
				}
				gl.uniform1i(unis["uEnvMap2"],6);
				gl.uniform1f(unis["uEnvRatio"],0);
