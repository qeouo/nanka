...
var Ono3d = (function(){
	...
	ret.prototype= {
		...
		,init:function(_canvas,_ctx){
			...
			this.verticesFloat32Array= new Float32Array(4096*3*20);//this.verticesArrayBuffer);
			this.indexBuffer = new Uint16Array(4096);//this.verticesArrayBuffer);
			gl = Rastgl.gl;
		...
		,getPersMatrix:function(mat,x,y,zn,zf){
			Mat44.set(mat
				,1/this.persx,0,0,0
				,0,1/this.persy,0,0
				,0,0,-(zn+zf)/(zf-zn),-1.0
		...
		,setPers:function(x,y,zn,zf){
			...
			this.getPersMatrix(this.projectionMatrix
				,this.x,this.persy
				,this.znear,this.zfar);
		}
		,getOrthoMatrix:function(mat,x,y,zn,zf){
			Mat44.set(mat
			,-1.0/x,0,0,0
			,0,1.0/y,0,0.0
			,0,0,2.0/(zf-zn),0.0
		...
		,setOrtho:function(x,y,zn,zf){
			...
			this.getOrthoMatrix(this.projectionMatrix
				,x,y
				,this.znear,this.zfar);
		}
		,push:function(){
	...
	ret.prototype.setStatic=function(){
		this.faces_static_index=this.faces_index;
...
//				buff[normalindex+vindex]=vertex.normal[0] ;
//				buff[normalindex+vindex+1]=vertex.normal[1] ;
//				buff[normalindex+vindex+2]=vertex.normal[2] ;
//				buff[uvindex+vindex]=vertex.uv[0];
//				buff[uvindex+vindex+1]=vertex.uv[1];
//
...
//
//				buff[uv2_index+vindex]=vertex.uv2[0]
//				buff[uv2_index+vindex+1]=vertex.uv2[1]
//
//				//ライトプローブ色
//				buff[17+vindex]=vertex.color[0];
//				buff[17+vindex+1]=vertex.color[1];
//				buff[17+vindex+2]=vertex.color[2];
//

				vindex+=vertexDataSize;

				idxbuff[idxsize]=idxsize;
				idxsize++;
			}
		...
		gl.bufferSubData(gl.ARRAY_BUFFER, voff*4
			, new Float32Array(this.verticesFloat32Array.buffer,voff*4,vindex-voff));
