...
var Ono3d = (function(){
	...
	ret.loadShader= function(vs,fs){
		...
		for(var i=0;i<atts.length;i++){
			shader.atts[atts[i]]=gl.getAttribLocation(program,atts[i]); 
		}
		shader.unis={};
	...
	ret.prototype.renderShader=function(shader,start,size,opFlg){
		...
		var gl = Rastgl.gl;
		gl.vertexAttribPointer(atts["aPos"], 3,gl.FLOAT, false, 14*4, 0);
		gl.vertexAttribPointer(atts["aNormal"], 3,gl.FLOAT, false, 14*4, 3*4);
		gl.vertexAttribPointer(atts["aSvec"], 3,gl.FLOAT, false, 14*4, 6*4);
		gl.vertexAttribPointer(atts["aTvec"], 3,gl.FLOAT, false, 14*4, 9*4);
		gl.vertexAttribPointer(atts["aUv"], 2,gl.FLOAT, false, 14*4, 12*4);
		...
		while(i<start+size){
			...
			gl.uniform4f(unis["uPbr"],material.spc,material.rough,material.transRough,(1.0/material.ior-1.0)*0.2);
