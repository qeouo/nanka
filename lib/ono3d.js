...
var Ono3d = (function(){
	...
	ret.prototype.stereoDraw = function(func){
		...
		if(globalParam.stereomode==0){
			...
			this.calcProjectionMatrix(this.projectionMatrix,this.aov,this.aov*HEIGHT/WIDTH
				,this.znear,this.zfar);
...
//シャドウマップ描画
		...
		this.setViewport(0,0,1024,2048);
		...
		if(lightSource && facesize){
			...
			gl.uniformMatrix4fv(shader.unis["projectionMatrix2"],false,lightSource.viewmatrix2);
			Mat44.getInv(f32a,lightSource.viewmatrix);
			gl.uniformMatrix4fv(shader.unis["projectionMatrix3"],false,f32a);
	...
	ret.prototype.renderShader=function(start,size,customMaterial,envIdx){
		...
		while(i<start+size){
			...
			//	Mat44.copy(fa16,environment.sun.viewmatrix);
				...
				gl.uniformMatrix4fv(unis["lightMat2"],false,lightSource.viewmatrix2);
				Mat44.getInv(fa16,lightSource.viewmatrix);
				gl.uniformMatrix4fv(unis["lightMat3"],false,fa16);
