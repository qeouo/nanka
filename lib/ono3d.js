	...
	ret.prototype.render = function(shadowTex,camerap,customMaterial){
		...
		for(var i=0;i<arrlength;i++){
			...
			if(oldshader != material.shader){
				renderlist_shader.push(materialIndexes.length-1);
		...
		if(customMaterial){
			...
			for(var i=0;i<renderlist_shader.length-1;i++){
				this.renderShader(materialIndexes[renderlist_shader[i]]
					,materialIndexes[renderlist_shader[i+1]]-materialIndexes[renderlist_shader[i]]
					,false,null,0);
			}
			...
			if(idx>=0){
				...
				for(var i=idx;i<materialIndexes.length-1;i++){
					this.renderShader(materialIndexes[i]
						,materialIndexes[i+1]-materialIndexes[renderlist_shader[i]]
						,false,null,0);
				}
