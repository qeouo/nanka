...
var Ono3d = (function(){
	...
	var ret = function(){
		this.rf = 0
		this.smoothing = 0
		this.viewMatrix=new Mat43()
		this.worldMatrix=new Mat43()
		this.transMat = this.worldMatrix
		this.projectionMat=new Array(16);//new Mat44()
		this.persx
		this.persy
		...
		this.stackIndex=0
		for(var i=32;i--;)this.stackTransMatrix.push(new Mat43())
	...
	ret.prototype= {
		...
		,getPos:function(pos,x,y,z){
			...
			pos[1]=y
			pos[2]=z
			Mat43.dotMat43Vec3(pos,this.transMat,pos)
...
//					target.fillRect(vertices[0].screenx-(b>>1)-0.5,vertices[0].screeny-(b>>1)-0.5,b,b)
		...
		,setTargetMatrix:function(target){
			if(target===0){
				...
				this.transMat = this.viewMatrix
			}
		}

		,setPers:function(x,y){
			
			if(x>0)this.persx = x;
			if(y>0)this.persy = this.persx*y;
			Mat44.set(this.projectionMat
				,-1.0/this.persx,0,0,0
			,0,1.0/this.persy,0,0
			,0,0,80/(80.0-1.),80/(80.0-1.)
			,0,0,-1.0,0);
		}
		...
		,rotate:function(r,x,y,z){
			...
			Mat43.dot(this.transMat,this.transMat,a)
		}
		...
		,transmat:function(m){
			Mat43.dot(this.transMat,this.transMat,m);
		}
	...
	ret.loadTexture = function(path){
		return Rastgl.loadTexture(path);
	}
