...
var Ono3d = (function(){
	...
	ret.prototype= {
		...
		,init:function(_canvas,_ctx){
			...
			this.verticesFloat32Array= new Float32Array(4096*3*20);
			this.indexBuffer = new Uint16Array(4096);

			this.shaders=[];
			shaders = this.shaders;

			var filenames=["main"
				,"static","shadow","plain","half","multiadd","fill","add"
...
//シャドウマップ描画
		...
		for(var i=1;i<4;i++){
			...
			Ono3d.postEffect(transTexture,0,1-size/bunbo*2,size/bunbo*2,size/bunbo,shaders["half"]);
		...
		gl.viewport(0,0,TEXSIZE*0.5,TEXSIZE*0.5);

		Ono3d.gauss(512,512,100
			,transTexture,0,0,1.0,1.0,1024,1024); 

		Ono3d.copyImage(transTexture,0,512,0,0,512,512);
...
ret.calcST = function(s,t,p0,p1,p2,u0,v0,u1,v1,u2,v2){
	...
	/** ガウスぼかし **/
	ret.gauss=function(width,height,d,src,x,y,w,h,srcWidth,srcHeight){
		//係数作成
	...
	ret.prototype.bloom = function(image,exposure_bloom){
		...
		var transTexture= this.env2Texture;
...
//		gl.clear(gl.COLOR_BUFFER_BIT);
		...
		//レンダリング画像をコピー(半分サイズ)
		this.setViewport(0,0,WIDTH*0.5,HEIGHT*0.5);
		...
		//ガウスぼかし
		this.setViewport(0,0,256,512);
		Ono3d.gauss(256,512,100
			,transTexture,0,0,0.25,0.5,1024,1024); 
		Ono3d.copyImage(transTexture,0,0,0,0,256,512);

		//全サイズ足す
		this.setViewport(0,0,512,512);
