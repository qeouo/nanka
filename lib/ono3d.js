...
var Ono3d = (function(){
	...
	ret.prototype.render = function(camerap,customMaterial){
		...
		var idxbuff = Rastgl.getJsIdxBuffer(facesize*3 + linesize*2);
		...
		for(var i=0;i<facesize;i++){
			...
			for(var j=0;j<3;j++){
				...
				var environment = renderface.environments[0];
				if(environment.bspTree){
					nrm[0]=buff[normalindex+vindex];
					nrm[1]=buff[normalindex+vindex+1];
					nrm[2]=buff[normalindex+vindex+2];
					//var angx = Math.abs(Math.asin(nrm[1]))*_PI*2.0;
					var ratioy = Math.abs(Math.asin(nrm[1]))*_PI*2.0;
					var angy = Math.atan2(Math.abs(nrm[0]),Math.abs(nrm[2]))*_PI*2.0;
					...
					var targetp = vertex.pos;
					var hitTriangle = bspTree.getItem(targetp);
					if(hitTriangle){
						//Geono.TETRA_POINT(ratio,targetp,hitTriangle.p);

						var bsps=hitTriangle.bsps;
						var pIdx=hitTriangle.pIdx;
						var oz= 1-Math.sign(nrm[2]);
						var ox= Math.sign(nrm[0])+2;
						var oy= (1+Math.sign(nrm[1])>>1)+4;
						Vec3.set(colx,0,0,0);
						Vec3.set(coly,0,0,0);
						Vec3.set(colz,0,0,0);
						for(var k=0;k<4;k++){
							var colorss=colors[pIdx[k]];
							var ratio=Vec3.dot(bsps[k].v,targetp)-bsps[k].m;
							Vec3.madd(colx,colx,colorss[ox],ratio);
							Vec3.madd(coly,coly,colorss[oy],ratio);
							Vec3.madd(colz,colz,colorss[oz],ratio);
				...
				buff[17+vindex]=color[0];
				buff[17+vindex+1]=color[1];
				buff[17+vindex+2]=color[2];
		...
		gl.bufferSubData(gl.ELEMENT_ARRAY_BUFFER, 0, idxbuff);
...
var calcST = function(s,t,p0,p1,p2,u0,v0,u1,v1,u2,v2){
	...
	ret.prototype.createLightField= function(tex,x,y,z,gridsize,func){
		var size = 32;
		if(!tex){
		...
		gl.bindFramebuffer(gl.FRAMEBUFFER, null);
		//キューブマップ作成
		//this.createCubeMap(envBuf,(x-64)*gridsize,(y-4)*gridsize,(z-64)*gridsize,size,func);
		var a=new Vec3(x,y,z);
		Mat44.dotVec3(a,this.worldMatrix,a);
	
		this.createCubeMap(envBuf,a[0],a[1],a[2],size,func);

		this.setViewport(0,0,size*4,size*2);
		Ono3d.postEffect(envBuf,0,0,size*4/envBuf.width,size*2/envBuf.height,this.shaders["cube2lightfield"]); 
		...
		while(2<texsize){
			//積分
			texsize>>=1;
			this.setViewport(0,0,texsize*4,texsize*2);
			Ono3d.postEffect(envBuf,0,0,1,1,this.shaders["cube2lightfield2"]); 
			Ono3d.copyImage(envBuf,0,0,0,0,texsize*4,texsize*2);

		}

		//積分2
		this.setViewport(0,0,6,1);
		Ono3d.postEffect(envBuf,0,0,1,1,this.shaders["cube2lightfield3"]); 
