...
var Ono3d = (function(){
	...
	var Ono3d = function(){
		...
		for(i=0;i<RENDERFACES_MAX;i++)this.renderEnvironments.push(new RenderEnvironment());
	...
	ret.RenderMaterial = RenderMaterial;
	...
	ret.prototype= {
		...
		,clear: function(){
			...
			this.renderEnvironments_index=0;
		}
	...
	ret.prototype.render = function(shadowTex,camerap,customMaterial){
		...
		for(var i=0;i<this.renderEnvironments_index;i++){
			this.renderEnvironments[i].index = i;
		}
		...
		s=Math.pow(0.5,4);
		var renderEnvironment=this.renderEnvironments[0];
		Env2D.draw(renderEnvironment.envTexture,0,1-s,s,s*0.5);
		gl.bindTexture(gl.TEXTURE_2D,transTexture);
	...
	ret.prototype.renderShader=function(start,size,opFlg,customMaterial){
		...
		while(i<start+size){
			var j=i;
			var env = arr[i].renderEnvironment;

			if(i==0 || arr[i].environment != arr[Math.max(i-1,0)].environment){
				var environment = arr[i].environment;
				var lightSources=environment.lights;
