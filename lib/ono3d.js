...
//シャドウマップ描画
	...
	ret.loadBumpTexture = function(path,func){
		return Ono3d.loadTexture(path,function(image){
			...
			Ono3d.initNormal(image,image.width,image.height);
...
var calcST = function(s,t,p0,p1,p2,u0,v0,u1,v1,u2,v2){
	...
	ret.prototype.rough=function(width,height,rough,src,x,y,w,h){
	...
	ret.initNormal=function(src,x,y){
	...
	ret.prototype.setExpose = function(level,upper){
		var a = new Vec4();
		Vec4.set(level,upper,0.5,0.5);
		gl.bindFramebuffer(gl.FRAMEBUFFER, Rastgl.frameBuffer);
		this.setViewport(0,511,1,1);
		gl.useProgram(shaders["fill"].program);
		Ono3d.encode2(param,param);
	...
	ret.prototype.createEnv = function(x,y,z,func){
		...
		for(var i=0;i<envs.length;i++){
			...
			this.setViewport(0,0,width,height);
			this.rough(width,height,rough,tex.glTexture,0,0,1,0.5);

			gl.bindTexture(gl.TEXTURE_2D,tex2);
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
			Ono3d.copyImage({glTexture:tex2},0,0,0,0,width,height);
			
			this.gauss(width,height,10,{glTexture:tex2},0,0,1,1,width,height); 
