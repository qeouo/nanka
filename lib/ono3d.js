...
var Ono3d = (function(){
	...
	ret.prototype= {
		...
		,init:function(_canvas,_ctx){
			...
			var filenames=["main"
				,"static","shadow","plain","half","fill","add"
		...
		while(i<start+size){
			...
			if(i==start || arr[i].environments[envIdx] != arr[Math.max(i-1,0)].environments[envIdx]){
				...
				if(environment.envTexture){
					...
					if(this.environments[0].envTexture){
						gl.bindTexture(gl.TEXTURE_2D,this.environments[0].envTexture.glTexture);
					}else{
						gl.bindTexture(gl.TEXTURE_2D,Rastgl.dummyTexture); //テクスチャ未指定の場合はダミーを設定
					}
				}
...
ret.calcST = function(s,t,p0,p1,p2,u0,v0,u1,v1,u2,v2){
	...
	ret.initNormal=function(src){
		...
		gl.useProgram(shader.program);
		gl.uniform2f(shader.unis["uUnit"],1.0/256,1.0/256);
	...
	ret.prototype.bloom = function(image,exposure_bloom){
		...
		var transTexture= this.transTexture;
		//
		Ono3d.copyImage(transTexture,0,0,0,0,WIDTH*emiSize,HEIGHT*emiSize);
		// 1/2 1/4 1/8...
		var size=emiTexture.width;
		var bunbo=transTexture.width;
		for(var i=0;i<4;i++){
			this.setViewport(0,0,size*0.5,size*0.5);
		
			Ono3d.postEffect(transTexture,0,(1024-size*2)/bunbo,size/bunbo,size/bunbo,shaders["half"]);
			//Ono3d.drawCopy(0,0,1,1,transTexture,0,(1024-size*2)/bunbo,size/bunbo,size/bunbo);
			size*=0.5;
			Ono3d.copyImage(transTexture,0,1024-size*2,0,0,size,size);
		}
		//ガウスぼかし
		this.setViewport(0,0,512,1024);
		this.gauss(512,1024,100
			,transTexture,0,0,0.5,1.0,1024,1024); 
		Ono3d.copyImage(transTexture,0,0,0,0,512,1024);
		
		//
		this.gauss(WIDTH*emiSize,HEIGHT*emiSize,100
			,emiTexture,0,0,WIDTH*emiSize/emiTexture.width,HEIGHT*emiSize/emiTexture.height,512,512); 
		//結果を光テクスチャに書き込み
		Ono3d.copyImage(emiTexture,0,0,0,0,WIDTH*emiSize,HEIGHT*emiSize);
