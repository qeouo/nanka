...
var Util=(function(){
	...
	ret.loadImage=function(url,norepeat,func,bamp){
		var image = new Image()
		if(func){
			image.onload =  function(e){
				...
				if(globalParam.enableGL){
					if(!bamp){
						...
						var tex=Rastgl.createTexture(image.width,image.height);
						nrm.draw(image.gltexture,1.0/image.width,1.0/image.height);
			...
			image.onload =  function(e){
				...
				if(globalParam.enableGL){
					if(!bamp){
						...
						gl.viewport(0,0,image.width,image.height);
						var tex=Rastgl.createTexture(image.width,image.height);
						var tex2= Rastgl.setTexture(image);
...
ret.hex2rgb=function(rgb,hex){
	...
	ret.init=function(canvas,_inputarea){
