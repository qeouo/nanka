...
var O3o=(function(){
	...
	O3o.drawObject_ = function(obj,physics){
		var o3o = obj.o3o;
	...
	var calcModifiers=function(obj,flg){
		for(var i=0,imax=obj.modifiers.length;i<imax;i++){
			if(obj.modifiers.type!="MIRROR" && flg){
				continue;
			}
			calcModifier(obj,obj.modifiers[i]);
		}
	}
	var calcModifier=function(obj,mod){
	...
	O3o.createPhyObj = function(obj,onoPhy){
		...
		if(obj.rigid_body){
			...
			phyobj.dfriction=rigid.friction;

      		var shape = rigid.colligion_shape;
			if(shape != ""){
				var collision=null;
				if(shape == "SPHERE"){
					collision=onoPhy.createCollision(OnoPhy.SPHERE);
				}else if(shape=="BOX"){
					collision=onoPhy.createCollision(OnoPhy.CUBOID);
				}else if(shape=="CAPSULE"){
					collision=onoPhy.createCollision(OnoPhy.CAPSULE);
				}else if(shape=="MESH"){
					collision=onoPhy.createMesh(obj.data);
				}
				if(collision){
					collision.size[0]=obj.bound_box[3];
					collision.size[1]=obj.bound_box[4];
					collision.size[2]=obj.bound_box[5];
