...
var O3o=(function(){
	...
	var deformBuf2=[];
	for(var i=0;i<128;i++){
		deformBuf0.push(new Vec3());
		deformBuf1.push(new Vec3());
		deformBuf2.push(new Vec3());
	...
	var modMeshDeform=function(dst,obj,mod,phyObjs){
		...
		for(var i=0;i<deformMesh.faces.length;i++){
			...
			//平面方向
			Vec3.sub(deformBuf0[i],dmv1,dmv0);
			if(face.idxnum==4){
				Vec3.sub(deformBuf1[i],dmv3,dmv0);
				Vec3.sub(deformBuf2[i],dmv2,deformBuf0[i]);
				Vec3.sub(deformBuf2[i],deformBuf2[i],dmv3);
			}else{
				Vec3.sub(deformBuf1[i],dmv2,dmv0);
			...
			var d = deformBuf[i];
			if(face.idx===4){
				Vec3.cross3(d,dmv0 ,dmv2 ,dmv1 ,dmv3);
			}else{
				Vec3.cross2(d,dmv0 ,dmv1 ,dmv2);
			}
			Vec3.norm(d);
		}
		for(var i=0;i<dst.vertexSize;i++){
			...
			for(var j=0;j<binds.length;j++){
				//バインド数分ループ
				var bind=binds[j];
				var bindidx = bind.idx;
				var dmv0 = dmv[deformMesh.faces[bind.idx].idx[0]].pos;

				//バインド情報に係数をかけて座標を算出
				Vec3.madd(f,dmv0,deformBuf0[bindidx],bind.pweight[0]);
				Vec3.madd(f,f,deformBuf1[bindidx],bind.pweight[1]);
				Vec3.madd(f,f,deformBuf2[bindidx],bind.pweight[0]*bind.pweight[1]);
				Vec3.madd(f,f,deformBuf[bindidx],bind.len);
	...
	var drawObject = function(o3o,obj,physics){
		...
		for(var i=0;i<bufMesh.faceSize;i++){
			...
			for(var j=0;j<face.idxnum-2;j++){
				...
				renderFace.materialIndex= materialTable[face.mat+1];
				renderFace.material= ono3d.renderMaterials[renderFace.materialIndex];
