...
var O3o=(function(){
		...
		var bind=function(object,modifier){
			...
			for(var i=0;i<bufMesh.vertexSize;i++){
				...
				for(var j=0;j<mod.faceSize;j++){
					...
					if(face.idxnum==4){
						Geono.calcSquarePos(ret1,mod.vertices[face.idx[0]].pos
					...
					//if(ret1[0]<0 || ret1[0]>1 || ret1[1]<0 || ret1[1]>1){
					//	continue;
					//}
	...
	for(var type in o3o2){
		if(type === "textures"){
			...
			for(line in o3o2[type]){
				...
				object.poseArmature = new PoseArmature();

				//for(line3 in o3o2[type][line].poseBones){
				//	var poseBone = new PoseBone()
				//	object.poseArmature.poseBones.push(poseBone)
				//	poseBone.target=object.data.bones[i];
		...
		for(j=o3o.objects.length;j--;){
			...
			if(object.type==="MESH"){
				...
				var poseBones = object.poseArmature.poseBones;
				for(var k=0;k<object.data.bones.length;k++){
					var poseBone = new PoseBone();
					poseBone.target = object.data.bones[k];
					//poseBone.parent= poseBone.target.parent;
					//poseBone.name=poseBone.target.name;
					poseBones.push(poseBone);
				}
				//for(k=0;k<poseBones.length;k++){
				//	poseBones[k].target=name2Obj(poseBones[k].target,object.data.bones)
				//	poseBones[k].parent=name2Obj(poseBones[k].parent,poseBones)
				//}
			}else{
				object.objecttype="";
				object.data=null
		...
		for(j=o3o.objects.length;j--;){
			...
			if(object.parent){
				if(object.parent_bone){
					object.parent_bone=name2Obj(object.parent_bone,object.parent.poseArmatur.eposeBones);
				}
			}
		}
	...
	var freezeMesh = function(dst,obj,physics){
		...
		var flg=false;
		Mat43.dotMat44(defMat,ono3d.worldMatrix,obj.mixedmatrix);
		if(physics){
			for(i=0;i<physics.length;i++){
				if( physics[i].name===obj.name){
					...
					matrix[3]=phyObj.matrix[3];
					matrix[4]=phyObj.matrix[4];
					matrix[5]=phyObj.matrix[5];
					matrix[6]=phyObj.matrix[6];
					matrix[7]=phyObj.matrix[7];
					matrix[8]=phyObj.matrix[8];
					matrix[9]=phyObj.matrix[9];
					matrix[10]=phyObj.matrix[10];
					matrix[11]=phyObj.matrix[11];
	...
	var calcMatrixArmature=function(armature,frame){
		var poseBones=armature.poseArmature.poseBones;
		for(var i=0;i<poseBones.length;i++){
			//ボーンの数だけ計算する
			var poseBone=poseBones[i];
			var matrix=poseBone.matrix;
			if(armature.action){
				//対象ボーンのアクションを計算
				addaction(poseBone,poseBone.target.name,armature.action,frame)
			}
			genMatrix(poseBone); //ボーンの姿勢から行列を作成
			Mat43.dot(matrix,matrix,poseBone.target.imatrix)
			Mat43.dot(matrix,poseBone.target.matrix,matrix)
			poseBone.flg=false; //行列の親子合成フラグをリセット
		}
		for(var i=0;i<poseBones.length;i++){
			mixBoneMatrix(poseBones[i],armature.poseArmature) //ボーンの行列の親子合成
		}
	}
	var genMatrix=function(obj){
		var mat=obj.matrix;
		if(obj.rotation_mode===QUATERNION){
			Vec4.toMat43(mat,obj.rotation);
		...
		s=obj.scale[1];
		mat[3]*=s; mat[4]*=s; mat[5]*=s;
		s=obj.scale[2];
		mat[6]*=s; mat[7]*=s; mat[8]*=s;

		mat[9]=obj.location[0];
		mat[10]=obj.location[1];
		mat[11]=obj.location[2];
	}
	...
	var mixBoneMatrix=function(poseBone,poseArmature){
		if(poseBone.target.parent){
			var parent = poseArmature.poseBones[poseBone.target.parent.id];
			if(!parent.flg){
				mixBoneMatrix(parent,poseArmature);
			}
			Mat43.dot(poseBone.mixedmatrix,parent.mixedmatrix,poseBone.matrix);
		}else{
			Mat43.copy(poseBone.mixedmatrix,poseBone.matrix);
		}
		poseBone.flg=true;
	}
	...
	var modMeshDeform=function(dst,obj,mod,phyObjs){
		...
		for(var i=0;i<dst.vertexSize;i++){
			...
			for(var j=0;j<binds.length;j++){
				var bind=binds[j];
				var face = deformMesh.faces[bind.idx];

				Vec3.sub(e,dmv[face.idx[1]].pos,dmv[face.idx[0]].pos);
				Vec3.madd(e,dmv[face.idx[0]].pos,e,bind.pweight[0]);
				if(face.idxnum==4){
					Vec3.sub(f,dmv[face.idx[2]].pos,dmv[face.idx[3]].pos);
					Vec3.madd(f,dmv[face.idx[3]].pos,f,bind.pweight[0]);
				}else{
					Vec3.sub(f,dmv[face.idx[2]].pos,dmv[face.idx[1]].pos);
					Vec3.add(f,f,e);
				}
				Vec3.sub(f,f,e);
				Vec3.madd(f,e,f,bind.pweight[1]);
				
				if(face.idx===4){
					Vec3.cross3(ret1,dmv[face.idx[0]].pos
						,dmv[face.idx[2]].pos
						,dmv[face.idx[1]].pos
						,dmv[face.idx[3]].pos);
				}else{
					Vec3.cross2(ret1,dmv[face.idx[0]].pos
						,dmv[face.idx[1]].pos
						,dmv[face.idx[2]].pos);
				}
				Vec3.norm(ret1);
				Vec3.madd(f,f,ret1,bind.len);
	...
	var modMirror = function(dst,obj,mod){
		...
		for(var j =0;j<faceSize;j++){
			...
			dstvertices[1] = srcvertices[2];
			dstvertices[2] = srcvertices[1];
			...
			if(srcFace.idxnum===4){
				dstvertices[0] = srcvertices[0];
				dstvertices[1] = srcvertices[3];
				dstvertices[2] = srcvertices[2];
				dstvertices[3] = srcvertices[1];
				dst[0] = src[0];
				dst[1] = src[1];
				dst[2] = src[6];
				dst[3] = src[7];
				...
				if(abs(bufMeshVertices[dstvertices[3]].pos[mrr])>0.01){
					dstvertices[3]+=vertexSize;
				}
			}
	...
	var modArmature=function(dst,obj,mod){
		...
		for(var j=groups.length;j--;){
			...
			for(var k=0,kmax=poseBones.length;k<kmax;k++){
				if(poseBones[k].target.name!=groupName)continue
				groupMatFlg[j] = true;
	...
	var drawObject = function(o3o,obj,physics){
		...
		if( ono3d.rf  & Ono3d.RF_OUTLINE){
			var bufFace,normal;
			Mat43.getInv(bM44,ono3d.viewMatrix);
			var cp0=bM44[9];
			var cp1=bM44[10];
			var cp2=bM44[11];
			for(i=0;i<rfCount;i++){
	...
	var movePhyObj = ret.movePhyObj = function(object,flg){
		...
		if(phyObj.type===OnoPhy.CLOTH){
			...
			if(phyObj.fix || flg){
				...
				Mat43.dotMat44(bM,ono3d.worldMatrix,object.mixedmatrix);
				var mat=bM;phyObj.matrix;

				Vec3.set(phyObj.location,mat[9],mat[10],mat[11]);
