...
var O3o=(function(){
	...
	O3o.createPhyObj = function(obj,onoPhy){
		...
		if(obj.rigid_body.type){
			...
			phyobj = onoPhy.createPhyObj(rigid.collision_shape,obj.data);
		...
		for(var i=0;i<obj.modifiers.length;i++){
			...
			if(mod.type==="CLOTHA"){
				...
				phyobj = OnoPhy.Cloth(calcMesh.vertexSize
					,calcMesh.edgeSize
					,calcMesh.faceSize);
				...
				for(j=0;j<calcMesh.faceSize;j++){
					face=renderFaces[j]
					if(face.idxnum=== 3){
						phyobj.faces[j].point1 = phyobj.points[face.idx[0]];
						phyobj.faces[j].point2 = phyobj.points[face.idx[1]];
						phyobj.faces[j].point3 = phyobj.points[face.idx[2]];
					}
				}

				for(var j =0;j<calcMesh.edgeSize;j++){
					var edge=calcMesh.edges[j];
					phyobj.edges[i].point1 = phyobj.points[edge.v0];
					phyobj.edges[i].point2 = phyobj.points[edge.v1];
					phyobj.edges[i].len=Vec3.len(phyobj.edges[i].point1.location
						,phyobj.edges[i].point2.location);

				}
			}
			else if(mod.type==="CLOTH" || mod.type==="SOFT_BODY"){
				mesh = obj.data;
				vertices = calcMesh.vertices;
				calcVertex(obj);
				_calcFaces(obj);
	...
	var createPhyJoint= O3o.createPhyJoint = function(sceneObject,phyObjs,onoPhy){
	...
	var movePhyObj = O3o.movePhyObj = function(object,flg){
		var i,imax;
