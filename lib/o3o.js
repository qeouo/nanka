...
var O3o=(function(){
	...
	ret.drawObject = function(obj,physics,environment){
		var o3o = obj.o3o;
		...
		if(obj.type=="MESH"){
			drawObject(o3o,obj,physics,environment);
		}
	}
	...
	var drawObject = function(o3o,obj,physics,environment){
		if(!environment){
		...
		for(var i=0;i<bufMesh.faceSize;i++){
			...
			for(var j=0;j<face.idxnum-2;j++){
				...
				renderFace.environment = environment;
		...
		if( ono3d.rf  & Ono3d.RF_OUTLINE){
			...
			renderFace.materialIndex= ono3d.materials_index;
			ono3d.materials_index++;
			...
			for(i=0;i<bufMesh.edgeSize;i++){
				...
				if(edge.f1<0){ //属面が1こだけの場合、その面が表でないならスキップ
					...
					if(renderFaces[edge.f0+ono3d.faces_index].cul
						* renderFaces[edge.f1+ono3d.faces_index].cul > -1)continue;
				}

				//線描画追加
				var renderFace=renderFaces[ono3d.faces_index+rfCount];
				renderFace.vertices[0] = renderVertices[ono3d.vertices_index+edge.v0]
				renderFace.vertices[1] = renderVertices[ono3d.vertices_index+edge.v1]
