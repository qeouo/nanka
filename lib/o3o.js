...
var O3o=(function(){
	...
	var Material = function(){
		...
		this.roughness=0.0;
		Vec3.set(this.metalColor,1,1,1);
		this.ior=1.0;
	...
	var onloadfunc= function(o3o,url,buf){
		...
		for(i=o3o.scenes.length;i--;){
			o3o.scenes[i].lightMap = loadMap(o3o.scenes[i].lightMap,0);
		}
		...
		for(var i=0;i<bufMesh.faceSize;i++){
			...
			for(var j=0;j<face.idxnum-2;j++){
				...
				if(lightMapUvData){
					uv = lightMapUvData[i];
					if(uv){
						renderFace.nuv[0][0] = uv[0];
						renderFace.nuv[0][1] = uv[1];
						renderFace.nuv[1][0] = uv[2+j*2];
						renderFace.nuv[1][1] = uv[3+j*2];
						renderFace.nuv[2][0] = uv[4+j*2];
						renderFace.nuv[2][1] = uv[5+j*2];
					}
				}
