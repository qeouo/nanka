...
var O3o=(function(){
	...
	var onloadfunc= function(o3o,url,buf){
		...
		var fu = function(A,B,C,D,P){
			...
			for(var i=0;i<3;i++){
				var i2=(i+1)%3;
				a  = BA[i]*CD[i2] - BA[i2]*CD[i];
				b  = BA[i]*DP[i2] + AP[i]*CD[i2] - (BA[i2]*DP[i] + AP[i2]*CD[i]);
				c  = AP[i]*DP[i2] - AP[i2]*DP[i];
			}
			a*=n[0];
			b*=n[1];
			c*=n[2];
			...
			var BP = new Vec3();
			Vec3.sub(BP,B,P);
			if((t>0 && (AD[0]*AP[1] - AD[1]*AP[0]>0))
			|| (t<1 && (BC[0]*BP[1] - BC[1]*BP[0]<0))){
		...
		var bind=function(object,modifier){
			...
			for(var i=0;i<bufMesh.vertexSize;i++){
				...
				for(var j=0;j<binds.length;j++){
					...
					if(face.idxnum==4){
						bind.pweight[0]=fu(mod.vertices[face.idx[0]].pos
										,mod.vertices[face.idx[1]].pos
										,mod.vertices[face.idx[2]].pos
										,mod.vertices[face.idx[3]].pos,ret2);
						bind.pweight[1]=fu(mod.vertices[face.idx[1]].pos
										,mod.vertices[face.idx[2]].pos
										,mod.vertices[face.idx[3]].pos
										,mod.vertices[face.idx[0]].pos,ret2);
					}else{
						var v = new Vec3();
