...
var O3o=(function(){
	...
	var SceneObject = function(){
		...
		this.iparentmatrix=new Mat44();
		this.actmatrix=new Mat44()
		this.action="";
		this.data="";
		this.parent="";
	...
	var bufMesh=new Mesh();
	for(i=0;i<1000;i++){
		var vertex=new Vertex();
		bufMesh.vertices.push(vertex);
		for(var j=0;j<8;j++){
			vertex.groups.push(-1);
			vertex.groupWeights.push(-1);
		}

		var face=new Face();
		bufMesh.faces.push(face);
	...
	var onloadfunc= function(o3o,url,buf){
		...
		var bind=function(object,modifier){
			var dst = object.data;
			var src = modifier.object.data;
			for(var i=0;i<dst.vertices.length;i++){
	...
	for(var type in o3o2){
		...
		for(j=o3o.objects.length;j--;){
			...
			Mat44.getInv(object.imatrix,object.matrix)
	...
	var calcVertices = function(obj,buf){
		if(!buf){
			buf = bufMesh;
		}
		var src,dst
			,srcarr,dstarr
			,dstvertices=buf.vertices
		...
		for(var i = vertexSize;i--;){
			...
			dst.groupWeights=src.groupWeights;
		}
		buf.vertexSize=vertexSize;

	}
	var calcFaces =function(obj){
		var mesh=obj.data;
		...
		- transMat[1]*transMat[4]*transMat[10]
			...
			for(i=faces.length;i--;){
				var calcFace=bufMesh.faces[i];
				var face=faces[i];
				if(uv_layerdata){
					uv=uv_layerdata[i];
				}
				//bufMesh.faces[i]=calcFace;
				calcFace.material = face.material;
				calcFace.mat= face.mat;
				calcFace.idxnum=face.idxnum;
				calcFace.fs = face.fs;

				if(calcFace.idxnum===4){

					calcFace.idx[3]= face.idx[3];
					calcFace.uv[6] = uv[6];
					calcFace.uv[3] = uv[7];
					calcFace.uv[4] = uv[4];
					calcFace.uv[5] = uv[5];
		...
		for(i=0;i<mesh.edges.length;i++){
			bufMesh.edges[i] = mesh.edges[i]
		}
		bufMesh.edgeSize = mesh.edges.length;
	...
	var calcMatrixArmature=function(armature,frame){
		...
		for(var i=posebones.length;i--;){
			...
			genMatrix(posebone);
			Mat44.dot(actmatrix,actmatrix,posebone.target.imatrix)
			Mat44.dot(actmatrix,posebone.target.matrix,actmatrix)
			posebone.flg=false;
		}
	...
	var getMatrix=function(obj,frame){
		...
		if(obj.parent){
			...
			Mat44.dot(obj.mixedmatrix,obj.iparentmatrix,obj.actmatrix);
			Mat44.dot(obj.mixedmatrix,obj.parent.mixedmatrix,obj.mixedmatrix);
		}else{
			Mat44.copy(obj.mixedmatrix,obj.actmatrix);
		}
		obj.flg=true;
		return obj.mixedmatrix;
	}
	...
	var mixBoneMatrix=function(bone){
		if(bone.parent){
			...
			Mat44.dot(bone.mixedmatrix,bone.parent.mixedmatrix,bone.actmatrix);
		}else{
			Mat44.copy(bone.mixedmatrix,bone.actmatrix);
		}
		bone.flg=true;
	}
	...
	ret.freeze=function(obj,modifier){
		...
		for(var i=0;i<bufMesh.vertexSize;i++){
			...
			var srcvertex=bufMesh.vertices[i];
	...
	var drawObject = function(o3o,obj,physics){
		...
		var flg=false;
		Mat44.dot(defMatrix,ono3d.worldMatrix,obj.mixedmatrix);
		if(physics){
				...
				if( physics[i].name===obj.name){
					...
					if(phyObj.type===OnoPhy.SPRING_MESH || phyObj.type===OnoPhy.CLOTH){
						for(j=phyObj.points.length;j--;){
							Vec3.copy(renderVertices[j+rvIndex].pos,phyObj.points[j].location);
						}
						flg =true;
					}
					break;
		...
		if(!flg){
			...
			for(j=bufMesh.vertexSize;j--;){
				a=renderVertices[j+rvIndex].pos;
				b=bufMeshVertices[j].pos;
				x=b[0];
				y=b[1];
				z=b[2];
		...
		for(var i=bufMesh.faceSize;i--;){
			...
			if(face.mat>=0){
				offsetx=renderFace.material.offsetx
				offsety=renderFace.material.offsety
				renderFace.uv[0][0] = face.uv[2]+offsetx
				renderFace.uv[0][1] = face.uv[3]+offsety
				renderFace.uv[1][0] = face.uv[4]+offsetx
				renderFace.uv[1][1] = face.uv[5]+offsety
				renderFace.uv[2][0] = face.uv[0]+offsetx
				renderFace.uv[2][1] = face.uv[1]+offsety
			}
	...
	var movePhyObj = ret.movePhyObj = function(object,flg){
		...
		if(phyObj.type===OnoPhy.CLOTH){
			...
			Mat44.dot(defMatrix,ono3d.worldMatrix,object.mixedmatrix);
			if(flg){
				for(var i=0,imax=phyObj.points.length;i<imax;i++){
					Mat44.dotMat44Vec3(phyObj.points[i].location,defMatrix,bufMeshVertices[i].pos)
			...
			for(var i=0,imax=phyObj.poses.length;i<imax;i++){
				Mat44.dotMat44Vec3(truepos[i],defMatrix,bufMeshVertices[i].pos)
			}
			if(flg){
