...
var O3o=(function(){
	...
	O3o.drawScene = function(scene,physics,obj){
		var i;
		var o3o=scene.o3o;
		var objects = scene.objects;
	...
	O3o.drawObject = function(obj,physics){
		var o3o = obj.o3o;
	...
	O3o.freeze=function(obj,modifier){
		//モディファイア適用
		calcVertex(obj);
		_calcFaces(obj);
	...
	O3o.createPhyObj = function(obj,onoPhy){
		...
		if(obj.rigid_body.type){
			...
			if(shape != ""){
				var collision=null;
				if(shape == "SPHERE"){
					collision=onoPhy.createCollision(Collider.SPHERE);
				}else if(shape=="BOX"){
					collision=onoPhy.createCollision(Collider.CUBOID);
				}else if(shape=="CAPSULE"){
					collision=onoPhy.createCollision(Collider.CAPSULE);
				}else if(shape=="MESH"){
					collision=onoPhy.createMesh(obj.data);
				}
				if(collision){
					var b = obj.bound_box;
					...
					collision.location[0]=(b[3] + b[0])*0.5;
					collision.location[1]=(b[4] + b[1])*0.5;
					collision.location[2]=(b[5] + b[2])*0.5;

					phyobj.children.push(collision);
		...
		for(var i=0;i<obj.modifiers.length;i++){
			...
			if(mod.type==="CLOTH" || mod.type==="SOFT_BODY"){
				...
				if(mod.type==="CLOTH"){
					phyobj.friction=mod.air_damping*0.5;
					phyobj.mass=mod.mass;
					phyobj.speed=1.0;
					phyobj.goalDefault=1.0;
					phyobj.goalMin= 0.;
					phyobj.goalMax= 1.0;
					phyobj.goalSpring= 0.1;
					phyobj.goalFriction= 0.1;//mod.vel_damping;
					phyobj.edgePull= mod.structural*0.01;
					phyobj.edgePush= mod.structural*0.01;
					phyobj.edgeDamping = mod.spring_damping*0.01;
					phyobj.plastic= 0.0;
				}
		...
		if(phyobj){
			...
			movePhyObj(obj,phyobj,1);

			if(obj.rigid_body_constraint.enabled){
				var joint = onoPhy.createJoint();
				var constraint = obj.rigid_body_constraint;
				phyobj.joint = joint;
				
				joint.breaking_threshold=constraint.breaking_threshold;
				joint.disable_collisions=constraint.disable_collisions;
				joint.enabled=constraint.enabled ;
				Vec3.copy(joint.limit_ang_lower,constraint.limit_ang_lower);
				Vec3.copy(joint.limit_ang_upper,constraint.limit_ang_upper);
				Vec3.copy(joint.limit_lin_lower,constraint.limit_lin_lower);
				Vec3.copy(joint.limit_lin_upper,constraint.limit_lin_upper);
				joint.motor_ang_max_impulse=constraint.motor_ang_max_impulse;
				joint.motor_ang_target_velocity=constraint.motor_ang_target_velocity;
				joint.motor_lin_max_impulse=constraint.motor_lin_max_impulse;
				joint.motor_lin_target_velocity=constraint.motor_lin_target_velocity;
				joint.object1=constraint.object1;
				joint.object2=constraint.object2;
				Vec3.copy(joint.spring_damping,constraint.spring_damping);
				Vec3.copy(joint.spring_stiffness,constraint.spring_stiffness);
				joint.use_breaking=constraint.use_breaking;
				Vec3.copy(joint.use_limit_ang,constraint.use_limit_ang);
				Vec3.copy(joint.use_limit_lin,constraint.use_limit_lin);
				joint.use_motor_ang=constraint.use_motor_ang;
				joint.use_motor_lin=constraint.use_motor_lin;
				Vec3.copy(joint.use_spring,constraint.use_spring);
				Vec3.copy(joint.use_spring_ang,constraint.use_spring_ang);
	...
	var movePhyObj = O3o.movePhyObj = function(object,phyObj,flg){
		var i,imax;
