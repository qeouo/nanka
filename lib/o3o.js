...
var O3o=(function(){
	...
	for(var type in o3o2){
		...
		for(j=o3o.objects.length;j--;){
			...
			if(object.type==="MESH"){
				...
				//object.poseArmature.armature = object.data;
				//var poseBones = object.poseArmature.poseBones;
				//for(var k=0;k<object.data.bones.length;k++){
				//	poseBones.push(new PoseBone());
				//	object.poseArmature.matrices.push(new Mat43());
				//}
			}else{
				object.objecttype="";
				object.data=null
	...
	var genMatrix=function(mat,obj){
		if(obj.rotation_mode===QUATERNION){
			...
			Mat43.fromRotVector(mat,obj.rotation[0],1,0,0)
			Mat43.fromRotVector(bM,obj.rotation[1],0,1,0)
			Mat43.dot(mat,bM,mat);
			Mat43.fromRotVector(bM,obj.rotation[2],0,0,1)
			Mat43.dot(mat,bM,mat);
		}
		var s=obj.scale[0];
	...
	var drawObject = function(o3o,obj,physics){
		...
		for(var i=0;i<bufMesh.faceSize;i++){
			...
			renderFace.vertices[1]=renderVertices[face.idx[1] + rvIndex];
			renderFace.vertices[2]=renderVertices[face.idx[2] + rvIndex];
			...
			if(face.idxnum==3){
				Vec3.cross2(renderFace.normal,renderFace.vertices[0].pos
					,renderFace.vertices[1].pos
					,renderFace.vertices[2].pos);
			}else{
				Vec3.cross3(renderFace.normal,renderFace.vertices[0].pos
					,renderFace.vertices[2].pos
					,renderFace.vertices[1].pos
		...
		for(var i=0;i<bufMesh.faceSize;i++){
			...
			if(face.idxnum==4){
				renderFace = renderFaces[rfIndex+rfCount];
				rfCount++;
				Vec3.copy(renderFace.normal,renderFaces[rfIndex+i].normal);
				renderFace.reverseFlg=0;
		...
		for(var i=0;i<rfCount;i++){
			...
			//Vec3.sub(bV0, renderFace.vertices[0].pos,renderFace.vertices[1].pos)
			//Vec3.sub(bV1, renderFace.vertices[0].pos,renderFace.vertices[2].pos)
			//
			//Vec3.cross(renderFace.normal,bV0,bV1);
			//Vec3.norm(renderFace.normal);

			Vec3.set(renderFace.angle,0,0,0)
			for(var j=3;j--;){
				Vec3.add(renderFace.vertices[j].normal
					,renderFace.vertices[j].normal
						,renderFace.normal)
				Vec3.add(renderFace.angle,renderFace.angle,renderFace.vertices[j].pos);
			}

			Vec3.norm(renderFace.angle);
		}
		Vec3.poolFree(2);
		if( ono3d.smoothing>0){
	...
	var movePhyObj = ret.movePhyObj = function(object,dt,flg){
		...
		if(phyObj.type===OnoPhy.CLOTH){
			...
			if(phyObj.fix || flg){
				...
				Mat43.dotMat44Mat43(bM,ono3d.worldMatrix,object.mixedmatrix);
