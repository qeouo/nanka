...
var O3o=(function(){
	...
	var calcMatrixArmature=function(obj,frame){
		var poseArmature=obj.poseArmature;
		var armature=poseArmature.armature;
		var poseBones = poseArmature.poseBones;
		for(var i=0;i<poseBones.length;i++){
			...
			if(obj.action){
				//対象ボーンのアクションを計算
				addaction(poseBone,bone.name,obj.action,frame)
			}
			poseBone.flg=false; //行列の親子合成フラグをリセット
		}
		for(var i=0;i<poseBones.length;i++){
			mixBoneMatrix(i,poseArmature) //ボーンの行列の親子合成
		}
	}

	var mixBoneMatrix=function(n,poseArmature){
		var poseBone = poseArmature.poseBones[n];
		var bones = poseArmature.armature.bones;

		var matrix = poseArmature.matrices[n];
		...
		if(bones[n].parent){
			...
			var parent = poseArmature.poseBones[m];
			if(!parent.flg){
				mixBoneMatrix(m,poseArmature);
			}

			Mat43.dot(poseArmature.matrices[n],poseArmature.matrices[m],matrix);
	//	}else{
	//		//Mat43.copy(poseArmature.matrices[n],poseBone.matrix);
		}
		poseBone.flg=true;
	}
