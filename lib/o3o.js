...
var O3o=(function(){
	...
	var onloadfunc= function(o3o,url,buf){
		...
		for(i=o3o.textures.length;i--;){
			...
			if(flg){
				texture.image = Util.loadImage(texture.path,0,null,1);
			}else{
	...
	for(var type in o3o2){
		if(type === "textures"){
			...
			for(line in o3o2[type]){
				...
				material.rough= 1.0/material.spchard;
		...
		for(i=o3o.materials.length;i--;){
			for(j=o3o.materials[i].texture_slots.length;j--;){
				o3o.materials[i].texture_slots[j].texture=name2Obj(o3o.materials[i].texture_slots[j].texture,o3o.textures)
				//var res=null;
	...
	var setFaces =function(){
		...
		//	renderFace.rough= material.rough;
		//	renderFace.env= material.env;
		...
		//				renderFace.normalmap= texture_slot.texture.image;
		//			}else{
		//				renderFace.texture = texture_slot.texture.image;
		//			}
		//		}
		//	}
			renderFace.material= materialTable[face.mat+1];
			if(face.mat>=0){
				offsetx=face.material.offset[0];
				offsety=face.material.offset[1];
	...
	var setMaterial=function(material,name){
		...
		renderMaterial.mrr = material.mrr
		...
		for(var j=0;j<obj.modifiers.length;j++){
			...
			if(mod.type==="CLOTH" || mod.type==="SOFT_BODY"){
				...
				if(mod.type==="SOFT_BODY"){
					res.friction= mod.friction*0.1;
					res.mass=mod.mass;///10;
					res.speed= mod.speed;
	...
	var initPhyObjs=O3o.initPhyObjs=function(scene,phyObjs){
		var i,imax,j;
		var objects = scene.objects;
		var object;
		var phyName,phyObj;
