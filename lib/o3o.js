...
//					Vec3.copy(renderFace.angle,bufFace.angle)
	...
	var calcNormal = function(mesh){
		...
		for(i=rfSize;i--;){
			...
			if(bufFace.rf & Ono3d.RF_OUTLINE){
				if(Vec3.dot(bufFace.normal,bufFace.vertices[0].pos)<0){
					bufFace.cul=1;
				}else{
					bufFace.cul=-1;
				}	
		...
		for(var i=0,imax=obj.modifiers.length;i<imax;i++){
			...
			if(mod.type=="MIRROR"){
				...
				if(phyObj){
					...
					Mat43.getInv(bM,ono3d.worldMatrix);
	...
	var drawObject = O3o.drawObject = function(obj,physics){
		...
		if(ono3d.transMat[0]*ono3d.transMat[5]*ono3d.transMat[10]
		+ ono3d.transMat[1]*ono3d.transMat[6]*ono3d.transMat[8]
		+ ono3d.transMat[2]*ono3d.transMat[4]*ono3d.transMat[9]
		- ono3d.transMat[0]*ono3d.transMat[6]*ono3d.transMat[9]
		- ono3d.transMat[2]*ono3d.transMat[5]*ono3d.transMat[8]
		- ono3d.transMat[1]*ono3d.transMat[4]*ono3d.transMat[10]
			<0){
			for(j=rfSize;j--;){
				var a= renderFaces[j+rfIndex].vertices[0]
				renderFaces[j+rfIndex].vertices[0] = renderFaces[j+rfIndex].vertices[1]
				renderFaces[j+rfIndex].vertices[1] = a
				a= renderFaces[j+rfIndex].uv[0][0]
				renderFaces[j+rfIndex].uv[0][0] = renderFaces[j+rfIndex].uv[1][0]
				renderFaces[j+rfIndex].uv[1][0] = a
				a= renderFaces[j+rfIndex].uv[0][1]
				renderFaces[j+rfIndex].uv[0][1] = renderFaces[j+rfIndex].uv[1][1]
				renderFaces[j+rfIndex].uv[1][1] = a
			}
...
//		var edges=obj.data.edges;
			for(i=0;i<edges.length;i++){
				...
				if((renderFaces[edge.f0+ono3d.renderFaces_index].cul>0 && edge.f1<0)
				|| (renderFaces[edge.f0+ono3d.renderFaces_index].cul
				* renderFaces[edge.f1+ono3d.renderFaces_index].cul
				<0 && edge.f1>=0)){
	...
	O3o.createPhyObjs = function(obj,onoPhy){
		...
		for(var j=0;j<obj.modifiers.length;j++){
			...
			if(mod.type=="CLOTH" || mod.type=="SOFT_BODY"){
				...
				if(mod.type=="CLOTH"){
					res.mass=mod.mass;
					res.structural=mod.structural;
					res.bending_stiffness=mod.bending_stiffness;
					res.spring_damping=mod.spring_damping;
				...
				for(j=0;j<rvSize;j++){
					for(k=j+1;k<rvSize;k++){
						if(Vec3.len(renderVertices[j+rvIndex].pos,renderVertices[k+rvIndex].pos)<0.0001){
							var joint={};
							joint.v0=j;
							joint.v1=k;
