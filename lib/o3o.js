...
var O3o=(function(){
	...
	var PoseArmature = ret.PoseArmature = (function(){
		...
		ret.add= function(a,b,c){
			for(var i=0;i<b.poseBones.length;i++){
				PoseBone.madd(a.poseBones[i],b.poseBones[i],c.poseBones[i],1);
			}
		}
	...
	var PoseBone = (function(){
		...
		ret.mul=function(a,b,c){
			...
			Vec3.mul(a.scale,b.scale,c);
			Vec4.qmul(a.rotation,b.rotation,c);
		}
		ret.madd=function(a,b,c,d){
			Vec3.madd(a.location,b.location,c.location,d);
			Vec3.madd(a.scale,b.scale,c.scale,d);
			Vec4.qmdot(a.rotation,b.rotation,c.rotation,d);
		}
		return ret;
	})();
	...
	var modArmature=function(dst,obj,mod){
		...
		for(var i=0;i<poseBones.length;i++){
			poseArmature.mixBoneMatrix(i) //ボーンの行列の親子合成
		}
