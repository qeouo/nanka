...
//				//近い頂点を探す
//				for(var j=0;j<mod.vertexSize;j++){
//					var l= Vec3.len2(mod.vertices[j].pos,pos);
					//より近いものを採用
					if(binds[0].weight>l){
						binds[0].idx=j;
						binds[0].weight=l;
	...
	//				//ソート
	//				for(var k=0;k<bind.weight.length-1;k++){
	//					if(bind.weight[k]<bind.weight[k+1]){
	//						var a=bind.weight[k+1];
	//						bind.weight[k+1]=bind.weight[k];
	//						bind.weight[k]=a;
	//						a=bind.idx[k+1];
	//						bind.idx[k+1]=bind.idx[k];
	//						bind.idx[k]=a;
	//					}else{
	//						break;
	//					}
	//				}
				...
				for(var j=0;j<binds.length;j++){
					...
					var lsum=0;
					for(var k=0;k<3;k++){
						bind.pweight[k] = Vec3.len(ret2,mod.vertices[face.idx[k]].pos);
						if(bind.pweight[k]===0){
							bind.pweight[0]=0;
							bind.pweight[1]=0;
							bind.pweight[2]=0;
						...
						bind.pweight[k]=1/ bind.pweight[k];
						lsum+=bind.pweight[k];
					}
					lsum=1/lsum;
					for(var k=0;k<3;k++){
