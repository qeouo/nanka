...
var O3o=(function(){
	...
	O3o.createPhyObj = function(obj,onoPhy){
		...
		if(obj.rigid_body.type){
			...
			if(shape != ""){
				...
				if(shape == "SPHERE"){
					...
					collision=onoPhy.createCollision(Collider.CYLINDER);
				}else if(shape=="CONE"){
					collision=onoPhy.createCollision(Collider.CONE);
				}else if(shape=="CAPSULE"){
					collision=onoPhy.createCollision(Collider.CAPSULE);
				}else if(shape=="CONVEX_HULL"){
					collision=onoPhy.createConvex(obj.data);
		...
		if(phyobj){
			...
			var rbc = obj.rigid_body_constraint;
