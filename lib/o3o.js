...
var O3o=(function(){
	...
	var onloadfunc= function(o3o,url,buf){
		...
		for(i=o3o.textures.length;i--;){
			...
			if(flg){
				...
				texture.image = Ono3d.loadTexture(texture.path,function(tex){
						var gl = Rastgl.gl;
	...
	for(var type in o3o2){
		...
		for(i=o3o.materials.length;i--;){
			for(j=o3o.materials[i].texture_slots.length;j--;){
				...
				texture_slot.texture=o3o.textures.find(function(a){return a.name === this;},texture_slot.texture)
				texture_slot.action=o3o.actions.find(function(a){return a.name === this;},texture_slot.action)
			}
		}
		var count=0
	...
	var setMaterial=function(material,name){
		...
		renderMaterial.opacity = material.baseColor[3];
		renderMaterial.metalness = material.spc;
		renderMaterial.roughness= material.rough;
		renderMaterial.metalColor = material.reflectionColor;
		renderMaterial.ior = material.ior;
		renderMaterial.subRoughness = material.trans_rough;
	...
	ret.drawObject = function(obj,physics,environment,environment2,envRatio){
		...
		if(obj.type=="MESH"){
			drawObject(o3o,obj,physics,environment,environment2,envRatio);
		}
	}
	...
	var drawObject = function(o3o,obj,physics,environment,environment2,envratio){
		if(!environment){
		...
		var maxindex=rfIndex+bufMesh.faceSize-1;
		for(var i=0;i<bufMesh.faceSize;i++){
			...
			if(face.idxnum==3){
				Vec3.cross2(bV0
					,renderVertices[idx[0] + rvIndex].pos
					,renderVertices[idx[1] + rvIndex].pos
					,renderVertices[idx[2] + rvIndex].pos);
			}else{
				Vec3.cross3(bV0
					,renderVertices[idx[0] + rvIndex].pos
					,renderVertices[idx[2] + rvIndex].pos
					,renderVertices[idx[1] + rvIndex].pos
					,renderVertices[idx[3] + rvIndex].pos);
			}
			Vec3.norm(bV0);
			...
			for(var j=0;j<face.idxnum-2;j++){
				...
				if(j>0){
					//四角以上のポリゴンは後ろに追加する
					renderFace = renderFaces[maxindex+(rfCount -i)];
				}
				rfCount++;
				...
				renderFace.vertices[0]=renderVertices[idx[0] + rvIndex];
				renderFace.vertices[1]=renderVertices[idx[1+j] + rvIndex];
				renderFace.vertices[2]=renderVertices[idx[2+j] + rvIndex];
		...
		if( ono3d.rf  & Ono3d.RF_OUTLINE){
			...
			renderMateriala.r = ono3d.lineColor[0];
			renderMateriala.g = ono3d.lineColor[1];
			renderMateriala.b = ono3d.lineColor[2];
			renderMateriala.a = ono3d.lineColor[3];
			...
			for(i=0;i<bufMesh.edgeSize;i++){
				...
				if(edge.f1<0){ //属面が1こだけの場合、その面が表でないならスキップ
					if(renderFaces[edge.f0+ono3d.faces_index].cul<1)continue;
					if(renderFaces[edge.f0+ono3d.faces_index].material.opacity !== 1.0){
						renderMat = renderMateriala;
					}

				}else{ //属面が2つある場合、裏表の境界になる場合以外はスキップ
					if(renderFaces[edge.f0+rfIndex].cul
						* renderFaces[edge.f1+rfIndex].cul > -1)continue;
					if(renderFaces[edge.f0+rfIndex].material.opacity !== 1.0
					|| renderFaces[edge.f1+rfIndex].material.opacity !== 1.0){
						renderMat = renderMateriala;
					}
				}
				...
				lines_index++;
				renderLine.vertices[0] = renderVertices[vertex_current + edge.v0]
				renderLine.vertices[1] = renderVertices[vertex_current + edge.v1]
