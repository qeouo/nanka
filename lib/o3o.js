	...
	var calcNormal = function(){
		...
		for(j=ono3d.lightSources.length;j--;){
			...
			if(lightSource.type ===Ono3d.LT_DIRECTION){
				//Mat44.dotMat33Vec3(lightSource.viewAngle,ono3d.viewMatrix,lightSource.angle)
		...
		if(physics){
			for(i=0;i<physics.length;i++){
				if( physics[i].name===obj.name){
					...
					Vec4.toMat44(matrix,phyObj.rotq);
		...
		if(phyObj.type===OnoPhy.CLOTH){
			...
			if(flg){
				for(var i=0,imax=phyObj.points.length;i<imax;i++){
					Mat44.dotMat44Vec3(phyObj.points[i].location,defMatrix,calcMeshVertices[i].pos)
			...
			for(var i=0,imax=phyObj.poses.length;i<imax;i++){
				Mat44.dotMat44Vec3(truepos[i],defMatrix,calcMeshVertices[i].pos)
			}
			if(flg){
			...
			if(phyObj.fix || flg){
				...
				Mat44.dot(phyObj.matrix,ono3d.worldMatrix,object.mixedmatrix);
