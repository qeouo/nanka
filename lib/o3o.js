...
//					setdata(dstobj,src[member]);
	...
	var _PI=1.0/Math.PI;
	var calcEnv=function(renderVertex,vertex,environment,ratio){
		...
		var nrmx=renderVertex.normal[0];
		var nrmy=renderVertex.normal[1];
		var nrmz=renderVertex.normal[2];
	...
	ret.drawObject = function(obj,physics,environment,environment2,envratio){
		...
		for(var i=0;i<bufMesh.faceSize;i++){
			...
			for(var j=0;j<face.idxnum-2;j++){
				...
				Vec3.copy(renderFace.normal,face.normal);

				Vec3.copy(renderVertices[jj].pos,vertices[idx[0]].pos);
				Vec3.mul(renderVertices[jj].normal,vertices[idx[0]].normal,smoothing);
				Vec3.madd(renderVertices[jj].normal,renderVertices[jj].normal,face.normal,1-smoothing);


				Vec3.copy(renderVertices[jj+1].pos,vertices[idx[1+j]].pos);
				Vec3.mul(renderVertices[jj+1].normal,vertices[idx[1+j]].normal,smoothing);
				Vec3.madd(renderVertices[jj+1].normal,renderVertices[jj+1].normal,face.normal,1-smoothing);
				Vec3.copy(renderVertices[jj+2].pos,vertices[idx[2+j]].pos);
				Vec3.mul(renderVertices[jj+2].normal,vertices[idx[2+j]].normal,smoothing);
				Vec3.madd(renderVertices[jj+2].normal,renderVertices[jj+2].normal,face.normal,1-smoothing);
				renderFace.vertices[0]=renderVertices[jj];
				renderFace.vertices[1]=renderVertices[jj+1];
				renderFace.vertices[2]=renderVertices[jj+2];

				calcEnv(renderVertices[jj],vertices[idx[0]],environment,bufMesh.ratios[idx[0]]);
				calcEnv(renderVertices[jj+1],vertices[idx[1+j]],environment,bufMesh.ratios[idx[1+j]]);
				calcEnv(renderVertices[jj+2],vertices[idx[2+j]],environment,bufMesh.ratios[idx[2+j]]);

				//renderFace.vertices[0]=renderVertices[rvIndex+idx[0]];
				//renderFace.vertices[1]=renderVertices[rvIndex+idx[1+j]];
				//renderFace.vertices[2]=renderVertices[rvIndex+idx[2+j]];
				...
				if(face.mat>=0 && uv_layerdata){
					...
					uv = uv_layerdata[i];
					if(uv){
						renderVertices[jj].uv[0]=uv[0]+offsetx;
						renderVertices[jj].uv[1]=uv[1]+offsety;
						renderVertices[jj+1].uv[0]=uv[2+j*2]+offsetx;
						renderVertices[jj+1].uv[1]=uv[3+j*2]+offsety;
						renderVertices[jj+2].uv[0]=uv[4+j*2]+offsetx;
						renderVertices[jj+2].uv[1]=uv[5+j*2]+offsety;

//						renderFace.uv[0][0] = uv[0]+offsetx;
//						renderFace.uv[0][1] = uv[1]+offsety;
//						renderFace.uv[1][0] = uv[2+j*2]+offsetx;
//						renderFace.uv[1][1] = uv[3+j*2]+offsety;
//						renderFace.uv[2][0] = uv[4+j*2]+offsetx;
//						renderFace.uv[2][1] = uv[5+j*2]+offsety;
					}
				}
				if(lightMapUvData){
					uv = lightMapUvData[i];
					if(uv){
						renderVertices[jj].uv2[0]=uv[0]+offsetx;
						renderVertices[jj].uv2[1]=uv[1]+offsety;
						renderVertices[jj+1].uv2[0]=uv[2+j*2]+offsetx;
						renderVertices[jj+1].uv2[1]=uv[3+j*2]+offsety;
						renderVertices[jj+2].uv2[0]=uv[4+j*2]+offsetx;
						renderVertices[jj+2].uv2[1]=uv[5+j*2]+offsety;

//						renderFace.uv2[0][0] = uv[0];
//						renderFace.uv2[0][1] = uv[1];
//						renderFace.uv2[1][0] = uv[2+j*2];
//						renderFace.uv2[1][1] = uv[3+j*2];
//						renderFace.uv2[2][0] = uv[4+j*2];
//						renderFace.uv2[2][1] = uv[5+j*2];
					}
				}
				jj+=3;
		...
		//ono3d.vertices_index+=bufMesh.vertexSize;
		ono3d.vertices_index+=rfCount*3;//bufMesh.vertexSize;
		ono3d.vertices_index=jj;//rfCount*3;//bufMesh.vertexSize;
	...
	ret.setEnvironments=function(scene){
		...
		for(var i=0;i<scene.objects.length;i++){
			...
			ono3d.setOrtho(5.0,5.0,1.0,20.0)
