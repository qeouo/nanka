...
//		return t * t * t * (t * (t * 6 - 15) + 10); 
	...
	var fade3 = function(t){
		return 1-(1- 3*t*t + 2*t*t*t);
	}
  ...
  var hash3=function(x,y,z){
	...
	ret.simplexnoise = function(xin,yin,zin){
	  ...
	  var v =
		  fade(Math.max(0.5 - (x0 * x0 + y0 * y0 + z0 * z0),0))
			 * grad(hash3(i,j,k),x0,y0,z0)
		+ fade(Math.max(0.5 - (x1 * x1 + y1 * y1 + z1 * z1),0))
			* grad(hash3(i+i1,j+j1,k+k1),x1,y1,z1)
		+ fade(Math.max(0.5 - (x2 * x2 + y2 * y2 + z2 * z2),0))
			* grad(hash3(i+i2,j+j2,k+k2),x2,y2,z2)
		+ fade(Math.max(0.5 - (x3 * x3 + y3 * y3 + z3 * z3),0))
