...
var h=[ 151,160,137,91,90,15,
  ...
  var hash3=function(x,y,z){
	...
	ret.simplexnoise = function(xin,yin,zin){
      ...
      var t0 = Math.max(0.5 - (x0 * x0 + y0 * y0 + z0 * z0),0);
      var t1 = Math.max(0.5 - (x1 * x1 + y1 * y1 + z1 * z1),0);
      var t2 = Math.max(0.5 - (x2 * x2 + y2 * y2 + z2 * z2),0);
      var t3 = Math.max(0.5 - (x3 * x3 + y3 * y3 + z3 * z3),0);
	  t0=fade(t0)*4.0;
	  t1=fade(t1)*4.0;
	  t2=fade(t2)*4.0;
	  t3=fade(t3)*4.0;
      v = t0 * grad(hash3(i,j,k),x0,y0,z0)
      	+ t1 * grad(hash3(i+i1,j+j1,k+k1),x1,y1,z1)
      	+ t2 * grad(hash3(i+i2,j+j2,k+k2),x2,y2,z2)
      	+ t3 * grad(hash3(i+1,j+1,k+1),x3,y3,z3);
