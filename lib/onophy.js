...
var OnoPhy = (function(){
	...
	var HitInfo = (function(){
		...
		ret.prototype.calcPre = function(){
			...
			if(this.counter==0 || 1){
				//位置補正
				Vec3.add(this.offset,this.obj2.location,this.pos2);
				Vec3.sub(this.offset,this.offset,this.obj1.location);
				Vec3.sub(this.offset,this.offset,this.pos1);
			...
			//加速に必要な力を求めるための行列
			LinConstraint.calcEfficM(nM,obj1,this.pos1,obj2,this.pos2);
			calcEffic(this.nV,nM,this.nVec);
			calcEffic2(this.t1V,this.t2V,nM,this.t1Vec,this.t2Vec);


			Vec3.mul(impulse,this.nVec,this.impulse[0]);
			Vec3.muladd(impulse,impulse,this.t1Vec,this.impulse[1]);
			Vec3.muladd(impulse,impulse,this.t2Vec,this.impulse[2]);
		...
		ret.prototype.calcConstraint=function(){
			...
			this.impulse[0]*=0.99;
			Vec3.mul(impulse, this.nVec, this.impulse[0]-o); 
			...
			Vec3.mul(impulse,this.t1Vec,this.impulse[1]-o);
			Vec3.muladd(impulse,impulse,this.t2Vec,this.impulse[2]-old2);
	...
	var LinConstraint = (function(){
		...
		ret.prototype.init=function(obj1,pos1,obj2,pos2){
			this.obj1=obj1;
			this.obj2=obj2;
			...
			calcEfficM(m,obj1,this.pos1,obj2,this.pos2);
			...
			if(a.length==1){
				HitInfo.calcEffic(b[0],m,a[0]);
			}else if(a.length==2){
				HitInfo.calcEffic2(b[0],b[1],m,a[0],a[1]);
			}else if(a.length==3){
				HitInfo.calcEffic3(b[0],b[1],b[2],m,a[0],a[1],a[2]);
			}
	...
	var Joint = (function(){
		...
		ret.prototype.calcPre=function(){
			...
			for(var i=0;i<DIMENSION;i++){
				...
				Vec3.copy(this.linConstraint.axes[i],axis);
			...
			this.linConstraint.init(object1,obj1p,object2,obj2p);
