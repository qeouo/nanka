...
//			Vec3.mul(bV2,bV2,PENALTY);
	...
	var CUBOID_CUBOID=function(boxA,boxB,dt){
		...
		bV1[0]=sqrt(matA[0]*matA[0] + matA[1]*matA[1] + matA[2]*matA[2];
		bV1[1]=sqrt(matA[4]*matA[4] + matA[5]*matA[5] + matA[6]*matA[6];
		bV1[2]=sqrt(matA[8]*matA[8] + matA[9]*matA[9] + matA[10]*matA[10];

		bV2[0]=matB[0]*matB[0] + matB[1]*matB[1] + matB[2]*matB[2];
		bV2[1]=matB[4]*matB[4] + matB[5]*matB[5] + matB[6]*matB[6];
		bV2[2]=matB[8]*matB[8] + matB[9]*matB[9] + matB[10]*matB[10];
	...
	var calc_t=function(phyObjs,dt){
		...
		for(i = phyObjs.length;i--;){
			...
			var sx=obj.scale[0]*obj.size[0];
			var sy=obj.scale[1]*obj.size[1];
			var sz=obj.scale[2]*obj.size[2];
			matrix[0]*=sx;
			matrix[1]*=sx;
			matrix[2]*=sx;
			matrix[4]*=sy;
