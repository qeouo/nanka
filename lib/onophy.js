...
var OnoPhy = (function(){
	...
	var REPETITION_MAX=50; //繰り返しソルバ最大回数
	...
	var AngConstraint = (function(){
		...
		ret.prototype.calcAfter=function(dt){
			...
			//補正
			//必要回転力
			Vec3.mul(bV0,axis,-this.dr*PENALTY*10*dt*(a*b)/(a+b));
	...
	var Joint = (function(){
		var Joint = function(){
			...
			this.linConstraint = new HitInfo();
			this.angConstraint_ = new HitInfo();
			this.angConstraint =[]
		...
		ret.prototype.calcPre=function(){
			...
			//子相対位置
			Mat44.dot(bM,object1.inv_matrix,joint.child.matrix);
			Vec3.set(n2,bM[12],bM[13],bM[14]);

			//ジョイント相対位置
			Mat44.dot(bM,joint.parent.matrix,joint.matrix);
			Mat44.dot(bM,joint.object1.inv_matrix,bM);

			Vec3.set(n,bM[12],bM[13],bM[14]);
			Vec3.set(n3,bM[12],bM[13],bM[14]);

			//位置制限
			for(var j=0;j<DIMENSION;j++){
				if(joint.use_limit_lin[j]){
					n3[j] = n2[j];
				}
			}
			Vec3.copy(n2,n3);
			...
			Mat44.dotMat44Vec3(n,object1.matrix,n);
			Mat44.dotMat44Vec3(n2,object1.matrix,n2);
			_registHitInfo(joint.linConstraint,joint.parent,n,joint.child,n2);

		
			//角度制限
			Vec3.sub(n3,joint.parent.location,joint.child.location);
			Vec3.add(n,n,n3);
			Vec3.add(n2,n2,n3);
			_registHitInfo(joint.angConstraint_,joint.child,n2,joint.parent,n);


			for(var j=0;j<DIMENSION;j++){
				if(joint.use_limit_ang[j] || 1){
					var j1=(j+1)%3;
					var j2=(j+2)%3;
					...
					Vec3.set(n3,bM[j2*4],bM[j2*4+1],bM[j2*4+2]);
					var ll=Vec3.scalar(n3);
					Vec3.norm(n3);
					var a =Math.asin(n3[j1]);
					...
					var ll=Vec3.scalar(n3);
					Vec3.norm(n3);
					var b =Math.asin(n3[j1]);
					//if(l>joint.limit_ang_upper[j]){
						//相対回転速度を0にする
					...
					if(dr<-Math.PI){
						...
						var hI=joint.angConstraint[j];//this._registHitInfo(n3,joint.parent,n,joint.child,n2);
						Vec3.set(n3,bM[j*4],bM[j*4+1],bM[j*4+2]);
						Mat33.dotVec(n3,joint.object1.rotmat,n3);
						Vec3.norm(n3);
						hI.init(joint.parent,joint.child,n3,dr);
					//}
				}
			}
		}
		ret.prototype.calcConstraint=function(){
			if(Vec3.scalar(this.linConstraint.nVec)!=0){
				this.linConstraint.calcConstraint();
			}
			if(Vec3.scalar(this.angConstraint_.nVec)!=0){
				this.angConstraint_.calcConstraint();
			}
			for(var j=0;j<DIMENSION;j++){
				this.angConstraint[j].calcConstraint();
			}
		}
		ret.prototype.calcAfter=function(dt){
			if(Vec3.scalar(this.linConstraint.nVec)!=0){
				this.linConstraint.calcAfter(dt);
			}
			if(Vec3.scalar(this.angConstraint_.nVec)!=0){
				this.angConstraint_.calcAfter(dt);
			}
			for(var j=0;j<DIMENSION;j++){
				this.angConstraint[j].calcAfter(dt);
			}
		}
		return ret;
	})();
	...
	var _registHitInfo = (function(){
		var vec3=new Vec3();
		var vec32=new Vec3();
		return function(hitInfo,obj1,pos1,obj2,pos2){
			if(!obj1 || !obj2){
				return;
			}
			hitInfo.obj1=obj1;
			...
			Vec3.copy(hitInfo.pos2,Vec3.ZERO);
	...
	ret.prototype.calc=function(dt){
		...
		for (repetition = 0; repetition < REPETITION_MAX; repetition++) {
			...
			if ( sum<= 0.000001*hitInfos.length) {
				break;
			}
		}
