...
var OnoPhy = (function(){
	...
	ret.prototype._registHitInfo = (function(){
		var vec3=new Vec3();
		var vec32=new Vec3();
		return function(nVec,obj1,pos1,obj2,pos2){
			if(!obj1 || !obj2){
				return;
			}
			var hitInfo=null;
			...
			if(!hitInfo){
				...
				if(idx<0){
					...
					hitInfo.fricCoe = 0;//obj1.friction* obj2.friction; 
					hitInfo.restCoe = 0;//obj1.restitution* obj2.restitution;
	...
	ret.prototype.calc=function(dt){
		...
		for(var i=0;i<joints.length;i++){
			...
			//Mat44.copy(bM,object2.matrix);
			//Mat44.dot(bM,object2.inv_matrix,joint.child.matrix);
			Mat44.dot(bM,object1.inv_matrix,joint.child.matrix);
			Vec3.set(n2,bM[12],bM[13],bM[14]);

			//Mat44.dot(bM,object1.inv_matrix,joint.matrix);
			//if(joint.object1==joint.parent){
			//	Mat44.copy(bM,joint.matrix);
			//}else{
...
//					Vec3.norm(n3);
//					var l =Math.asin(n3[2]);
//					if(l<joint.limit_ang_lower[j]){
//						//本来の位置を計算
//						var dr = joint.limit_ang_lower[j]-l;
//						Mat43.getRotMat(rotmat,dr,bM[j],bM[j*4+1],bM[j*4+2]);
//						//Mat44.dot(bM,rotmat,bM);
//						Vec3.set(n3,bM[12],bM[13],bM[14]);
//						Mat44.dotMat44Vec3(n3,rotmat,n3);
//						Mat44.dotMat44Vec3(n,phyObj1.matrix,n3);
//						Vec3.sub(n,phyObj2.location,n);
//						Vec3.add(n,n,phyObj1.location);
//
//						Vec3.sub(n3,n,phyObj2.location);
//						
//						var hI=this._registHitInfo(n3,phyObj1,n,phyObj2,phyObj2.location);
//					//	hI.counter=2;
//					}
//				}
//			}
			...
			Mat44.dotMat44Vec3(n2,object1.matrix,n3);
			Vec3.sub(n3,n2,n);
			var hI=this._registHitInfo(n3,joint.parent,n,joint.child,n2);
