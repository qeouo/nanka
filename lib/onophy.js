...
//			this.impulse[2]+=Vec3.dot(this.coeff[2],dv); //必要摩擦力更新
	...
	var LinConstraint = (function(){
		...
		ret.prototype.calcConstraintPre=function(){
			...
			var l=Vec3.scalar(this.offset);
			//if(l!=0){
//				Vec3.mul(this.offset,this.offset,);
			//}
		}
		...
		ret.prototype.calcConstraint=function(){
			...
			//for(var i=0;i<3;i++){
			//	impulse[i]+=this.coeffM[i*3+i]*this.offset[i];
			//}
			...
			for(var i=0;i<DIMENSION;i++){
				...
				if(this.motorMax!==0 && i===0){
					//モーター最大力積を超えないようにする
					if(this.impulse[i] > this.motorMax){
					...
					//逆方向の力は加えない
				//	if(!this.lim[i]){
						this.impulse[i]= MAX(this.impulse[i],0); //撃力が逆になった場合は無しにする
				//	}
				}
			}
	...
	var Joint = (function(){
		...
		ret.prototype.setConstraint=function(){
			...
			Vec3.set(obj2p,bM2[9],bM2[10],bM2[11]);
			...
			for(var i=0;i<3;i++){
				...
				var l = 1/Math.sqrt(bM2[j]*bM2[j]+bM2[j+1]*bM2[j+1]+bM2[j+2]*bM2[j+2]);
				obj2m[i*3]=bM2[j]*l;
				obj2m[i*3+1]=bM2[j+1]*l;
				obj2m[i*3+2]=bM2[j+2]*l;
			}
			...
			for(var ii=0;ii<DIMENSION;ii++){
				...
				if(this.use_limit_ang[i]){
					...
					if(dr< this.limit_ang_lower[i]){
						dr=  this.limit_ang_lower[i] - dr ;
						Vec3.mul(axis,axis,-1);
					}else if(dr > this.limit_ang_upper[i]){
