...
var OnoPhy = (function(){
	...
	var impact = function(obj,p,force){
		Vec3.sub(a_l,p,obj.location); //重心からの距離
		
		//加速
		Vec3.add(obj.a,obj.a,force);
		//角加速
		Vec3.cross(a_a,a_l,force);
	...
	var act = function(obj,p,force,obj2){
		...
		Vec3.muladd(obj.aa,obj.aa,a_n,Vec3.dot(a_n,mem2)*obj.adamper);
		//Vec3.muladd(obj.aa,obj.aa,mem2,-obj.adamper*10);
		...
		if(f>friction){
			Vec3.mul(a_fric,a_fric,friction/f);//必要摩擦力に満たない場合
			f=friction;
		}
		friction-=f;
		...
		if(Math.abs(Vec3.dot(a_n,a_a))<3){
			Vec3.muladd(a_a,a_a,a_n,-Vec3.dot(a_n,a_a));
		}
		Vec3.add(obj.aa,obj.aa,a_a);
		...
		if(friction>0){
			Vec3.muladd(mem,obj.v,obj.a,_dt/obj.mass);
			Vec3.muladd(mem2,obj.av,obj.aa,_dt/obj.imoment);
			var l=Vec3.scalar(mem2);
			if(l>0.){
				var d=1/l;
				Mat43.getRotMat(bM2,l*_dt,mem2[0]*d,mem2[1]*d,mem2[2]*d);
				Mat43.dotMat43Vec3(mem2,bM2,a_l);
				Vec3.sub(mem2,mem2,a_l);
			}else{
				Vec3.set(mem2,0,0,0);
			}	
			Vec3.mul(mem,mem,_dt);
			Vec3.add(a_a,mem,mem2);
			Vec3.muladd(a_a,a_a,a_n,-Vec3.dot(a_a,a_n));
				
			Vec3.mul(a_fric,a_a,-1/_dt*obj.mass/_dt);
			Vec3.add(obj.a,obj.a,a_fric);
...
//		act(obj2,act2p,act2f,obj);
	...
	var calcEffic = function(n, hitInfo) {
		...
		Vec3.mul(calcBuf2,calcBuf,obj1.inv_moment);

		Vec3.cross(calcBuf, hitInfo.pos2,n);
		Vec3.cross(calcBuf,calcBuf,hitInfo.pos2);
		Vec3.mul(calcBuf,calcBuf,obj2.inv_moment);
	...
	var calcDiffVelocity = function(dv,hitInfo){
		...
		Vec3.cross(calcBuf,obj1.av,hitInfo.pos1);
		Vec3.sub(dv,dv,calcBuf);
		Vec3.cross(calcBuf,obj2.av,hitInfo.pos2);
	...
	var addimpulse = function(hitInfo,impulse){
		...
		if(obj1.inv_mass > 0 && !obj1.fix){
			Vec3.muladd(obj1.v,obj1.v,impulse, obj1.inv_mass);
			Vec3.cross(addimpulseBuf,hitInfo.pos1,impulse);
			Vec3.muladd(obj1.av,obj1.av,addimpulseBuf,obj1.inv_moment);
		}
		if(obj2.inv_mass > 0 && !obj2.fix){
			Vec3.muladd(obj2.v,obj2.v,impulse, -obj2.inv_mass);
			Vec3.cross(addimpulseBuf,hitInfo.pos2,impulse);
			Vec3.muladd(obj2.av,obj2.av,addimpulseBuf,-obj2.inv_moment);
		}
	}
	var calc_t=ret.calc_t=function(phyObjs,dt,ret){
		...
		for (repetition = 0; repetition < REPETITION_MAX; repetition++) {
			...
			for(i = phyObjs.length;i--;){
				...
				Vec3.sub(dv,o.oldav,o.av);
				sum+=(dv[0]*dv[0]+dv[1]*dv[1]+dv[2]*dv[2])*o.imoment*o.imoment;
			}
			if ( sum<= 0.0000001 ) {
				break;
			}
...
//			Vec3.sub(n2Vec,p1,p0);
	...
	ret.setPhyObjData = function(phyobj){
		...
		switch(type){
		case SPHERE:
			phyobj.imoment=phyobj.mass*2/5*(sx*sx+sy*sy+sz*sz)/3;
			phyobj.adamper=0.01*phyobj.imoment;
			break;
		default:
			phyobj.imoment=phyobj.mass*2/5*(sx*sx+sy*sy+sz*sz)/3;
			phyobj.adamper=0.01*phyobj.imoment;
		...
		phyobj.inv_moment = 1.0/phyobj.imoment;
	}
