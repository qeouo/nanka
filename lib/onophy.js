...
//		Vec3.muladd(obj.aa,obj.aa,a_a,l2*-obj.adamper);
		...
		Vec3.nrm(a_a,a_n);
		l2=Vec3.dot(a_a,obj.v);
		Vec3.muladd(a_a,obj.v,a_a,-l2);
		
		Vec3.cross(a_a2,obj.av,a_l);
		Vec3.add(a_a,a_a,a_a2);
		var friction;
		if(a_a[0]*a_a[0]+a_a[1]*a_a[1]+a_a[2]*a_a[2]<0.005){
			friction = 2*obj.sfriction*obj2.sfriction/(obj.sfriction+obj2.sfriction);
			if(obj.type==SPHERE){
//				console.log("HOGE",Vec3.scalar(a_ma));
			}
			friction = friction * n;
			friction = 0;
		}else{
...
//				console.log(Vec3.scalar(a_ma));
		...
		Vec3.norm(a_a);
		Vec3.mul(a_fric,a_a,-friction);//摩擦力

		Vec3.cross(a_a2,obj.av,a_l);
		Vec3.muladd(a_fric,a_fric,a_a2,1);//摩擦力+
