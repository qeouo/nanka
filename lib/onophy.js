...
var OnoPhy = (function(){
	...
	var HitInfo = (function(){
		...
		ret.prototype.calcPre = function(){
			...
			Vec3.add(impulse, this.impulse,  this.tImpulse );
		...
		ret.prototype.calcConstraint=function(){
			...
			//従法線方向(摩擦力)
			Vec3.copy(old,this.tImpulse); //前の値
			this.calcDiffVelocity(dv);
			var max = Vec3.dot(this.impulse,this.nVec)* this.fricCoe; //法線撃力から摩擦最大量を算出
			if(dv[0]*dv[0]+dv[1]*dv[1]+dv[2]*dv[2]>0.0001){
				max*=0.9; //静止していない場合はちょっと減らす
			}
			Mat33.dotVec(impulse,this.tM,dv); //今回の必要摩擦
			Vec3.add(this.tImpulse,this.tImpulse,impulse); //総摩擦力
			
			var l =Vec3.scalar(this.tImpulse);
			if (l > max) { //摩擦力が最大量を上回る場合は最大量でセーブ
				Vec3.mul(this.tImpulse,this.tImpulse,max/l);
			}
			Vec3.sub(impulse, this.tImpulse,old); 
