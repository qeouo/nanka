...
var OnoPhy = (function(){
	...
	var MESH_SPHERE=function(mesh,sphere){
		...
		Vec3.muladd(sphere.a,sphere.a,bV2,200);
	...
	var CUBOID_SPHERE=function(cuboid,sphere){
		...
		//console.log(cuboid.matrix,cuboid.type,sphere.type);
		var flg=0;
		if(bV0[0]*bV0[0] > bV0[1]*bV0[1]){
			if(bV0[0]*bV0[0] > bV0[2]*bV0[2]){
				flg=0;
			}else{
				if(bV0[1]*bV0[1] > bV0[2]*bV0[2]){
					flg=1;
				}else{
					flg=2;
				}
		...
		&& bV0[2]*bV0[2] < 4){
			...
			Vec3.muladd(sphere.a,sphere.a,bV1,20);
		}
	}
...
//	for(var i=0;i<8;i++){
//		for(var j=i+1;j<8;j++){
//			if(hantei[i*8+j]!=null){
//				var func=hantei[i*8+j];
//				hantei[j*8+i] = function(a,b,c){ func(b,a,c); };
//
//			}else if(hantei[j*8+i]!=null){
//				var func=hantei[j*8+i];
//				hantei[i*8+j] = function(a,b,c){ func(b,a,c); };
//
//			}
//		}
//	}
