...
var OnoPhy = (function(){
	...
	var HitInfo = (function(){
		...
		ret.prototype.calcPre = function(){
			...
			//Vec3.add(impulse, impulse,  this.tImpulse );
		...
		ret.prototype.calcConstraint=function(){
			...
			Vec3.copy(old,hitInfo.impulse);
			
			Mat33.dotVec(impulse,hitInfo.nM,dv);
			hitInfo.nImpulse += Vec3.dot(dv, hitInfo.nVec) * hitInfo.nEffic 
			//	+ hitInfo.repulsion; //速度差から新しい撃力を算出
			Vec3.add(hitInfo.impulse,hitInfo.impulse,impulse);
			if (hitInfo.nImpulse < 0) { //撃力が逆になった場合は0にする
				hitInfo.nImpulse = 0;
			}
			var a = Vec3.dot(hitInfo.impulse,hitInfo.nVec);
			if(a<0){
				Vec3.muladd(hitInfo.impulse,hitInfo.impulse,hitInfo.nVec,-a);
			}
			//Vec3.mul(impulse, hitInfo.nVec, (hitInfo.nImpulse-old));
			Vec3.sub(impulse, hitInfo.impulse, old);
			...
			var max = Vec3.scalar(hitInfo.impulse)* hitInfo.fricCoe; //法線撃力から摩擦最大量を算出
	...
	//		var hitInfo=null;
			...
			if(!flg){
				//if(Vec3.len2(pos1,pos2)!==0){
					...
					hitInfo.nImpulse *= Vec3.dot(hitInfo.nVec,vec3);
					Vec3.set(hitInfo.impulse,0,0,0);
					Vec3.set(hitInfo.tImpulse,0,0,0);
			//	}
			}
