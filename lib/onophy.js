		...
		,calc:function(dt){
			...
			for(i = phyObjs.length;i--;){
				...
				if(obj.type==SPRING_MESH){
					...
					CLOTH_TENSION=obj.friction*dt*5;
					...
					for(j=positions.length;j--;){
						...
						if(fixes[j]){
							...
							Vec3.mult(bV3,bV3,CLOTH_TENSION*obj.goal_default);
					...
					for(j=edges.length;j--;){
						...
						y/=len;
						z/=len;
						len= -(Vec3.len(vertices[p0],vertices[p1])-len)/Vec3.len(vertices[p0],vertices[p1]) *CLOTH_TENSION
