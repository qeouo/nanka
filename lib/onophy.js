...
var OnoPhy = (function(){
	...
	var HitInfo = (function(){
		...
		ret.prototype.calcPre = function(){
			...
			this.axes[1][0] = this.axes[0][2];
			this.axes[1][1] = this.axes[0][0];
			this.axes[1][2] = -this.axes[0][1];
			this.axes[2][0] = -this.axes[0][1];
			this.axes[2][1] = this.axes[0][2];
			this.axes[2][2] = this.axes[0][0];
	...
	ret.prototype.readjustHitInfo=function(target){
		...
		for(var i=hitInfos.length;i--;){
			...
			if(Vec3.dot(hitInfo.axes[0],target.axes[0])<0){
				//めり込み方向が反対の場合は削除
				this.removeHitInfo(i);
				continue;
			}
	...
	ret.prototype.registHitInfo = (function(){
		return function(obj1,pos1,obj2,pos2,hitInfo){
			...
			Vec3.sub(hitInfo.axes[0],pos2,pos1);
			Vec3.norm(hitInfo.axes[0]);
