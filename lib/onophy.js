...
var OnoPhy = (function(){
	...
	var PhyObj = (function(){
		...
		ret.prototype.refresh = function(){
			...
			case Collider.CONE:
				var r2 = sx*sx+sy*sy;
				var xy=3/80*this.mass*(4.0*r2 + 4*sz*sz);
				var z=3/10*this.mass*r2;
				Vec3.set(this.inertia,xy,xy,z);
	...
	ret.prototype.calc=function(dt){
		...
		for(i = phyObjs.length;i--;){
			...
			if(l>0){
				...
				//オブジェクト中心と重心のズレ補正
				Mat33.dotVec3(bV0,bM,obj.offset2);
				Vec3.sub(bV0,obj.offset2,bV0);
				Vec3.add(obj.location,obj.location,bV0);
			}
