...
var OnoPhy = (function(){
	...
	var Spring =  function(){
		...
		this._p1 = new Vec3();
		this.con1Obj = null;
		this.con1Pos = new Vec3();
		this.con2Obj = null;
		this.con2Pos = new Vec3();
	}
...
//			}
			...
			Vec3.mul(bV0,bV3,(Vec3.dot(bV1,bV3))*0.5*0.01);
	...
	var springmeshMove2=function(mesh,dt){
		...
		//		Vec3.muladd(positions[j],positions[j],bV3,goal);
				Vec3.mul(bV3,bV3,mesh.plastic*wei);
				Vec3.muladd(bV3,bV3,bV2,-Vec3.dot(bV2,velocity)*0.1);

				Vec3.sub(bV3,bV3,accs[j]);
				Vec3.muladd(accs[j],accs[j],bV3,goal);

				Vec3.muladd(velocity,velocity,accs[j],1.0/mesh.mass);
	...
	ret.setPhyObjData = function(phyobj){
		...
		default:
			phyobj.imoment=phyobj.mass*2/5*Vec3.dot(phyobj.scale,phyobj.scale)/3;
			phyobj.adamper=1*phyobj.imoment;
			break;
		}
