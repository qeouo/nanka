...
var OnoPhy = (function(){
	...
	var MESH_CUBOID=function(mesh,cuboid,dt){
		...
		for(var j = faces.length;j--;){
			...
			Vec3.nrm(bV3,bV4);
			Vec3.mul(bV3,bV3,-1);
			Vec3.mul(bV6,bV3,Vec3.dot(bV2,bV3));
			Vec3.sub(bV4,bV2,bV6);
			Vec3.copy(bV2,bV6);

			Vec3.cross(bV4,bV3,bV4);
			l=Vec3.scalar(bV4);
			Vec3.muladd(cuboid.moment,cuboid.moment,bV4,-10*l);

			Vec3.nrm(bV1,bV2);


			//ダンパ
			l2=Vec3.dot(bV1,cuboid.v);
			l = Vec3.dot(bV5,bV1);
			if(l2 * l>0){
				if(l2*l2>l*l){
					Vec3.muladd(bV5,bV5,bV1,l2-l);
				}	
			}else{
				Vec3.muladd(bV5,bV5,bV1,l2);
			}
			//Vec3.muladd(cuboid.a,cuboid.a,bV1,-l*DAMPER);
			
			//摩擦
			var n=Vec3.scalar(bV2);
			
			Vec3.muladd(bV3,cuboid.v,bV1,-l);
			if(bV3[0]*bV3[0]+bV3[1]*bV3[1]+bV3[2]*bV3[2]<0.0000001){
			}else{
				
				Vec3.norm(bV3);

				Vec3.muladd(cuboid.a,cuboid.a,bV3,-n*cuboid.dfriction);
				
			}	

			Vec3.muladd(cuboid.a,cuboid.a,bV2,PENALTY);
			Mat43.dotMat33Vec3(bV2,mesh.imatrix,bV2);
			bM[12]+=bV2[0]*1.01
			bM[13]+=bV2[1]*1.01
			bM[14]+=bV2[2]*1.01
		...
		for(var j = edges.length;j--;){
			...
			bV1[1]*=bV4[1];
			bV1[2]*=bV4[2];
			//Vec3.muladd(bV0,bV0,bV1,tin);
			//Vec3.mul(bV1,bV1,tout-tin);
			...
			Vec3.mul(bV3,bV3,-1);
			Vec3.mul(bV6,bV3,Vec3.dot(bV2,bV3));
			Vec3.sub(bV7,bV2,bV6);
			Vec3.copy(bV2,bV7);

			Vec3.cross(bV7,bV3,bV7);
			Vec3.muladd(cuboid.moment,cuboid.moment,bV7,-10*l);
			
			Vec3.nrm(bV1,bV2);

			//ダンパ
			//l=Vec3.dot(bV1,cuboid.v);
			//Vec3.muladd(cuboid.a,cuboid.a,bV1,-l*DAMPER);
			l2=Vec3.dot(bV1,cuboid.v);
			l = Vec3.dot(bV5,bV1);
			if(l2 * l>0){
		...
		for(var j = vertices.length;j--;){
			...
			Mat43.dotMat33Vec3(bV2,cuboid.matrix,bV1);
	...
	var calc_t=function(phyObjs,dt){
		...
		for(i = phyObjs.length;i--;){
			...
			var l=Vec3.scalar(obj.moment);
			if(l!=0){
				var d=1/l;
				l*=dt;
				Mat43.getRotMat(bM,l,obj.moment[0]*d,obj.moment[1]*d,obj.moment[2]*d);
				Mat43.dot(obj.rotmat,bM,obj.rotmat);
				Vec3.mul(obj.moment,obj.moment,0.95);
			}
