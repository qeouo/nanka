...
1
	...
	var CUBOID_SPHERE=function(cuboid,sphere){
		...
		registHitInfo(sphere,pos,nVec,cuboid);
	...
	var SPHERE_SPHERE=function(sphereA,sphereB){
		...
		registHitInfo(sphereA,pos,nVec,sphereB);
	...
	var LINE_LINE_result={};
	var LINE_LINE=function(p1,p2,p3,p4){
		var dir1 = new Vec3();
		var dir2 = new Vec3();
		var cross = new Vec3();
		var cross2 = new Vec3();
		...
		Vec3.cross(cross2,dir2,cross); //線分2に垂直かつ線分1に並行?なベクトル
		l = Vec3.dot(cross2,dir1); 
		Vec3.sub(dpos,p3,p1); 
		LINE_LINE_result.r1 = Vec3.dot(cross2,dpos)/l; //線分1の交差比率

		Vec3.cross(cross2,dir1,cross); //線分1に垂直かつ線分2に並行?なベクトル
		l = Vec3.dot(cross2,dir2); 
		Vec3.sub(dpos,p1,p3); 
		LINE_LINE_result.r2= Vec3.dot(cross2,dpos)/l; //線分2の交差比率
	}
	...
	var MESH_CUBOID=function(mesh,cuboid,dt){
		...
		for(var i=0;i<8;i++){
			Vec3.set(pos,((i&1)<<1) - 1,(i&2) - 1 , ((i&4)>>1) -1);
			Vec3.copy(dVec,pos);
			...
			for(var j = faces.length;j--;){
				var face = faces[j];
				var n = faceNormals[j]; //ポリ法線

				Vec3.copy(p0,poses[face.idx[0]]);
				dVec[0]= bM[12] - p0[0];
				dVec[1]= bM[13] - p0[1];
				dVec[2]= bM[14] - p0[2]; //板ポリと箱の位置差

				var l=Vec3.dot(dVec,n); //法線距離
				if(l<0){
					continue; //板ポリの内側の場合はスルー
				}

				Vec3.sub(dVec,pos,p0);
				l=Vec3.dot(dVec,edge1Normals[j]);
				if( l<0){
					continue; //ポリ辺1の外
				}
			...
			var face = faces[minn];
			var n = faceNormals[minn]; //ポリ法線

			Vec3.muladd(dVec,pos,n,min);

			registHitInfo2(cuboid,pos,mesh,dVec);
			

		}

		for(var j = edges.length;j--;){
			var e1 = poses[edges[j].v0];
			var e2 = poses[edges[j].v1];
			var rotmat = cuboid.rotmat;
			var direction = new Vec3();
			var cross = new Vec3();

			//線分の矩形貫通判定
			CUBOID_LINE2(cuboid.matrix,e1,e2);
			if(CUBOID_LINE2_result.inr > CUBOID_LINE2_result.outr
				|| CUBOID_LINE2_result.inr>1
				|| CUBOID_LINE2_result.outr< 0
			){
				continue;
			}

			Vec3.sub(direction,e2,e1);
			Vec3.norm(direction); //線分の向き単位ベクトル
			Vec3.sub(dVec,cuboid.location,e1); //線分から矩形中心までのベクトル
			var min=9999;
			var minn=-1;
			//矩形xyz辺で一番めり込み距離が短いのを探す
			var mat = cuboid.matrix;
			for(var i=0;i<3;i++){
				Vec3.set(r,rotmat[i*3+0] ,rotmat[i*3+1] ,rotmat[i*3+2]);
				Vec3.cross(cross,r,direction);
				Vec3.norm(cross); //線分とxyz軸に垂直なベクトル
				var x = cross[0];
				var y = cross[1];
				var z = cross[2];
				//crossに対する矩形のサイズ
				l = Math.abs(mat[0]*x + mat[1]*y + mat[2]*z)
					+ Math.abs(mat[4]*x + mat[5]*y + mat[6]*z)
					+ Math.abs(mat[8]*x + mat[9]*y + mat[10]*z);
				var l2 = Math.abs(Vec3.dot(dVec,cross)); //crossに対する線分と矩形の間隔
				l = l-l2; //めり込み距離
				
				if(l<0){
					continue;//事前に貫通判定をしているのでありえないはずだが一応
				}
				if(l<min || minn<0){
					//よりめり込みが浅い場合更新
					min=l;
			...
			Vec3.set(r,rotmat[minn*3+0] ,rotmat[minn*3+1] ,rotmat[minn*3+2]);
			Vec3.cross(cross,r,direction);
			if(Vec3.dot(dVec,cross)>0){
				Vec3.mul(cross,cross,-1);
			}

			for(var i=0;i<3;i++){
				Vec3.set(r,rotmat[i*3+0] ,rotmat[i*3+1] ,rotmat[i*3+2]);
				if(Vec3.dot(r,cross)>0){
					p1[i]=1;
					p2[i]=1;
				}else{
					p1[i]=-1;
			...
			Mat43.dotMat43Vec3(p1,cuboid.matrix,p1);
			Mat43.dotMat43Vec3(p2,cuboid.matrix,p2);
			
			LINE_LINE(p1,p2,e1,e2);
			if(LINE_LINE_result.r1<0
			|| LINE_LINE_result.r1>1
			|| LINE_LINE_result.r2<0
			|| LINE_LINE_result.r2>1
			){
				continue; //二辺の交差点が線上にない場合は判定なし
			}
			Vec3.sub(dVec,p2,p1);
			Vec3.muladd(p1,p1,dVec,LINE_LINE_result.r1);
			Vec3.sub(dVec,e2,e1);
			Vec3.muladd(p2,e1,dVec,LINE_LINE_result.r2);

			registHitInfo2(cuboid,p1,mesh,p2);
		...
		for(var j = vertices.length;j--;){
			...
			Vec3.sub(pos,p0,n);
			registHitInfo(cuboid,pos,n,mesh);
			//bM[12]+=n[0]*(1+DELTA);
			//bM[13]+=n[1]*(1+DELTA);
			//bM[14]+=n[2]*(1+DELTA);
		}	
...
//			Vec3.sub(n2Vec,p1,p0);
	...
	var CUBOID_CAPSULE=function(cuboid,capsule,dt){
		...
		if(minn<3){
			...
			registHitInfo(capsule,posb,nVec,cuboid);
		}else{
			var i =minn-3;
			...
			registHitInfo(capsule,posa,nVec,cuboid);
		}
