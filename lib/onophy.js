...
var OnoPhy = (function(){
	...
	var RigidBody = (function(){
		...
		ret.prototype.calcPre = function(){
			...
			Vec4.copy(collision.rotq,this.rotq);
			...
			case Collider.CAPSULE:
				bold = Math.max(this.collisionSize[0],this.collisionSize[1]);
			...
			Vec3.set(collision.scale,sx,sy,sz);
			collision.bold = bold;

			//重心位置
			Vec4.rotVec3(this.COG,this.rotq,this.COGBase);
		...
		ret.prototype.update=function(dt){
			...
			if(l>0){
				var d=1/l;
				Vec4.getRotQuat(rq,l*dt,this.rotV[0]*d,this.rotV[1]*d,this.rotV[2]*d);
				Vec4.qdot(this.rotq,rq,this.rotq);
				
				//オブジェクト中心と重心のズレ補正
				Vec3.sub(bV0,this.COG,this.location);
		...
		ret.prototype.refresh = function(){
			...
			case Collider.CONVEX:
				...
				calcTranslateInertia(I,this.COGBase);
				Mat33.mul(I,I,-1);
				Mat33.add(this.inertiaTensorBase,this.inertiaTensorBase,I);
			...
			for(var i=0;i<DIMENSION;i++){
				...
				if(this.use_limit_ang[i]){
					...
					|| dr > this.limit_ang_upper[i]){
						...
						Vec4.getRotQuat(quat,-dr,axis[0],axis[1],axis[2]);
	...
	var Cloth = ret.Cloth = (function(){
		...
		ret.prototype.calcPre=function(onophy){
			...
			for(var i=0;i<list.length;i++){
				...
				for(var j=0;j<this.faces.length;j++){
					...
					if(face.idx===4){
						triangle.v[0]=face.points[0].location;
						triangle.v[1]=face.points[2].location;
						triangle.v[2]=face.points[3].location;
						triangle.AABB = face.AABB;
		...
		ret.prototype.calcCollision=function(target,onophy){
			...
			for(var i=0;i<this.faces.length;i++){
				...
				for(var j=0;j<target.faces.length;j++){
					var face2= target.faces[j]
					if(!AABB.hitCheck(face.AABB,face2.AABB)){
						continue;
					}
					triangle2.v[0]=face2.points[0].location;
					triangle2.v[1]=face2.points[1].location;
					triangle2.v[2]=face2.points[2].location;
					triangle2.AABB = face2.AABB;

					var l = Collider.calcClosest(ans1,ans2,triangle,triangle2);
					if(l<0){
						var phyFace = this.getPhyFace(face.points[0],face.points[1],face.points[2]
							,face,ans2);
						var phyFace2 = this.getPhyFace(face2.points[0],face2.points[1],face2.points[2]
							,face2,ans2);
						onophy.registHitInfo(phyFace,ans1,phyFace2,ans2);
					}

					if(face2.idxnum===4){
						triangle2.v[0]=face2.points[0].location;
						triangle2.v[1]=face2.points[2].location;
						triangle2.v[2]=face2.points[3].location;
						triangle2.AABB = face2.AABB;
						...
						if(l<0){
							var phyFace = this.getPhyFace(face.points[0],face.points[1],face.points[2]
								,face,ans2);
							var phyFace2 = this.getPhyFace(face2.points[0],face2.points[2],face2.points[3]
								,face,ans2);
							onophy.registHitInfo(phyFace,ans1,phyFace2,ans2);
						}
					}
	...
	var SoftBody = ret.SoftBody = (function(){
		...
		var  Edge = ret.Edge = (function(){
			...
			ret.prototype.calcPre=function(flg){
				...
				//l= ( l - this.len);
				Vec4.rotVec3(dv,this.point1.rotq,this.sabun);
				Vec3.add(dv,this.point1.location,dv);
				...
				Vec4.slerp(rotq,this.point2.rotq,this.point1.rotq,0.5);
				Vec4.rotVec3(dv,rotq,this.sabun);
				Vec3.sub(impulse,this.point2.location,this.point1.location);
				Vec3.norm(dv);
				Vec3.norm(impulse);
				var a=0;
				if(Vec3.dot(impulse,dv)<0){
					a=1;
				}
				Vec3.cross(impulse,dv,impulse);
				var s = Math.asin(Vec3.scalar(impulse));
				if(a){
					if(s<0){
						s=-(Math.PI+s);
					}else{
				...
				Vec3.norm(impulse);
				Vec3.mul(impulse,impulse,s);
				Vec3.mul(impulse,impulse,_dt*this.cloth.inv_pointMass*2);
				Vec3.add(this.point1.rotV,this.point1.rotV,impulse);
				Vec3.add(this.point2.rotV,this.point2.rotV,impulse);

				Vec4.qmdot(rotq,this.point2.rotq,this.point1.rotq,-1);
				Vec4.toTorque(impulse,rotq);
				Vec3.mul(impulse,impulse,_dt*this.cloth.inv_pointMass);
				Vec3.add(this.point1.rotV,this.point1.rotV,impulse);
	...
	ret.prototype.calc=function(dt){
		...
		for (repetition = 0; repetition < REPETITION_MAX; repetition++) {
			...
			if ( sum<= 0.000001*impnum && repetition>5) {
				break;
			}
		}
