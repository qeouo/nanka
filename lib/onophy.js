...
var OnoPhy = (function(){
	...
	var act = function(obj,p,f){
		...
		Vec3.nrm(a_fn,p);
		Vec3.mul(a_ma,a_fn,Vec3.dot(a_fn,f));
		Vec3.sub(a_ma,f,a_ma);
		Vec3.cross(a_fn,a_fn,a_ma);
		l=Vec3.scalar(a_ma)*len;
		Vec3.muladd(obj.moment,obj.moment,a_fn,l*0.003);


		Vec3.nrm(a_fn,f);
		//ダンパ
		var l2=Vec3.dot(a_fn,obj.v);
		var l = Vec3.dot(bV5,a_fn);
		if(l2 * l>0){
			if(l2*l2>l*l){
...
//			Vec3.muladd(cuboid.moment,cuboid.moment,bV7,-10*l);
		...
		var damper = (1-Math.pow(1-cuboid.damper*(1/30),(dt)/(1/30)))/(dt)
