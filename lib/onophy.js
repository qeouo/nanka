...
var OnoPhy = (function(){
	...
	var act = function(obj,p,f,obj2){
		...
		//ダンパ
		Vec3.nrm(a_a,a_n);
		l2=Vec3.dot(a_a,obj.v);
		Vec3.muladd(obj.a,obj.a,a_a,l2*-obj.damper);
		//角ダンパ
		Vec3.cross(a_a,a_l,a_n);
		Vec3.nrm(a_a,a_a);
		...
		Vec3.cross(a_a2,obj.av,a_l);
		Vec3.mul(a_a2,a_a2,0);
		Vec3.add(a_a2,a_a2,obj.v);

		Vec3.nrm(a_a,a_n);
		l2=Vec3.dot(a_a,a_a2);
		Vec3.muladd(a_a,a_a2,a_a,-l2);

		var ll=Vec3.scalar(a_a);
		var stop=false;
		var friction;
		if(ll<0.1 ){
			var stop=true;
	...
	//角速度
		Vec3.cross(a_a2,obj.av,a_l);
		Vec3.mul(a_a2,a_a2,1);
		Vec3.muladd(a_a2,a_a2,obj.v,1);
		Vec3.nrm(a_a,a_n);
		l2=Vec3.dot(a_a,a_a2);
		Vec3.muladd(a_a,a_a2,a_a,-l2);
		f= Vec3.scalar(a_a);
		...
		if(obj.name=="tes"){
			var aaa=new Vec3();
			Vec3.nrm(aaa,a_n);
			Vec3.muladd(aaa,obj.v,aaa,-Vec3.dot(aaa,obj.v));
			f= Vec3.scalar(aaa)*60;
...
//		f=friction;
		...
		if(Vec3.scalar(a_a)>0){
			Vec3.norm(a_a);
			Vec3.mul(a_fric,a_a,-f);//摩擦力
			...
			Vec3.nrm(a_a2,a_n);
			Vec3.muladd(a_a,a_a,a_a2,-Vec3.dot(a_a2,a_a));
			Vec3.add(obj.aa,obj.aa,a_a);
		}else{
		}
		...
		if(obj.name=="tes"){
			if(stop && 0){
				...
				var sfriction = 2*obj.sfriction*obj2.sfriction/(obj.sfriction+obj2.sfriction);
				sfriction = sfriction * n;
				friction = (sfriction-friction);

				Vec3.cross(a_a2,obj.a,a_l);
				Vec3.mul(a_a2,a_a2,0);
				Vec3.muladd(a_a2,a_a2,obj.v,1);
				Vec3.nrm(a_a,a_n);
				Vec3.mul(a_a2,a_a,Vec3.dot(a_a,a_a2));
				Vec3.sub(a_a2,a_a2,obj.v);
				//Vec3.add(obj.v,obj.v,a_a2);
				Vec3.muladd(obj.a,obj.a,a_a2,60);
				...
				Vec3.mul(a_a2,a_a,Vec3.dot(a_a,a_a2));
				Vec3.sub(a_a2,a_a2,mem2);
				Vec3.add(obj.a,obj.a,a_a2);
	//			Vec3.muladd(obj.a,obj.a,a_a,-Vec3.dot(a_a,a_a2));
