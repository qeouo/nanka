...
//			Vec3.mul(bV2,bV2,PENALTY);
	...
	var CUBOID_CUBOID=function(boxA,boxB,dt){
		var matA=boxA.matrix;
		var matB=boxB.matrix;
		bV0[0]= matB[12] - matA[12];
		bV0[1]= matB[13] - matA[13];
		bV0[2]= matB[14] - matA[14];

		bV1[0]=Math.sqrt(matA[0]*matA[0] + matA[1]*matA[1] + matA[2]*matA[2]);
		bV1[1]=Math.sqrt(matA[4]*matA[4] + matA[5]*matA[5] + matA[6]*matA[6]);
		bV1[2]=Math.sqrt(matA[8]*matA[8] + matA[9]*matA[9] + matA[10]*matA[10]);

		bV2[0]=Math.sqrt(matB[0]*matB[0] + matB[1]*matB[1] + matB[2]*matB[2]);
		bV2[1]=Math.sqrt(matB[4]*matB[4] + matB[5]*matB[5] + matB[6]*matB[6]);
		bV2[2]=Math.sqrt(matB[8]*matB[8] + matB[9]*matB[9] + matB[10]*matB[10]);

		var l = Math.abs((matB[0]+matB[4]+matB[8])*matA[0])
		   	+ Math.abs((matB[1]+matB[5]+matB[9])* matA[1])
		   	+ Math.abs((matB[2]+matB[6]+matB[10])*matA[2]);

		var l2 = bV0[0] * matA[0] + bV0[1]*matA[1] + bV0[2]*matA[2];

		if(l2 * l2 > (l+bV1[0])*(l+bV1[0])){
			return
		}
		l = Math.abs((matB[0]+matB[4]+matB[8])*matA[4])
		   	+ Math.abs((matB[1]+matB[5]+matB[9])* matA[5])
		   	+ Math.abs((matB[2]+matB[6]+matB[10])*matA[6]);

		l2 = bV0[0] * matA[4] + bV0[1]*matA[5] + bV0[2]*matA[6];
		if(l2 * l2 > (l+bV1[1])*(l+bV1[1])){
			return
		}

		console.log("A");

		l = Math.abs((matB[0]+matB[4]+matB[8])*matA[8])
		   	+ Math.abs((matB[1]+matB[5]+matB[9])* matA[9])
		   	+ Math.abs((matB[2]+matB[6]+matB[10])*matA[10]);

		var l2 = bV0[0] * matA[8] + bV0[1]*matA[9] + bV0[2]*matA[10];
		if(l2 * l2 > (l+bV1[2])*(l+bV1[2])){
			return
		}
			
		l = Math.abs((matA[0]+matA[4]+matA[8])*matB[0])
		   	+ Math.abs((matA[1]+matA[5]+matA[9])* matB[1])
		   	+ Math.abs((matA[2]+matA[6]+matA[10])*matB[2]);

		var l2 = bV0[0] * matB[0] + bV0[1]*matB[1] + bV0[2]*matB[2];

		if(l2 * l2 > (l+bV2[0])*(l+bV2[0])){
			return
		}
		l = Math.abs((matA[0]+matA[4]+matA[8])*matB[4])
		   	+ Math.abs((matA[1]+matA[5]+matA[9])* matB[5])
		   	+ Math.abs((matA[2]+matA[6]+matA[10])*matB[6]);

		var l2 = bV0[0] * matB[4] + bV0[1]*matB[5] + bV0[2]*matB[6];
		if(l2 * l2 > (l+bV2[1])*(l+bV2[1])){
			return
		}


		l = Math.abs((matA[0]+matA[4]+matA[8])*matB[8])
		   	+ Math.abs((matA[1]+matA[5]+matA[9])* matB[9])
		   	+ Math.abs((matA[2]+matA[6]+matA[10])*matB[10]);

		var l2 = bV0[0] * matB[8] + bV0[1]*matB[9] + bV0[2]*matB[10];
		if(l2 * l2 > (l+bV2[2])*(l+bV2[2])){
			return
		}
