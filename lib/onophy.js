...
var OnoPhy = (function(){
	...
	var HitInfo = (function(){
		...
		ret.prototype.calcConstraint=function(){
			...
			Vec3.mul(this.impulse,this.impulse,0.999);
			...
			var maxr = max * 0.1; //法線撃力から最大転がり抵抗を算出
			...
			Mat33.dotVec(impulse,this.rM,dv);
			...
			Vec3.mul(this.impulseR,this.impulseR,0.999);
	...
	ret.prototype.readjustHitInfo=function(target){
		...
		//同一の組み合わせが5以上の場合は古いやつから消して4以下にする
		var count=0;
		for(var i=hitInfos.length;i--;){
			hitInfo=hitInfos[i];
			if(hitInfo.obj1!== obj1|| hitInfo.obj2!==obj2){
