...
//			Vec3.mul(bV2,bV2,PENALTY);
	...
	var cuboidsub = function(a,b){
		var v0 = a[0]
		,v1 = a[1]
		,v2 = a[2];
		return  Math.abs(b[0] * v0 + b[1]*v1 + b[2]*v2)
		   	+ Math.abs(b[4] * v0 + b[5]*v1 + b[6]*v2)
		   	+ Math.abs(b[8] * v0 + b[9]*v1 + b[10]*v2)
	}
	var cuboidsub2 = function(a,b,c){
		var v0 = a[0]
		,v1 = a[1]
		,v2 = a[2];
		return  Math.abs(b[0] * v0 + b[1]*v1 + b[2]*v2)
		   	+ Math.abs(b[4] * v0 + b[5]*v1 + b[6]*v2)
		   	+ Math.abs(b[8] * v0 + b[9]*v1 + b[10]*v2)
		    + Math.abs(c[0] * v0 + c[1]*v1 + c[2]*v2)
		   	+ Math.abs(c[4] * v0 + c[5]*v1 + c[6]*v2)
		   	+ Math.abs(c[8] * v0 + c[9]*v1 + c[10]*v2)
	}
	var CUBOID_CUBOID=function(boxA,boxB,dt){
		...
		Vec3.set(bV5,0,0,0);
		for(var i=0;i<3;i++){
			Vec3.set(bV2,rotA[i*4+0],rotA[i*4+1],rotA[i*4+2]);
			l = cuboidsub(bV2,matB)
				+ matA[i*4+0]*bV2[0] + matA[i*4+1]*bV2[1]+ matA[i*4+2]*bV2[2];
			l2 = Math.abs(bV0[0] * bV2[0] + bV0[1]*bV2[1] + bV0[2]*bV2[2]);
			l = l - l2;
			if( l<0 ){
				return
			}
		...
		for(var i=0;i<3;i++){
			Vec3.set(bV2,rotB[i*4+0],rotB[i*4+1],rotB[i*4+2]);
			l = cuboidsub(bV2,matA)
				+ matB[i*4+0]*bV2[0] + matB[i*4+1]*bV2[1]+ matB[i*4+2]*bV2[2];
			l2 = Math.abs(bV0[0] * bV2[0] + bV0[1]*bV2[1] + bV0[2]*bV2[2]);
			l = l - l2;
			if( l<0 ){
				return
		...
		for(var i =0;i<3;i++){
			for(var j =0;j<3;j++){
				bV2[0] = rotA[4*i + 1]* rotB[4*j+2] - rotA[4*i+2]*rotB[4*j+1];
				bV2[1] = rotA[4*i + 2]* rotB[4*j+0] - rotA[4*i+0]*rotB[4*j+2];
				bV2[2] = rotA[4*i + 0]* rotB[4*j+1] - rotA[4*i+1]*rotB[4*j+0];
				if(Vec3.scalar(bV2)<=0){
					continue;
				}
				Vec3.norm(bV2);
				l = cuboidsub2(bV2,matA,matB);
				l2 = Math.abs(bV0[0] * bV2[0] + bV0[1]*bV2[1] + bV0[2]*bV2[2]);
				l = l - l2;
				if( l<0 ){
					return
		...
		case 2:
			i+=minn;
			Vec3.set(bV1,rotA[i*4+0],rotA[i*4+1],rotA[i*4+2]);
			if(Vec3.dot(bV1,bV0)<0){
				Vec3.mul(bV1,bV1,-1);
			}
			for(i=0;i<3;i++){
				l= matB[0+i*4]*bV1[0] + matB[1+i*4]*bV1[1] + matB[2+i*4]*bV1[2];
				if(l<0){
					bV2[i]=1;
				}else{
					bV2[i]=-1;
			...
			Mat43.dotMat43Vec3(bV2,matB,bV2);
			Vec3.sub(bV4,bV2,boxB.location);
			Vec3.mul(bV3,bV1,min);
			//Vec3.mul(bV4,bV4,0);
			act(boxB,bV4,bV3);

			Vec3.add(bV2,bV2,bV1);
			Vec3.sub(bV4,bV2,boxA.location);
			Vec3.mul(bV3,bV1,-min);
			//Vec3.mul(bV4,bV4,0);
			act(boxA,bV4,bV3);

			var damper = boxA.damper;
			
			Vec3.muladd(boxA.a,boxA.a,bV5,-damper);
		...
		default:
			...
			for(k=0;k<3;k++){
				l= matB[0+k*4]*bV1[0] + matB[1+k*4]*bV1[1] + matB[2+k*4]*bV1[2];
				if(l<0){
					bV2[i]=1;
				}else{
					bV2[i]=-1;
			...
			Mat43.dotMat43Vec3(bV4,matB,bV2);

			for(k=0;k<3;k++){
				l= matA[0+k*4]*bV1[0] + matA[1+k*4]*bV1[1] + matA[2+k*4]*bV1[2];
				if(l<0){
					bV3[i]=-1;
				}else{
					bV3[i]=1;
			...
			Mat43.dotMat43Vec3(bV5,matA,bV3);
			Vec3.sub(bV5,bV5,bV4);

			Vec3.cross(bV2,bV2,bV1);

			Vec3.muladd(bV2,bV4,bV3,Vec3.dot(bV2,bV5)/Vec3.dot(bV2,bV3);
			
			Vec3.sub(bV4,bV2,boxB.location);
			Vec3.mul(bV3,bV1,min);
			act(boxB,bV4,bV3);

			Vec3.muladd(bV2,bV2,bV1,min);
			Vec3.sub(bV4,bV2,boxA.location);
			Vec3.mul(bV3,bV1,-min);
			act(boxA,bV4,bV3);
