...
var OnoPhy = (function(){
	...
	ret.prototype.calc=function(dt){
		...
		for(var i=0;i<hitInfos.length;i++){
			...
			sphere.calcPre();
			var l1 = Collider.calcClosest(ans1,ans2,sphere,obj2.children[0]);

			Mat43.dotMat43Vec3(sphere.location,obj2.matrix,hitInfo.pos2ex);
			Vec3.sub(sphere.location,sphere.location,bV0);
			sphere.calcPre();
			var l2 = Collider.calcClosest(ans3,ans4,obj1.children[0],sphere);
			...
			if(l1>0 && l2>0){
				...
				if(l1<l2){
					...
					hi=this.registHitInfo(obj1,bV1,obj2,ans2,hitInfo);
				}else{
					Mat43.dotMat43Vec3(bV1,obj1.matrix,hitInfo.pos1ex);
					hi=this.registHitInfo(obj1,ans3,obj2,bV1,hitInfo);
				}
			//	hitInfos.splice(i,1);
			//	disableHitInfos.push(hitInfo);
			//	i--;
