...
var OnoPhy = (function(){
	...
	var REPETITION_MAX=100; //繰り返しソルバ最大回数
	var PENALTY =20; //押出し係数
	var _dt; //ステップ時間
	var dv = new Vec3();
	...
	ret.prototype.registHitInfo = (function(){
		var vec3=new Vec3();
		var vec32=new Vec3();
		return function(obj1,pos1,obj2,pos2){
			if(!obj1 || !obj2){
				return;
			}
			var hitInfo=null;
			...
			for(var i=0;i<hitInfos.length;i++){
				...
				counter++;
				if(idx<0){
					if(Vec3.len2(hitInfos[i].pos1,vec3)>0.00001){
						continue;
					}
					if(Vec3.len2(hitInfos[i].pos2,vec32)>0.00001){
						continue;
					}
					idx=i;
	...
	ret.prototype.calc=function(dt){
		...
		for(var i=0;i<hitInfos.length;i++){
			...
			var obj1=hitInfo.obj1;
			var obj2=hitInfo.obj2;
			Vec3.sub(bV0,hitInfo.pos2org,hitInfo.pos1org);
			var l = Vec3.scalar(bV0);
			Mat43.dotMat43Vec3(sphere.location,obj1.matrix,hitInfo.pos1ex);
			Vec3.add(sphere.location,sphere.location,bV0);
			Vec3.set(sphere.size,l,l,l);
			sphere.calcPre();
			var l  = Collider.calcClosest(ans1,ans2,sphere,obj2.children[0]);
			
			if(l>=0){
			//Mat43.dotMat43Vec3(ans2,obj2.matrix,hitInfo.pos2ex);
			//if(hitInfo.counter>20){
//			 ( Vec3.len2(ans1,hitInfo.pos1org) > 0.01
//				|| Vec3.len2(ans2,hitInfo.pos2org) > 0.01)){
				hitInfos.splice(i,1);
				disableHitInfos.push(hitInfo);
				i--;
		//		continue;
			}
		//	Mat43.dotMat43Vec3(hitInfo.pos1org,obj1.matrix,hitInfo.pos1ex);
		//	Mat43.dotMat43Vec3(hitInfo.pos2org,obj2.matrix,hitInfo.pos2ex);
		//	Vec3.sub(hitInfo.pos1,hitInfo.pos1org,hitInfo.obj1.location);
		//	Vec3.sub(hitInfo.pos2,hitInfo.pos2org,hitInfo.obj2.location);
		...
		for (var i = 0;i<hitInfos.length; i++) {
			...
			//Vec3.mul(dv,dv,Math.max(Vec3.scalar(dv)-0.0001,0)/Vec3.scalar(dv));
