...
var OnoPhy = (function(){
	...
	var LinConstraint = (function(){
		var LinConstraint = function() {
			...
			this.pos2 = new Vec3(); //接触相対位置2
			this.nVec = new Vec3(); //法線方向
			this.dp= new Vec3(); //法線方向
			this.nEffic = 0; //法線方向に対する力の影響係数
			this.nImpulse = 0;  //法線方向にかかる力
			this.flg=false;
			this.effic = new Mat44();
		...
		var mat1 = new Mat33();
		var mat2 = new Mat33();
		ret.prototype.init=function(nVec,obj1,pos1,obj2,pos2,flg){
			var hitInfo = this;
			if(!obj1 || !obj2){
				return;
			}
			...
			Vec3.nrm(hitInfo.nVec,nVec);
			Vec3.sub(hitInfo.dp,pos2,pos1);
			Vec3.mul(hitInfo.dp,hitInfo.nVec,Vec3.dot(hitInfo.nVec,hitInfo.dp));
			//Vec3.nrm(hitInfo.nVec,hitInfo.dp);
			...
			Mat33.add(hitInfo.effic,mat1,mat2);
			Mat33.mul(hitInfo.effic,hitInfo.effic,-1);
			var inv_m = obj1.inv_mass + obj2.inv_mass;
			hitInfo.effic[0]+=inv_m;
			hitInfo.effic[4]+=inv_m;
			hitInfo.effic[8]+=inv_m;

			Mat33.getInv(hitInfo.effic,hitInfo.effic);
		...
		ret.prototype.calcConstraint=function(){
			...
			calcDiffVelocity(dv,hitInfo); //衝突点の速度差
			var old = hitInfo.nImpulse; //現在の撃力
			//Vec3.copy(oldvec,hitInfo.impulse);
			hitInfo.nImpulse += Vec3.dot(dv, hitInfo.nVec) * hitInfo.nEffic ; //速度差から新しい撃力を算出
			Mat33.dotVec(vec,hitInfo.effic,dv);
			Vec3.add(hitInfo.impulse,hitInfo.impulse,vec);
//			if(hitInfo.nImpulse<0 && this.flg){
//				hitInfo.nImpulse=0;
//			}
			Vec3.mul(impulse, hitInfo.nVec, (hitInfo.nImpulse-old));
//			Vec3.sub(vec,hitInfo.impulse,oldvec);
		...
		ret.prototype.calcAfter=function(dt){
			...
			Vec3.mul(impulse, hitInfo.dp, PENALTY*dt*hitInfo.nEffic);
			addimpulse(hitInfo,impulse);
		}
	...
	var Joint = (function(){
		var Joint = function(){
			...
			this.linImpulse = new Vec3();
			this.linEffic_ = new Mat33();
			this.linEffic = new Mat33();
			this.linR1 = new Vec3();
			this.linR2 = new Vec3();
			...
			for(var j=0;j<DIMENSION;j++){
				this.linConstraint.push(new LinConstraint());
				this.angConstraint.push(new AngConstraint());
				this.angConstraint_.push(new LinConstraint());
			}
		}
		var ret = Joint;
		var n = new Vec3();
		...
		ret.prototype.calcPre=function(){
			...
			for(var i=0;i<DIMENSION;i++){
				...
				if(joint.use_limit_lin[i]){
					...
					Vec3.set(axis2,joint.object1.rotmat[i*3]
						,joint.object1.rotmat[i*3+1]
						,joint.object1.rotmat[i*3+2]);
					//位置差
					...
					|| l > joint.limit_lin_upper[i]){
						...
						Vec3.mul(n,axis,l);
						Vec3.add(n2,n,jointp);
						Vec3.add(truepos,truepos,n);
						//this.linConstraint[i].init(n,this.parent,jointp,this.child,n2);
						if(count==0){
							Vec3.set(vec,this.object1.rotmat[i],this.object1.rotmat[i*3+1],this.object1.rotmat[i*3+2]);
						}else{
							Vec3.set(vec2,this.object1.rotmat[i],this.object1.rotmat[i*3+1],this.object1.rotmat[i*3+2]);
						}
			...
			Vec3.sub(n,truepos,jointp);
			Vec3.sub(this.linR1,jointp,this.parent.location);
			Vec3.set(this.linR2,0,0,0);

			calcEfficM(this.linEffic,this.parent,this.linR1,this.child,this.linR2);
			Mat33.getInv(this.linEffic_,this.linEffic);

				
			if(count==1){
				// 位置制限1軸の場合
				// F=(vX/((MX)X)X
				Mat33.dotVec(n,this.linEffic_,vec);
				var b = 1/Vec3.dot(n,vec);
				var m = this.linEffic;
				m[0]=vec[0]*vec[0]*b;
				m[1]=vec[0]*vec[1]*b;
				m[2]=vec[0]*vec[2]*b;
				...
				Mat33.dotVec(n,this.linEffic_,vec);
				Mat33.dotVec(n2,this.linEffic_,vec2);
				var mxx=Vec3.dot(n,vec);
				var mxy=Vec3.dot(n,vec2);
				var myx=Vec3.dot(n2,vec);
				var myy=Vec3.dot(n2,vec2);

				var denom = 1/ (mxx*myy  - mxy*myx);
				var m = this.linEffic;
				m[0]=(myy*vec[0]*vec[0] - myx*vec2[0]*vec[0] - mxy*vec[0]*vec2[0] + mxx*vec2[0]*vec2[0])  *denom;
				m[1]=(myy*vec[0]*vec[1] - myx*vec2[0]*vec[1] - mxy*vec[0]*vec2[1] + mxx*vec2[0]*vec2[1])  *denom;
				m[2]=(myy*vec[0]*vec[2] - myx*vec2[0]*vec[2] - mxy*vec[0]*vec2[2] + mxx*vec2[0]*vec2[2])  *denom;
				m[3]=(myy*vec[1]*vec[0] - myx*vec2[1]*vec[0] - mxy*vec[1]*vec2[0] + mxx*vec2[1]*vec2[0])  *denom;
				m[4]=(myy*vec[1]*vec[1] - myx*vec2[1]*vec[1] - mxy*vec[1]*vec2[1] + mxx*vec2[1]*vec2[1])  *denom;
				m[5]=(myy*vec[1]*vec[2] - myx*vec2[1]*vec[2] - mxy*vec[1]*vec2[2] + mxx*vec2[1]*vec2[2])  *denom;
				m[6]=(myy*vec[2]*vec[0] - myx*vec2[2]*vec[0] - mxy*vec[2]*vec2[0] + mxx*vec2[2]*vec2[0])  *denom;
				m[7]=(myy*vec[1]*vec[1] - myx*vec2[2]*vec[1] - mxy*vec[2]*vec2[1] + mxx*vec2[2]*vec2[1])  *denom;
				m[8]=(myy*vec[2]*vec[2] - myx*vec2[2]*vec[2] - mxy*vec[2]*vec2[2] + mxx*vec2[2]*vec2[2])  *denom;
			}


			Vec3.set(this.linImpulse,0,0,0);
			Vec3.copy(vec,this.linImpulse);
			if(!this.parent.fix){
				addimpulse_(this.parent,this.linR1,vec);
			}
			if(!this.child.fix){
				Vec3.mul(vec,vec,-1);
				addimpulse_(this.child,this.linR2,vec);
			}
			this.parent.impFlg=true;
			this.child.impFlg=true;

			Vec3.sub(n2,joint.parent.location,jointp);
			//角度制限
			var drarray=new Array(3);
			for(var i=0;i<DIMENSION;i++){
				...
				if(joint.use_limit_ang[i]){
					...
					if(i==0){
						...
						Vec3.cross(axis4,axis3,axis);//角度判定基準軸1
						Vec3.cross(axis3,axis4,axis);//角度判定基準軸2

					}else if(i==1){
						Vec3.set(axis2,bM[i2*4],bM[i2*4+1],bM[i2*4+2]); //角度判定軸
						Vec3.set(axis3,cM[i1*4],cM[i1*4+1],cM[i1*4+2]); //角度判定基準軸
						Vec3.norm(axis3);
						Vec3.muladd(axis4,axis2,axis3,-Vec3.dot(axis3,axis2));
						Vec3.cross(axis,axis3,axis2);//ワールド回転軸
					}else if(i==2){
						Vec3.set(axis2,bM[i1*4],bM[i1*4+1],bM[i1*4+2]); //角度判定軸
						Vec3.set(axis,cM[i*4],cM[i*4+1],cM[i*4+2]);//ワールド回転軸
						Vec3.cross(axis,axis,axis2);
						Vec3.cross(axis,axis,axis2);
						Vec3.set(axis,-cM[i*4],-cM[i*4+1],-cM[i*4+2]);//ワールド回転軸
						Vec3.set(axis3,cM[i2*4],cM[i2*4+1],cM[i2*4+2]); //角度判定基準軸
						Vec3.set(axis4,cM[i1*4],cM[i1*4+1],cM[i1*4+2]); //角度判定基準軸2
					...
					|| dr > joint.limit_ang_upper[i]){
						...
						joint.angConstraint[i].init(joint.child,joint.parent,axis,dr);
						Mat33.getRotMat(rotmat,-dr,axis[0],axis[1],axis[2]);
						if(i==0){
							Vec3.set(n2,-bM[0],-bM[1],-bM[2]);
							Vec3.set(n3,cM[8],cM[9],cM[10]);
							Vec3.cross(n2,n2,n3);

						}else{
							Vec3.set(n2,bM[0],bM[1],bM[2]);
						}
						Mat33.dotVec(n3,rotmat,n2);
						Vec3.sub(n3,n2,n3);
						Vec3.add(n,joint.child.location,n2);
						n2[0]+=bM[12];
						n2[1]+=bM[13];
						n2[2]+=bM[14];
						//Vec3.add(n,n3,joint.child.location);
						//Vec3.sub(n3,joint.parent.location,n3);
						joint.angConstraint_[i].init(n3,joint.child,n,joint.parent,n2);
					}
				}
			}
		}
		ret.prototype.calcConstraint=function(){
			_calcDiffVelocity(dv,this.parent,this.child,this.linR1,this.linR2); //速度差
			Vec3.copy(oldimp,this.linImpulse); //前回の撃力
			Mat33.dotVec(vec,this.linEffic,dv); //必要撃力
			Vec3.add(this.linImpulse,this.linImpulse,vec); //撃力に足す

			for(var i=0;i<DIMENSION;i++){
				if(this.enableLin[i] ){
				}
			}
			Vec3.sub(vec,this.linImpulse,oldimp); //今回加える撃力
			...
			for(var i=0;i<DIMENSION;i++){
				if(this.enableAng[i]){
					...
					this.angConstraint_[i].calcConstraint();
				}
			}
		}
		ret.prototype.calcAfter=function(dt){
			for(var i=0;i<DIMENSION;i++){
				if(this.enableLin[i]){
		//			this.linConstraint[i].calcAfter(dt);
				}
			}
			Vec3.sub(vec,truepos,jointp);
			Vec3.mul(vec, vec, PENALTY*dt*0.01);
			if(!this.parent.fix){
				addimpulse_(this.parent,this.linR1,vec);
			}
			if(!this.child.fix){
			...
			for(var i=0;i<DIMENSION;i++){
				if(this.enableAng[i]){
					...
					this.angConstraint_[i].calcAfter(dt);
				}
			}
		}
		return ret;
