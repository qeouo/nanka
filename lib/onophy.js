...
var OnoPhy = (function(){
	...
	var RigidBody = (function(){
		...
		ret.prototype.calcPre = function(){
			...
			case Collider.CAPSULE:
				bold = Math.max(sx,sy);
				sz=Math.max(this.collisionSize[2]*this.scale[2]-bold,0)
				break;
			case Collider.MESH:
		...
		ret.prototype.refresh = function(){
			...
			case Collider.MESH:
				...
				Mat33.mul(this.inertiaTensorBase,this.inertiaTensorBase,S);//重心を中心とした慣性テンソル
	...
	var HitInfo = (function(){
		...
		ret.prototype.calcPre = function(){
			...
			//Vec3.cross(this.axis[1],this.axis[1],this.axis[0]);
			//Vec3.cross(this.axis[2],this.axis[1],this.axis[0]);
			//Vec3.norm(this.axis[1]);
			//Vec3.norm(this.axis[2]);
			...
			//加速に必要な力を求めるための行列
			this.calcEfficM(nM);
			Constraint.calcEffic(this.coeff[0],nM,this.axis[0]);
			Constraint.calcEffic2(this.coeff[1],this.coeff[2],nM,this.axis[1],this.axis[2]);
		...
		ret.prototype.calcConstraintPre=function(){
			...
			for(var i=0;i<DIMENSION;i++){
				Vec3.madd(impulse,impulse,this.axis[i],this.impulse[i]);
			}
		...
		ret.prototype.calcConstraint=function(){
			...
			Vec3.mul(impulse,this.axis[1],this.impulse[1]-o);
			Vec3.madd(impulse,impulse,this.axis[2],this.impulse[2]-old2);
	...
	var LinConstraint = (function(){
		...
		ret.prototype.calcPre=function(){
			for(var i=0;i<DIMENSION;i++){
				this.axisM[i*3]=this.axis[i][0];
				this.axisM[i*3+1]=this.axis[i][1];
				this.axisM[i*3+2]=this.axis[i][2];
			}
		...
		ret.prototype.calcConstraint=function(){
			...
			Mat33.dotVec3(impulse,this.axisM,impulse);
			this.addImpulse(impulse);
		...
		ret.prototype.setConstraint=function(){
			...
			for(var ii=0;ii<DIMENSION;ii++){
				...
				if(i===0){
					...
					Vec3.set(axis,obj1m[3],obj1m[4],obj1m[5]);
					Vec3.set(vec,obj2m[0],obj2m[1],obj2m[2]);
					Vec3.cross(axis,vec,axis);
				}else if(i===1){
					Vec3.cross(axis,this.angConstraint.axis[2],this.angConstraint.axis[0]);
				}
				Vec3.norm(axis);
				...
				if(this.use_spring_ang[i]){
					//ばね
					Vec3.mul(vec,axis,-dr*this.spring_stiffness_ang[i]*DT);//角度差
					Vec3.madd(vec,vec,axis,Vec3.dot(dv,axis)*this.spring_damping_ang[i]*DT);
	...
	ret.prototype.readjustHitInfo=function(target){
		...
		for(var i=hitInfos.length;i--;){
			...
			if(Vec3.dot(hitInfo.axis[0],target.axis[0])<0){
				//めり込み方向が反対の場合は削除
				this.removeHitInfo(i);
				continue;
			}
	...
	ret.prototype.registHitInfo = (function(){
		return function(obj1,pos1,obj2,pos2,hitInfo){
			...
			Vec3.sub(hitInfo.axis[0],pos2,pos1);
			Vec3.norm(hitInfo.axis[0]);
	...
	//id1とid2のペアIDを求める
	var getPairId=function(id1,id2){
		if(id1<id2){
			return (id1<<16) | id2;
		}else{
			return (id2<<16) | id1;
	...
	ret.prototype.calc=function(dt){
		...
		for(var i=0;i<this.joints.length;i++){
			...
			if(this.joints[i].disable_collisions){
				//ジョイント接続されたもの同士の接触無効
				this.disableList[idx]=getPairId(
		...
		for(var i=0;hitList[i].col1;i++){
			...
			if(hit.col1.parent && hit.col2.parent){
				var hitInfo = this.registHitInfo(hit.col1.parent,hit.pos1,hit.col2.parent,hit.pos2);
		...
		for(var i=0;i<this.hitInfos.length;i++){
			...
			Vec3.sub(t,ans1,hitInfo.axis[0]);
			...
			Vec3.madd(ans2,ans1,hitInfo.axis[0],-l);

			Mat43.dotVec3(ans4,obj2.matrix,hitInfo.pos2ex);
			Vec3.add(t,ans4,hitInfo.axis[0]);
			...
			if(l2<l){
				//めり込みが大きい方を採用
				Vec3.copy(ans2,ans4);
				Vec3.madd(ans1,ans2,hitInfo.axis[0],l2);
				l=l2;
			}
