...
//		var ff = obj.mass/(1+obj.mass*a/obj.imoment)
	...
	//	friction-=f;
		...
		if(obj.name=="tes"){
			...
			if(l>0.){
				var d=1/l;
				//Mat43.getRotMat(bM,l*_dt,mem2[0]*d,mem2[1]*d,mem2[2]*d);
				//Vec3.muladd(a_a,a_l,mem2,Vec3.dot(mem2,a_l));
				//Vec3.mul(a_l,a_a,Math.cos(d));
				//Vec3,cross(mem2,mem2,a_a);
				//Vec3.add(a_l,a_a,Math.sin(d));
			...
			if(friction>0){
			console.log(Vec3.scalar(a_a),mem,mem2);
				Vec3.mul(a_fric,a_a,-1/_dt*obj.mass);
				Vec3.add(obj.a,obj.a,a_fric);
				//Vec3.cross(a_a,a_l,a_fric);
				//Vec3.add(obj.aa,obj.aa,a_a);
			}
