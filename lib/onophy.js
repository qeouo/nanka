...
var OnoPhy = (function(){
	...
	var a_a=new Vec3();
	var a_a2=new Vec3();
	var act = function(obj,p,f,obj2){
		var l,l2;

		var penalty = 2*obj.penalty*obj2.penalty/(obj.penalty+obj2.penalty)
		Vec3.mul(a_n,f,penalty);//óÕ
		Vec3.sub(a_l,p,obj.location); //èdêSÇ©ÇÁÇÃãóó£
		...
		Vec3.nrm(a_n,a_n);
		//É_ÉìÉp
		Vec3.muladd(obj.a,obj.a,a_n,Vec3.dot(a_n,obj.v)*-obj.damper);
		//äpÉ_ÉìÉp
		Vec3.cross(a_a,a_l,a_n);
		Vec3.nrm(a_a,a_a);
		l2 =Vec3.dot(a_a,obj.av);
		//Vec3.muladd(obj.aa,obj.aa,a_a,l2*-obj.adamper);
	...
	var mem=new Vec3();
		Vec3.copy(mem,obj.aa);
	var mem2=new Vec3();
		Vec3.copy(mem2,obj.a);

		Vec3.cross(a_a2,obj.aa,a_l);
		Vec3.add(a_a2,a_a2,obj.a);
		Vec3.muladd(a_a2,a_a2,a_n,-Vec3.dot(a_n,a_a2));

	//äpë¨ìx
		Vec3.cross(a_a,obj.av,a_l);
		Vec3.mul(a_a,a_a,obj.imoment);
		Vec3.add(a_a,a_a,obj.v);
		Vec3.muladd(a_a,a_a,a_n,-Vec3.dot(a_a,a_n));
		Vec3.mul(a_fric,a_a,-60);
		f= Vec3.scalar(a_fric);

		if(f==0 && Vec3.scalar(a_a2)==0){
			return;
		}


		var nanka=false;
		if(Vec3.dot(a_a,a_a2)<0){
			nanka=true;
		}

		if(nanka){
			if(friction>0){
				Vec3.cross(a_a,obj.aa,a_l);
				Vec3.mul(a_a,a_a,0*1/Vec3.scalar(a_l));
				Vec3.add(a_a,a_a,obj.a);
				Vec3.muladd(a_fric,a_a,a_n,-Vec3.dot(a_n,a_a));
				Vec3.mul(a_fric,a_fric,-1);
				f=Vec3.scalar(a_fric);
				if(f>friction){
					Vec3.mul(a_fric,a_fric,friction/f);//ñÄéCóÕ
					f=friction;
				}
				friction-=f;
		...
		var aaa=new Vec3();
		Vec3.muladd(aaa,obj.v,a_n,-Vec3.dot(a_n,obj.v));
		var f2= Vec3.scalar(aaa)*60;
		Vec3.cross(aaa,obj.av,a_l);
		var f3= Vec3.scalar(aaa)*60*obj.imoment;
		if(f2<f3){
			f2=f3;
		}
		Vec3.mul(a_fric,a_fric,f2/f);//ñÄéCóÕ
		f=f2;
...
//			}
//
//			Vec3.mul(a_a,a_n,Vec3.dot(a_n,obj.v));
//			Vec3.sub(a_a,a_a,obj.v);
//			Vec3.mul(a_a,a_a,60);
//			f=Vec3.scalar(a_a);
//			if(f>friction){
//				f=friction;
...
//			}
//			friction-=f;
//			Vec3.add(obj.a,obj.a,a_a);
//
//			Vec3.cross(a_a,a_l,obj.av);
//			Vec3.mul(a_a,a_a,-1);
//			Vec3.add(obj.aa,obj.aa,a_a);
//		}
//		if(friction>0){
//			Vec3.mul(a_a,a_n,Vec3.dot(a_n,mem2));
//			Vec3.sub(a_a,a_a,mem2);
//			f=Vec3.scalar(a_a);
//			if(f>friction){
//				f=friction;
//				Vec3.norm(a_a);
