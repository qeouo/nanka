...
var OnoPhy = (function(){
	...
	var act = function(obj,p,f,obj2){
		...
		var friction;
		if(a_a[0]*a_a[0]+a_a[1]*a_a[1]+a_a[2]*a_a[2]<0.0001 ){
	...
	//â¡ë¨ìx
		Vec3.copy(a_a2,obj.a);
		Vec3.nrm(a_a,a_n);
		l2=Vec3.dot(a_a,a_a2);
		Vec3.muladd(a_a,a_a2,a_a,-l2);

		var f=Vec3.scalar(a_a);
		if(f>friction){
			f=friction;
		}
		friction-=f;
	...
	//äpë¨ìx
		Vec3.cross(a_a2,obj.av,a_l);
		Vec3.mul(a_a2,a_a2,60*obj.imoment);
		//Vec3.mul(a_a2,a_a2,60);
		Vec3.muladd(a_a2,a_a2,obj.v,60);
		Vec3.nrm(a_a,a_n);
		l2=Vec3.dot(a_a,a_a2);
		Vec3.muladd(a_a,a_a2,a_a,-l2);
		f= Vec3.scalar(a_a);
