...
//			Vec3.add(ans,ans,this.location);
	...
	ret.prototype.createPhyObj= function(type){
		var obj =new PhyObj();
		obj.id=idcount;
		idcount++;
	...
	ret.prototype.createSpringMesh = function(num){
			//スプリングメッシュオブジェクト作成
			...
			for(j = i;j--;){
				...
				if(func){
					...
					if(l<0){
						...
						registHitInfo2(obj1.parent,ans1,obj2.parent,ans2);
					}
				}
			}
		}
...
//				if(Math.abs(Vec3.dot(vbuf,axis))<0.0000000000001){
		...
		while(1){
			...
			if(Vec3.dot(s,axis) - Vec3.dot(v[face.v[0]],axis)<0.000000001){
					//重複する場合はその時点での点が最短
	...
	ret.collisionLine = function(p0,p1,collision){
		...
		case OnoPhy.CUBOID:
			if(OnoPhy.CUBOID_LINE(p0,p1,collision)){
				flg=true;
			}	
			break;
		case OnoPhy.SPHERE:
			if(ret.SPHERE_LINE(p0,p1,collision)){
				flg=true;
			}
			break;
		case OnoPhy.CAPSULE:
			if(OnoPhy.CAPSULE_LINE(p0,p1,collision)){
				flg=true;
			}
			break;
		}
		if(flg){
			return OnoPhy.result;
		}
		return -1;
	}
