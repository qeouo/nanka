...
//			Vec3.add(ans,ans,this.location);
	...
	var checkHitInfo=(function(){
		var n=new Vec3();
		return function(hitInfo,obj1,pos1,obj2,pos2){
			
			if(!(obj1 == hitInfo.obj1 && obj2 == hitInfo.obj2)){
				return 0;
			}
			if(Vec3.len2(hitInfo.pos1,pos1)>0.000001){
				return 0;
			}
			if(Vec3.len2(hitInfo.pos2,pos2)>0.000001){
				return 0;
			}
			Vec3.sub(n,obj2.location,obj1.location);
			if(Vec3.len2(n,hitInfo.dpos)>0.0001){
				return 0;
			}
			Vec3.add(n,n,pos2);
			Vec3.sub(n,n,pos1);
			Vec3.norm(n);
			if(Vec3.dot(hitInfo.nVec,n)<=0.3){
				return 0;
			}
			return 1;
		}
	...
	var registHitInfo = (function(){
		...
		return function(obj1,pos1,obj2,pos2){
			...
			for(var i=0;i<hitInfos.length;i++){
				if(checkHitInfo(hitInfos[i],obj1,vec3,obj2,vec32)){
					hitInfo = hitInfos[i];
					break;
				}
			}	
	...
	ret.prototype.calc_t=function(dt){
		...
		for(var i=0;i<hitInfos.length;i++){
			...
			if(checkHitInfo(hitInfo,hitInfo.obj1,hitInfo.pos1,hitInfo.obj2,hitInfo.pos2)){
			...
			for(j = i;j--;){
				...
				if(func){
					...
					if(l<0){
						...
						registHitInfo(obj1.parent,ans1,obj2.parent,ans2);
					}
				}
			}
		}
