...
var OnoPhy = (function(){
	...
	var AngConstraint = (function(){
		...
		var vec1 = new Vec3();
		var vec2 = new Vec3();
		ret.prototype.init=function(obj1,obj2,enables,vecs){
			if(!obj1 || !obj2){
				return;
			}
			obj1.impFlg=true;
			...
			Mat33.getInv(this.effic,this.effic);
			Mat33.getInv(this.effic_,this.effic);
			...
			if(!obj1.fix){
				Mat33.dotVec(dv,obj1.inv_inertiaTensor,this.impulse);
				Vec3.add(obj1.rotV,obj1.rotV,dv);
			}
			if(!obj2.fix){
				Mat33.dotVec(dv,obj2.inv_inertiaTensor,this.impulse);
				Vec3.sub(obj2.rotV,obj2.rotV,dv);
			}
		...
		ret.prototype.calcConstraint=function(){
			...
			var obj1=this.obj1;
			var obj2=this.obj2;
			Vec3.sub(dv,obj2.rotV,obj1.rotV);
			Vec3.copy(old,this.impulse); //前回の撃力
			Mat33.dotVec(vec,this.effic,dv); //必要撃力
			Vec3.add(this.impulse,this.impulse,vec); //撃力に足す
			...
			//それぞれに加える
			if(!obj1.fix){
				Mat33.dotVec(dv,obj1.inv_inertiaTensor,vec);
				Vec3.add(obj1.rotV,obj1.rotV,dv);
			}
			if(!obj2.fix){
				Mat33.dotVec(dv,obj2.inv_inertiaTensor,vec);
				Vec3.sub(obj2.rotV,obj2.rotV,dv);
			}
	...
	var Joint = (function(){
		...
		ret.prototype.calcPre=function(){
			...
			//差
			Vec3.sub(dp,this.child.location,jointp);
			...
			for(var i=0;i<DIMENSION;i++){
				ar[i]=0;
				//軸
				Vec3.set(axis,this.object1.rotmat[i*3]
					,this.object1.rotmat[i*3+1]
					,this.object1.rotmat[i*3+2]);
			...
			Vec3.add(this.truepos,this.truepos,jointp);

			this.linConstraint.init(this.parent,jointp,this.child,this.child.location,ar,vecs);


			//ばね
			Vec3.sub(dp,jointp,this.child.location);
			Vec3.set(vec,bM[12],bM[13],bM[14]);
			_calcDiffVelocity(dv,this.parent,this.child,vec,Vec3.ZERO); //速度差
			for(var i=0;i<DIMENSION;i++){
				if(this.use_spring[i]){
					...
					Vec3.mul(vec,axis,_dt*Vec3.dot(axis,dv)*this.spring_damping[i]);
					addimpulse(this.linConstraint,vec);
				}
			}
			...
			if(this.parent==this.object1){
				...
				Mat33.getInv(rotmat,this.object1.rotmat);
				Mat33.dot(rotmat,rotmat,mat33); //差分回転行列
				Mat33.getEuler(vec,rotmat); //オイラー角に変換
				Vec3.mul(vec,vec,-1); //object1がparentでない場合は逆になる

			}

			Mat33.copy(this.truemat,mat33);
			var cM = this.child.matrix;
			for(var i=0;i<DIMENSION;i++){
				ar[i]=0;
				//軸の向き
				if(i==0){
					Vec3.set(axis,bM[0],bM[1],bM[2]);
				}else if(i==1){
				...
				if(this.use_limit_ang[i]){
					...
					|| dr > this.limit_ang_upper[i]){
						...
						Mat33.getRotMat(mat33,dr,axis[0],axis[1],axis[2]);
						Mat33.dot(this.truemat,mat33,this.truemat);
			...
			this.angConstraint.init(this.parent,this.child,ar,vecs);
		...
		ret.prototype.calcAfter=function(dt){
			...
			Vec3.sub(vec,this.truepos,jointp);
	...
	var calcDiffVelocity = function(dv,hitInfo){
		_calcDiffVelocity(dv,hitInfo.obj1,hitInfo.obj2,hitInfo.pos1,hitInfo.pos2);
//		var obj1 = hitInfo.obj1;
//		var obj2 = hitInfo.obj2;
//		Vec3.sub(dv, obj2.v, obj1.v);
