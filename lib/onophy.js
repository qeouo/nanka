...
var OnoPhy = (function(){
	...
	var REPETITION_MAX=20; //繰り返しソルバ最大回数
	...
	var Constraint=(function(){
		...
		ret.prototype.addImpulse = function(impulse){
			...
			obj2.addImpulse(this.pos2,addimpulseBuf);
	...
	var HitInfo = (function(){
		...
		ret.prototype.calcConstraint=function(){
			...
			if(hitInfo.type==0){
				...
				this.addImpulse(impulse); //摩擦力を速度に反映
			}
		}
	...
	var LinConstraint = (function(){
		...
		ret.prototype.calcConstraint=function(){
			this.calcDiffVelocity(dv)
			Vec3.muladd(dv,dv,this.vecs[0],this.motorVelocity);
...
//ばね
			...
			this.linConstraint.init(object1,obj1p,object2,obj2p,ar,vecs);
			...
			for(var i=0;i<DIMENSION;i++){
				...
				if(this.use_spring_ang[i]){
					...
					var obj2 = this.child;
					Vec3.mul(n,axis,dr*this.spring_stiffness_ang[i]*_dt);//角度差
					if(!obj1.fix){
						Mat33.dotVec(dv,obj1.inv_inertiaTensor,n);
						Vec3.add(obj1.rotV,obj1.rotV,dv);
					}
					if(!obj2.fix){
						Mat33.dotVec(dv,obj2.inv_inertiaTensor,n);
						Vec3.sub(obj2.rotV,obj2.rotV,dv);
					}

					Vec3.sub(dv,obj2.rotV,obj1.rotV);//回転差
					Vec3.mul(n,axis,Vec3.dot(dv,axis)*this.spring_damping_ang[i]*_dt);
					if(!obj1.fix){
						Mat33.dotVec(dv,obj1.inv_inertiaTensor,n);
						Vec3.add(obj1.rotV,obj1.rotV,dv);
					}
			...
			this.angConstraint.init(this.object1,this.object2,ar,vecs);
...
1
	...
	ret.setPhyObjData = function(phyobj){
		...
		case Collider.SPHERE:
			...
			var i=(2.0/5.0)* phyobj.mass*a*a;
			Vec3.set(phyobj.inertia,i,i,i);
			break;
		case Collider.CAPSULE:
			var a =Math.max(sx,sy);
			var xy=phyobj.mass*(a*a/4.0 + sz*sz/12.0);
