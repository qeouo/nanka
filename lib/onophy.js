...
function inherits(childCtor, parentCtor) {
	...
	var Capsule = ret.Capsule = (function(){
		...
		ret.prototype.calcPre =function(){
			var sc=this.scale;
			var si=this.size;
			this.bold = MAX(sc[0]*si[0],sc[1]*si[1]);
			var temp=this.size[2];
			this.size[2]=MAX(temp - this.bold,0);
			PhyObj.prototype.calcPre.call(this);
			this.size[2]=temp;
		}
		ret.prototype.calcSupport=function(ans,v){
...
//		Vec3.mul(axis,va,0.5);
		...
		while(1){
			...
			//取得した点が重複するかチェック
			for(var i=0;i<idx;i++){
				if(Vec3.len2(v[i],s)<0.0000000000001){
					//重複する場合はその時点での点が最短
					if(idx==1){
						Vec3.copy(ans1,v1[0]);
						Vec3.copy(ans2,v2[0]);
			...
			if(idx==1){
				...
				for(var i=0;i<4;i++){
					...
					|| l1*l2<0){
						...
						var l=vbuf[0]*vbuf[0]+vbuf[1]*vbuf[1]+vbuf[2]*vbuf[2];//面と原点との距離
						if(min<0 || l<min){
							min=l;
							Vec3.copy(axis,vbuf);
							next=(i+3)&3;
		...
		while(1){
			...
			var face = faces[minn];
			//最短面の法線取得
			if(!face.len){  // if(face.len<0.00000000000001) のほうが安心
				//原点に非常に近い場合は、法線方向をチェック
				for(var i=0;i<idx;i++){
					var flg=false;
					for(var j=0;j<3;j++){
						if(face.v[j]==i){
