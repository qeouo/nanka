		...
		ret.prototype.calc =function(){
			...
			if(this.con2){
				if(this.con2.type===FACE){
					...
					for(var i=0;i<3;i++){
						Vec3.madd(this.p1,this.p1,this.con2.face.points[i].location,this.con2.ratio[i]);
					}
				}else{
	...
	var Cloth = ret.Cloth = (function(){
		...
		var PhyFace = ret.PhyFace = (function(){
			...
			ret.prototype.calcVelocity = function(dv){
				...
				Vec3.madd(dv,dv,this.face.points[1].v,this.ratio[1]);
				Vec3.madd(dv,dv,this.face.points[2].v,this.ratio[2]);
			}

			ret.prototype.addImpulse = function(pos,impulse){
				for(var i=0;i<DIMENSION;i++){
					if(!this.face.points[i].fix){
						Vec3.madd(this.face.points[i].v
		...
		ret.prototype.ray=function(res,p0,p1){
			...
			for(var i=0;i<faces.length;i++){
				...
				var t0=face.points[0].location;//poses[face.idx[0]];
				var t1=face.points[1].location;//poses[face.idx[1]];
				var t2=face.points[2].location;//poses[face.idx[2]];
		...
		ret.prototype.getPhyFace = function(i,ans2){
			...
			Vec3.sub(v1,phyFace.face.points[1].location,phyFace.face.points[0].location);
			Vec3.sub(v2,phyFace.face.points[2].location,phyFace.face.points[0].location);
			Vec3.cross(v1,v1,v2);
			var p=[phyFace.face.points[0].location
				,phyFace.face.points[1].location
				,phyFace.face.points[2].location
				,phyFace.face.points[0].location
				,phyFace.face.points[1].location];
		...
		ret.prototype.calcPre=function(onophy){
			...
			for(var i=0;i<list.length;i++){
				...
				for(var j=0;j<this.faces.length;j++){
					...
					if(l<0){
						var phyFace = this.getPhyFace(j,ans2);//disablePhyFace.pop();
//						phyFace.face=face;
//
//						var obj2=phyFace;
//						//ポリゴン接点から各頂点の影響比率を求める
