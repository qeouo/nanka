...
//			Vec3.add(ans,ans,this.location);
	...
	var aabbHitFlg=new Array(3);
	var aabbHitFlgIdx=new Array(3);
	var aabbHitFlgAll=[];
	for(var i=0;i<aabbHitFlg.length;i++){
		aabbHitFlg[i]=new Array(1024);
	}
	...
	var backetSort=function(arr,n,func){
		for(var i=0;i<backet.length;i++){
			backetIndex[i]=0;
	...
	var kisu=function(arr,func){
		for(var n=0;n<8;n++){
			backetSort(arr,n*4,func);
		}
	}
	ret.prototype.checkAABB=function(){
		for(var i=0;i<3;i++){
			//ソート
			var AABBSort = this.AABBSorts[i];
			AABBSort.sort(function(a,b){
				if(a.AABB.min[i]< b.AABB.min[i]){
					return -1;
				}else if(a.AABB.min[i]> b.AABB.min[i]){
					return 1;
				}else{
					return 0;
				}
		...
		for(var i=1;i<3;i++){
			...
			for(var j=0;j<AABBSort.length;j++){
				...
				for(var k=j+1;k<AABBSort.length;k++){
					...
					aabbHitFlg[i][aabbHitFlgIdx[i]]=pairId;
					aabbHitFlgIdx[i]++;
				}
			}

			aabbHitFlg[i][aabbHitFlgIdx[i]]=-1;
			kisu(aabbHitFlg[i]);
		}

		//AABBの重なりチェックx軸
		var aabbHitFlgAllIdx=0;
		for(var j=0;j<AABBSort.length;j++){
			...
			for(var k=j+1;k<AABBSort.length;k++){
				...
				aabbHitFlgAll[aabbHitFlgAllIdx].pairId=pairId;
				aabbHitFlgAll[aabbHitFlgAllIdx].col1=AABBSort[j];
				aabbHitFlgAll[aabbHitFlgAllIdx].col2=AABBSort[k];
				aabbHitFlgAllIdx++;
			}
			aabbHitFlgAll[aabbHitFlgAllIdx].pairId=-1;
			kisu(aabbHitFlgAll,function(a){a.pairId});
		}
		...
		aabbHitFlgIdx[1]=0;
		aabbHitFlgIdx[2]=0;
		for(var i=0;aabbHitFlgAll[i].pairId>=0;i++){
			var pairId = aabbHitFlgAll[i].pairId;
			var flg=1;
			
			//x軸と同じペアIDがy,zにあるか
			for(var j=1;j<3;j++){
				var aabbHitF=aabbHitFlg[j];
				var k=aabbHitFlgIdx[j];
				if(!flg){
					break;
				}
				for(;aabbHitF[k]>=0;k++){
			...
			if(flg){
				//あった場合追加
				aabbHitFlgAll[idx].pairId=aabbHitFlgAll[i].pairId;
				aabbHitFlgAll[idx].col1=aabbHitFlgAll[i].col1;
				aabbHitFlgAll[idx].col2=aabbHitFlgAll[i].col2;
				idx++;
			}
		}
		aabbHitFlgAll[idx].pairId=-1;
	}
