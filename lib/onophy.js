...
var OnoPhy = (function(){
	...
	var act = function(obj,p,force,obj2){
		...
		Vec3.nrm(a_n,force);
		//ダンパ
		Vec3.muladd(mem,obj.v,obj.a,0/obj.mass*_dt);
		Vec3.muladd(mem2,obj.av,obj.aa,0/obj.imoment*_dt);
		...
		//角ダンパ
		Vec3.cross(a_a,mem2,a_l);
		//Vec3.mul(a_a,mem2,-obj.adamper);
		Vec3.mul(a_a,a_a,1/Vec3.scalar(a_l)/Vec3.scalar(a_l));
		Vec3.mul(a_fric,a_n,Vec3.dot(a_n,a_a)*obj.adamper);
		Vec3.cross(a_a,a_l,a_fric);
		//Vec3.add(obj.aa,obj.aa,a_a);
		...
		Vec3.mul(mem,mem,-1);
		var a = obj.imoment + Vec3.scalar(a_l)*Vec3.scalar(a_l)*obj.mass;

		a_fric[0] = obj.mass * obj.imoment
			*(a_a[0] * a + obj.mass
				*(a_l[1]*(a_a[0]*mem[1]-a_a[1]*mem[0])
				 + a_l[2]*(a_a[0]*mem[2]-a_a[2]*mem[0])
				))/((a + obj.mass*Vec3.dot(a_l,mem))*a)
		a_fric[1] = obj.mass * obj.imoment
			*(a_a[1] * a + obj.mass
				*(a_l[2]*(a_a[1]*mem[2]-a_a[2]*mem[1])
				 + a_l[0]*(a_a[1]*mem[0]-a_a[0]*mem[1])
				))/((a + obj.mass*Vec3.dot(a_l,mem))*a)
		
		a_fric[2] = obj.mass * obj.imoment
			*(a_a[2] * a + obj.mass
				*(a_l[0]*(a_a[2]*mem[0]-a_a[0]*mem[2])
				 + a_l[1]*(a_a[2]*mem[1]-a_a[1]*mem[2])
				))/((a + obj.mass*Vec3.dot(a_l,mem))*a)
...
//		var ff = obj.mass/(1+obj.mass*a/obj.imoment)
		...
		if(obj.name=="tes"){
			...
			Vec3.add(a_a,mem,mem2);
			Vec3.muladd(a_a,a_a,a_n,-Vec3.dot(a_n,a_a));
			console.log(Vec3.scalar(a_a),a_a);
		}
