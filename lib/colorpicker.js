...
var rgb2hsv=function(hsv,rgb){
	...
	Colorpicker.init=function(){
		...
		var setCursor=function(rgb,flg){
			rgb2hsv(hsv,rgb);
		...
		for(var i=0; i<e.length; i+=1) {
			...
			if( node.className && (node.className == "colorpicker")) {
				node.addEventListener("blur",function(evt){this.parentNode.removeChild(div);},false);
				node.addEventListener("click",function(evt){
					...
					hex2rgb(rgb,this.value);
					setCursor(rgb,false);
				},false);
				node.addEventListener("change",function(evt){
					this.style.backgroundColor="#"+this.value;
					hex2rgb(rgb,this.value);
					this.style.color=(0.3*rgb[0]+0.6*rgb[1]+0.1*rgb[2]<0.5)?"white":"black";
				},false);
			}
		}
	}
