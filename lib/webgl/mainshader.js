...
")
	...
	for(var i=0;i<nattributes.length;i++){
		nargs[nattributes[i].name]=Rastgl.initAtt(program,nattributes[i].name,nattributes[i].size,gl.FLOAT);
		nattributes[i].arg=nargs[nattributes[i].name];
	}
	var uniforms=[
		"projectionMat",
		"lightMat",
	...
	for(var i=0;i<uniforms.length;i++){
		var name = uniforms[i];
		nargs[name]=gl.getUniformLocation(program,name);
	}
