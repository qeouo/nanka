...
ret.draw=function(ono3d,shadowTex,envtexes,camerap,frenel){
	...
	for(var i=1;i<envtexes.length/2;i++){
		size/=2;
		//gl.viewport(0,0,size,size*s2);
		//gl.disable(gl.BLEND);
		//Rastgl.copyframe(textures[i-1],(Math.random()-0.5)/1024,(Math.random()-0.5)/1024 ,960/1024,540/1024);
		//gl.enable(gl.BLEND);
		//gl.blendColor(0,0,0,0.4);
		//Rastgl.copyframe(textures[i]
		//		,(Math.random()-0.5)/size*0
		//		,(Math.random()-0.5)/size*0
		//		,960/1024,540/1024); //既存の光テクスチャを重ねる
		//gl.bindTexture(gl.TEXTURE_2D, textures[i]);
		//gl.copyTexSubImage2D(gl.TEXTURE_2D,0,0,0,0,0,size,size*s2);//結果を光テクスチャに書き込み
	...
	//Rastgl.copyframe(textures[3],0,0,2,2,0,0,960/1024,540/1024);
