...
ret.draw=function(ono3d,shadowTex,env2dtex,camerap){
	...
	var buff = Rastgl.getJsBuffer(arr.length*3*shader.amax);
	if(buff.length<arr.length*3*shader.amax){
	...
	render(ono3d,shadowTex,env2dtex,camerap,false);
	...
	gl.depthMask(true);
	render(ono3d,shadowTex,env2dtex,camerap,true);

}
var render = function(ono3d,shadowTex,env2dtex,camerap,opFlg){
