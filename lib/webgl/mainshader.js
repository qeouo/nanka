...
void main(void){ \
	...
	gl_FragColor = mix(vColor2,colA,vf0); \
} \
")
	args={};
	...
	args["af0"]=Rastgl.initAtt(program,"af0",1,gl.FLOAT,Rastgl.floatGlBuffer);
	...
	ret.draw=function(ono3d,shadowTex,envtexes,camerap){
		...
		var floatbuffer= args["af0"].buffer.jsbuffer;
		...
		for(var i=0;i<facessize;i++){
			...
			if(flg){
				...
				floatbuffer[index]=renderface.mrr;
				floatbuffer[index+1]=renderface.mrr;
				floatbuffer[index+2]=renderface.mrr;
				uvindex+=6;
				index+=3;
			}
			}
			...
			Rastgl.setArg(args["af0"],floatbuffer);
