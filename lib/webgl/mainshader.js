...
void main(void){ \
	...
	if(normalmappow>0.){ \
		lowp vec4 nrmmap = texture2D(uNormalmap,vec2(vNormalUv.s,vNormalUv.t)); \
		uv = vUv + (vView*eye).xy * nrmmap.w * normalmappow; \
	...
	highp vec4 colA; \
	colA = texture2D(uEnvmap \
		,vec2(atan(angle.x,-angle.z)*_PI*0.5 + 0.5 \
		,-atan(angle.y,length(angle.xz))*_PI + 0.5)); \
	\
	colA.rgb = colA.rgb*2.0 + max(vec3(0.0,0.0,0.0),colA.rgb - 0.5)*8.0; \
	lowp vec3 light; \
	...
	lowp vec4 vColor2 = vec4(uColor.xyz * light *clamp(vFar,0.,1.),uColor.w); \
	if(uTex==1){ \
...
_offset=0;
	...
	ret.draw=function(ono3d,shadowTex,envtexes,camerap){
		...
		var normaluvindex=args["aNormalUv"].offset;
		...
		while(1){
			...
			normaluvindex=args["aNormalUv"].offset-args["aSvec"].offset;
			posindex=0;
			index=0;
			for(var i=0;i<facessize;i++){
				...
				if(material.texture){
					...
					jsbuffer2[normaluvindex]=renderface.nuv[0][0]
					jsbuffer2[normaluvindex+1]=renderface.nuv[0][1]
					jsbuffer2[normaluvindex+2]=renderface.nuv[1][0]
					jsbuffer2[normaluvindex+3]=renderface.nuv[1][1]
					jsbuffer2[normaluvindex+4]=renderface.nuv[2][0]
					jsbuffer2[normaluvindex+5]=renderface.nuv[2][1]
