...
ret.init= function(){
	...
	texture= Rastgl.createTexture(null,1024,1024);
...
void main(void){ \
	...
	/*angle = eye + dot(eye,nrm)* ( uRefract -1.0) * nrm;*/ \
	angle = refract(eye,nrm,1.0/uRefract); \
	angle = normalize((uViewMat * vec4(angle,1.0)).xyz); \
	mediump vec3 angle2; \
	angle2= vec3((gl_FragCoord.x/360.0 - 1.0)*0.577,(gl_FragCoord.y/240.0 -1.0)*(480.0/720.0)*0.577,1.0); \
	angle2 = normalize(angle2); \
	vec4 refCol2 = texture2D(uTransparentmap,gl_FragCoord.xy/1024.0+(angle - angle2).xy*0.2); \
	refCol2.rgb = refCol2.rgb  * (refCol2.a * 3.0 + 1.0) * uColor.rgb; \
	/*乱反射強度*/ \
	vec3 light; \
	float diffuse = -dot(normalize(nrm),uLight)*.5+.5; \
...
ret.draw=function(ono3d,shadowTex,envtexes,camerap,frenel){
	...
	gl.uniformMatrix4fv(args["lightMat"],false,new Float32Array(ono3d.pvMat));

	Mat44.copy(ono3d.pvMat,bM);
	...
	while(1){
		...
		var envtex=envtexes[envtexes.length-1];
	...
	gl.uniformMatrix4fv(targs["lightMat"],false,new Float32Array(ono3d.pvMat));
	...
	while(1){
		...
		gl.uniform3f(targs["uReflectionColor"]
			...
			,material.reflectionColor[1]
			,material.reflectionColor[2]);
		gl.uniform1f(targs["uRefract"],material.refract);
