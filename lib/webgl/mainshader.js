...
ret.init= function(){
	...
	jsbuffer=new Float32Array(4096*(3+4+3+3+3));
	gl.bufferData(gl.ARRAY_BUFFER, jsbuffer, gl.STATIC_DRAW);
...
_offset=0;
	...
	args["aPos"]=initAtt(program,"aPos",3,gl.FLOAT,glbuffer,4096);
	args["aColor"]=initAtt(program,"aColor",4,gl.FLOAT,glbuffer,4096);
	args["aNormal"]=initAtt(program,"aNormal",3,gl.FLOAT,glbuffer,4096);
	args["aSvec"]=initAtt(program,"aSvec",3,gl.FLOAT,glbuffer,4096);
	args["aTvec"]=initAtt(program,"aTvec",3,gl.FLOAT,glbuffer,4096);
	...
	ret.draw=function(ono3d,shadowTex,envtexes,camerap){
		...
		for(var i=0;i<facessize;i++){
			...
			if(material.normalmap){
				...
				gl.uniform1f(args["normalmappow"],0.05);
			}else{
			...
			gl.uniform1f(args["f0"],material.mrr);
...
var initAtt= function(program,attName,size,type,buffer,arraySize){
	var arg={};
	arg.att = gl.getAttribLocation(program,attName); 
	arg.size=size;
	arg.type=type;
	arg.offset=_offset;
	_offset+=arraySize*size;
