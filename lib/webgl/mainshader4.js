...
void main(void){ \
	...
	vec2 refV = uv +angle.xy *refract*0.2 ; \
...
var renderParallax = function(ono3d,camerap){
	...
	for(var i=0;i<arrIndex.length;i++){
		...
		var begin=arrIndex[i].begin;
		var size=arrIndex[i].size;
		var jsize=size;
		var jmax=j;
		if(jsize>=limit){
			jsize=limit;
		...
		material = arr[begin].material;
		for(var j=0;j<size;j++){
			renderface=arr[j+begin];
...
var renderReflect = function(ono3d,opFlg){
	...
	while(i<arrlength){
		...
		gl.uniform4f(shader.args["uColor"],material.spc,material.rough,material.trans_rough,1.0- 1.0/material.ior);
