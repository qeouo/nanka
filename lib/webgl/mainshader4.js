...
void main(void){ \
	...
	/*視差*/ \
	vec2 uv = vUv + (texture2D(uParallax,st).rg -0.5)*0.01; \
	/*反射率,ラフネス,透過ラフネス,屈折率*/ \
...
ret.draw=function(ono3d,shadowTex,env2dtex,camerap,frenel){
	...
	var faceSize = 3*parallaxShader.amax;
	var jsize= arrlength*faceSize;
	
	var limit =buffers[buffers.length-1].length ;
	if(jsize>=limit){
		jsize= limit;
		arr.length=limit/faceSize;
		arrlength=arr.length;
	}
		
	var buff ;
	for(var i=0;i<buffers.length;i++){
		if(jsize<=buffers[i].length){
			buff=buffers[i];
			break;
		}
	}
...
var renderParallax = function(ono3d,camerap,opFlg){
	...
	for(var i=0;i<arrIndexes.length;i++){
		...
		if(material.normalmap){
			...
			gl.uniform1f(args["uNormpow"],material.normal*0.1);
		}else{
...
var renderPbr = function(ono3d,opFlg){
	...
	for(var i=0;i<arrIndexes.length;i++){
		...
		gl.uniform4f(args["uColor"],material.spc,material.rough,material.trans_rough,-(1.0/material.ior-1.0)*0.2);
...
var renderNormal = function(ono3d,opFlg){
	...
	for(var i=0;i<arrIndexes.length;i++){
		...
		gl.uniform1i(args["uNormalMap"],1);
		if(material.normalmap){
			gl.bindTexture(gl.TEXTURE_2D,material.normalmap.gltexture);
			gl.uniform1f(args["uNormpow"],material.normal*0.1);
		}else{
