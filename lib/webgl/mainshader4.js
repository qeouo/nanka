...
void main(void){ \
	...
	float refx = floor(sqrt(roughness/0.06)); \
	float refa = (roughness -refx*refx*0.06)/((((1.0+refx)*(1.0+refx))-refx*refx)*0.06); \
	refa = min(refa,1.0); \
	refx = pow(0.5,refx); \
	...
	vec4 q = texture2D(uEnvmap,refV*refx*0.5 + vec2(0.0,1.0-refx*0.5)); \
	q.rgb *= (q.a * 3.0 + 1.0); \
	refCol.rgb = mix(refCol.rgb,q.rgb,refa) ; \
    highp float m = max(1.0,max(refCol.r,max(refCol.g,refCol.b))); \n \
	gl_FragColor = vec4(refCol.rgb / m ,(m-1.0)/3.0); \n \
} \
"
	,[	{name:"aPos",size:2}
	]
...
void main(void){ \
	...
	float transRough= transparentdata.b; \
	...
	vec2 refV = uv +angle.xy *(1.0-refract)*0.2 ; \
	vec4 transCol = texture2D(uEnvmap,refV*refx + vec2(0.0,1.0-refx)); \
	transCol.rgb *= (transCol.a * 3.0 + 1.0); \
	vec4 q = texture2D(uEnvmap,refV*refx*0.5 + vec2(0.0,1.0-refx*0.5)); \
	q.rgb *= (q.a * 3.0 + 1.0); \
	transCol.rgb = mix(transCol.rgb,q.rgb,refa); \
    highp float m = max(1.0,max(transCol.r,max(transCol.g,transCol.b))); \n \
	gl_FragColor = vec4(transCol.rgb / m ,(m-1.0)/3.0); \n \
} \
"
	,[	{name:"aPos",size:2}
	]
...
var drawSub = function(ono3d,env2dtex,camerap,transFlg){
	...
	gl.copyTexSubImage2D(gl.TEXTURE_2D,0,0,0,0,0,WIDTH,HEIGHT);
	compositeReflect(ono3d,env2dtex,transFlg);
	gl.bindTexture(gl.TEXTURE_2D,reflectTexture);
	gl.copyTexSubImage2D(gl.TEXTURE_2D,0,0,0,0,0,WIDTH,HEIGHT);

	if(transFlg){
		//透過
		compositeTransparent(ono3d,transTexture,transFlg);
	...
	composite(ono3d,env2dtex,transFlg);
}
...
var compositeTransparent= function(ono3d,env2dtex,opFlg){
...
var compositeReflect = function(ono3d,env2dtex,opFlg){
