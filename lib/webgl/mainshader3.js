...
void main(void){ \
	...
	/*乱反射強度*/ \
	vec3 light; \
	float diffuse = max(-dot(normalize(nrm),uLight),0.0); \
	...
	refV = vec2(atan(nrm.x,-nrm.z)*_PI*0.5 + 0.5 \
		,(-atan(nrm.y,length(nrm.xz))*_PI *(1.0-1.0/(1024.0*refx)) + 0.5)*0.5); \
	refCol02 = texture2D(uEnvMap,refV*refx + vec2(0.0,1.0-refx)); \
	refCol02.rgb *= (refCol02.a * 3.0 + 1.0); \
	light = diffuse * uLightColor + uAmbColor*refCol02.rgb  + uEmi; \
	/*表面色*/ \
...
ret.draw=function(ono3d,shadowTex,env2dtex,camerap,frenel){
	...
	for(var i=1;i<4;i++){
		...
		Gauss.filter(size,size*0.5,100
			,transTexture,0,(1024-size*2)/1024,size*2/1024,size/1024,1024,1024);
		gl.bindTexture(gl.TEXTURE_2D,transTexture);
	...
	//gl.disable(gl.BLEND);
	//Rastgl.copyframe(transTexture,0,0 ,1,1.0); //今回結果を書き込み
	//Rastgl.copyframe(env2dtex,0,0 ,1,1.0); //今回結果を書き込み
