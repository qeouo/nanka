...
void main(void){ \
	...
	if(uRough<0.05) { \
		...
		refOffset = 0.5; \
		refx= 0.5; \
		refa = (uRough-0.05)/(0.2-0.05); \
	}else if(uRough<0.5){ \
		refOffset = 0.75; \
		refx= 0.25; \
		refa = (uRough-0.5)/(1.0-0.5); \
	} \
	vec2 refV = vec2(atan(angle.x,-angle.z)*_PI*0.5 + 0.5 \
		,-atan(angle.y,length(angle.xz))*_PI + 0.5); \
	vec4 refCol = texture2D(uReflectmap,refV*refx + vec2(0.0,refOffset)); \
	refCol.rgb *= (refCol.a * 3.0 + 1.0); \
	vec4 refCol02 = texture2D(uReflectmap,refV*refx*0.5 + vec2(0.0,refOffset*1.5)); \
