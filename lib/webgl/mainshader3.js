...
ret.init= function(){
	...
	subTexture = Rastgl.createTexture(null,1024,1024);
...
void main(void){ \
	...
	float refx= 1.0; \
	float refa = 1.0; \
	refx = floor(sqrt(uRough/0.06)); \
	refa = (uRough-refx*refx*0.06)/((((1.0+refx)*(1.0+refx))-refx*refx)*0.07); \
	refa = min(refa,1.0); \
	refx = pow(0.5,refx); \
	vec2 refV = vec2(atan(angle.x,-angle.z)*_PI*0.5 + 0.5 \
		,(-atan(angle.y,length(angle.xz))*_PI *(1.0-1.0/(1024.0*refx)) + 0.5)*0.5); \
	vec4 refCol = texture2D(uReflectmap,refV*refx + vec2(0.0,1.0-refx)); \
	refCol.rgb *= (refCol.a * 3.0 + 1.0); \
	vec4 refCol02 = texture2D(uReflectmap,refV*refx*0.5 + vec2(0.0,1.0-refx*0.5)); \
...
ret.draw=function(ono3d,shadowTex,env2dtex,camerap,frenel){
	...
	for(var i=1;i<5;i++){
		...
		Rastgl.copyframe(subTexture ,0,0 ,i,i*0.5); //前回の結果を重ねる
		gl.copyTexSubImage2D(gl.TEXTURE_2D,0,0,0,i32a[0],i32a[1],i32a[2],i32a[3]);
...
var render = function(ono3d,shadowTex,env2dtex,camerap,frenel,opFlg){
	...
	while(1){
		...
		gl.uniform1f(args["uInnerRough"],material.innerRough);
