...
void main(void){ \
	...
	/* 透過 */ \
	col = mix(base.rgb*transparent.rgb, col , ref.a); \
	/* 反射 */ \
...
var drawSub = function(ono3d,envtexes,camerap,transFlg){
	...
	if(transFlg){
		...
		renderTransparent(ono3d,transFlg);
...
ret.draw=function(ono3d,shadowTex,envtexes,camerap,frenel){
	...
	gl.bindTexture(gl.TEXTURE_2D,transparentShader.texture);
...
var setVertices=function(ono3d){
	...
	return {index:index,material:material};
...
var renderReflect = function(ono3d,opFlg){
	...
	while(1){
		...
		//反射強度、ラフネス,,不透明度
		gl.uniform4f(shader.args["uColor"],material.spc,material.rough,0,material.opacity);
...
var renderTransparent= function(ono3d,opFlg){
	var shader = transparentShader;
	...
	while(1){
		...
		//ラフネス,屈折率,余り,不透明度
		gl.uniform4f(shader.args["uColor"],material.rough,1.0/material.ior,0,material.opacity);

		gl.activeTexture(gl.TEXTURE0); //反射テクスチャ
		gl.uniform1i(shader.args["uSampler"],0);
		if(material.transparentTexture){
			gl.bindTexture(gl.TEXTURE_2D,material.trasparentTexture.gltexture);
		}else{
			gl.bindTexture(gl.TEXTURE_2D,Rastgl.dummyTexture); //テクスチャ未指定の場合はダミーを設定
		}
		...
		Rastgl.stereoDraw(ono3d,function(){
			gl.uniformMatrix4fv(shader.args["uProjectionMatrix"],false,new Float32Array(ono3d.pvMatrix));
			gl.drawArrays(gl.TRIANGLES, 0, ret.index);
		});
