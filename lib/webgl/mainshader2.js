...
ret.init= function(){
	...
	parallaxShader.program= Rastgl.setShaderProgram(" \
...
void main(void){ \
	...
	uv =  (vView*eye).xy * nrmmap.w * uNormpow*0.5; \
	/*表面色*/ \
	gl_FragColor =vec4((uv*10.0+1.0)*0.5,0.0,0.0); \
} \
")
	parallaxShader.args={};
	gl.useProgram(parallaxShader.program);
	parallaxShader.attributes=[
		{name:"aPos",size:3}
		,{name:"aNormal",size:3}
		,{name:"aSvec",size:3}
		,{name:"aTvec",size:3}
	...
	Rastgl.initAtts2(parallaxShader,uniforms);
	parallaxShader.texture= Rastgl.createTexture(null,TEXSIZE,TEXSIZE);


	roughnessShader.program= Rastgl.setShaderProgram(" \
precision lowp float; \
attribute vec3 aPos; \
attribute vec2 aUv; \
varying vec2 vUv; \
...
")
	roughnessShader.args={};
	gl.useProgram(roughnessShader.program);
	roughnessShader.attributes=[
		{name:"aPos",size:3}
		,{name:"aUv",size:2}
	];
	var uniforms=[
	...
	Rastgl.initAtts2(roughnessShader,uniforms);
	roughnessShader.texture= Rastgl.createTexture(null,TEXSIZE,TEXSIZE);
...
var render = function(ono3d,opFlg){
	...
	while(1){
		...
		gl.bufferSubData(gl.ARRAY_BUFFER, 0, jsbuffer);
		//gl.bufferSubData(gl.ARRAY_BUFFER, jsbuffer.length*0.5*4, jsbuffer2);
...
var renderRoughness = function(ono3d,opFlg){
	...
	while(1){
		flg=false;
		uvindex=roughness.args["aUv"].offset;
		posindex=0;
		index=0;
		for(var i=0;i<facessize;i++){
			if(faceflg[i])continue;
		...
		gl.uniform4f(roughness.args["uColor"],material.reflectColor[0]
			,material.reflectColor[1]
			,material.reflectColor[2]
			,material.rough);

		gl.activeTexture(gl.TEXTURE0); //カラーテクスチャ
		gl.uniform1i(roughness.args["uSampler"],0);
		if(material.texture){
			gl.bindTexture(gl.TEXTURE_2D,material.texture.gltexture);
		}else{
			gl.bindTexture(gl.TEXTURE_2D,Rastgl.dummyTexture); //テクスチャ未指定の場合はダミーを設定
		...
		gl.activeTexture(gl.TEXTURE1); //マップ
		gl.uniform1i(roughness.args["uParallax"],1);
		gl.bindTexture(gl.TEXTURE_2D,parallaxShader.texture);
		...
		Rastgl.stereoDraw(ono3d,function(){
			gl.uniformMatrix4fv(roughness.args["uProjectionMatrix"],false,new Float32Array(ono3d.pvMatrix));
			gl.drawArrays(gl.TRIANGLES, 0, index);
		});

		gl.bindTexture(gl.TEXTURE_2D,roughness.texture);
		gl.copyTexSubImage2D(gl.TEXTURE_2D,0,0,0,0,0,TEXSIZE,TEXSIZE);
	}
}
var renderNormal = function(ono3d,opFlg){
	...
	while(1){
		...
		gl.uniform1i(baseShader.args["uParallax"],2);
		gl.bindTexture(gl.TEXTURE_2D,parallaxShader.texture);
