...
void main(void){ \
	...
	/*反射強度とラフネス*/ \
	vec4 reflect =  texture2D(uReflectmap,uv); \
	...
	vec4 result ; \
 	if(reflect.g<0.1){ \
		env1 =  textureCube(uEnvmap0,nrm); \
		env2 =  textureCube(uEnvmap1,nrm); \
		a = (0.1 -reflect.g )/0.1; \
	}else if(reflect.g<0.2){ \
		env1 =  textureCube(uEnvmap1,nrm); \
		env2 =  textureCube(uEnvmap2,nrm); \
		a = (0.2 - reflect.g )/0.1; \
	}else if(reflect.g<0.5){ \
		env1 =  textureCube(uEnvmap2,nrm); \
		env2 =  textureCube(uEnvmap3,nrm); \
		a = (0.5 - reflect.g)/0.3; \
	}else if(reflect.g<0.8){ \
		env1 =  textureCube(uEnvmap3,nrm); \
		env2 =  textureCube(uEnvmap4,nrm); \
		a = (0.8 - reflect.g )/0.3; \
	}else { \
		env1 =  textureCube(uEnvmap4,nrm); \
		env2 =  textureCube(uEnvmap5,nrm); \
		a = (1.0 - reflect.g )/0.2; \
	} \
	gl_FragColor = env1* a + env2*(1.0-a); \
} \
"
	,[	{name:"aPos",size:2}
	]
...
void main(void){ \
	...
	light  = light + env.rgb*0.4; \
	gl_FragColor = vec4(base.rgb*light,0.0); \
} \
"
	,[ {name:"aPos",size:2}
	]
...
ret.draw=function(ono3d,shadowTex,envtexes,camerap,frenel){
	...
	gl.clear(gl.COLOR_BUFFER_BIT);
	renderReflect(ono3d,envtexes,false);
...
var renderReflect = function(ono3d,envtexes,opFlg){
	...
	while(1){
		...
		//反射強度、ラフネス
		gl.uniform4f(shader.args["uColor"],material.mrr,material.rough,0,0);
