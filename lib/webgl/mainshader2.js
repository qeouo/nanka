...
")
	...
	parallax2Shader.program= Rastgl.setShaderProgram(" \
precision lowp float; \
attribute vec3 aPos; \
uniform mat4 uProjectionMatrix; \
void main(void){ \
...
")
	parallax2Shader.args={};
	gl.useProgram(parallax2Shader.program);
	parallax2Shader.attributes=[
		{name:"aPos",size:3}
	];
	var uniforms=[
		"uProjectionMatrix"
	];
	Rastgl.initAtts2(parallax2Shader,uniforms);

	reflectShader.program= Rastgl.setShaderProgram(" \
precision lowp float; \
attribute vec3 aPos; \
attribute vec2 aUv; \
varying vec2 vUv; \
...
")
	reflectShader.args={};
	gl.useProgram(reflectShader.program);
	reflectShader.attributes=[
		{name:"aPos",size:3}
		,{name:"aUv",size:2}
	];
	var uniforms=[
	...
	Rastgl.initAtts2(reflectShader,uniforms);
