...
void main(void){ \
	...
	highp vec4 yp= texture2D(uSampler \
		,vec2(vUv.s,vUv.t+uy)); \
	highp vec3 aa=vec3((-(xp.r-def.r)/ux*0.5+0.5),(-(yp.r-def.r)/uy*0.5+0.5),1.0); \
...
"
	...
	args["size"]=gl.getUniformLocation(program,"size");
	}
	...
	ret.draw=function(src,x,y){
		...
		gl.uniform1f(args["ux"],1.0/64.);
		gl.uniform1f(args["uy"],1.0/64.);
		gl.uniform1f(args["size"],x);
