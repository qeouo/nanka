...
var OpenEXR=(function(){
	...
	ret.toArrayBuffer = function(obj){
		...
		var data_stream = new DataStream(new ArrayBuffer(line_size));
		var data_stream2 = new DataStream(new ArrayBuffer(line_size));
		for(var li=0;li<height;li++){
			...
			var data =new Uint8Array(data_stream.dv.buffer);
			if(obj.attributes.compression){
				...
				data2 = new Uint8Array(Deflate.compress(data2,1));
				data = new Uint8Array(data2.length+2);
				data[0]=(7<<4) + 8;
				data[1]=(2<<6) + 0;
	...
	//			idx+=value.length+1;
		...
		for(var i=0;i<obj.height;i++){
			...
			if(attributes.compression===0){
				...
				linemax=1;//compression=2 のときは1行固定
//パリティ計算
//				var aa=1,bb=0;
